...
create.params <- function(user.input){
  with(as.list(user.input),{
    ...
    if (sex == "M"){
      RAFOatp_k <- 9.784422e-01
      RAFOat1 <- 1.615647e+04
      RAFOatp_l <- 5.996414e+01
      
      
    }else if(sex == "F"){
      RAFOatp_k <- 1.982194e-08
      RAFOat1 <- 1.000000e+10
      RAFOatp_l <- 6.043252e-07
    ...
    RAFOatp_lu_ap <- 5.225584e+00
    RAFOatp_lu_bas <- RAFOatp_lu_ap
    RAFNtcp <- RAFOatp_l
    RAFOatp2_Int <- 7.876250e+01
    f_fabp_avail <- 1
    f_alb_avail <- 1
    
    P_liver_bile <- 8.001414e+00 
    ...
    return(list('VB'=VB, 'Vplasma'=Vplasma, 'VK'=VK, 'VKB'=VKB, 
                ...
                'PeffSK' = PeffSK,  'PeffBo' = PeffBo, 'PeffUA'=PeffUA, "P_liver_bile" = P_liver_bile,
