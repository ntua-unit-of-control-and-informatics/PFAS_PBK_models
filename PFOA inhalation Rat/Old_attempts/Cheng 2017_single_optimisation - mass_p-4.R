...
ode.func <- function(time, inits, params){
  with(as.list(c(inits, params)),{
    ...
    CLuA = MLuA/VLuA # alveolar region concentration
    CLuTB = MLuTB/VLuTB #Tracheobronchial region concentration
    ...
    #Calculation of free concentrations in alveolar region
    CLuAf = CLuA * 1.0 / (1.0 + CalbLuA * Ka)
    ...
    #Stomach lumen
    dMSTL = -kabST*CSTL - QGE*CSTL + CLEtb*CLuTB + CLEua*CUA
    ...
    #Upper airways
    dMUA = IVR*EC*Dua - CLEua*CUA - kUABrT* (CUA - CBrT) - kUAB * (CUA - CBven)
    
    #Lung (CAUTION: returns should be corrected accordingly!!)
    ...
    dMLuF = CLuBf*QBLu/500 - CLuFf*QBLu/500 + PeffLu*ALu*(CLuBf-CLuFf) - kLuFLuA*(CLuFf-CLuAf) - (kLuF*CLuFf - kPCF*CLuFf)
    #Alveolar region (it has albumin)
    dMLuA = IVR*EC*Dar  + kLuFLuA*(CLuFf-CLuAf) #- (kLuA*CLuAf - kPCal*CLuAf)
    #Tracheobronchial region
    dMLuTB = IVR*EC*Dtb - CLEtb*CLuTB #+ kPCtb*CPCal
    
    # #PCs
    # #Alveolar region PCs
    # dMAPC = (kLuA*CLuAf - kPCal*CLuAf) - kPCtb*CPCal
    # #Interstitial fluid PCs
    # dMFPC = kLuF*CLuFf - kPCF*CLuFf
    ...
    dMBrT = kBrFBrT*(CBrFf -CBrT) + kUABrT* (CUA - CBrT)
...
#======================================df1=========================================================
  ...
  Mfeces_dry <- Mfeces_wet*0.8 # Enqi et al., 2021, control rats:20.1%, https://doi.org/10.3389/fcimb.2020.581974
