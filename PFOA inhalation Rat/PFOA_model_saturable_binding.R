...
create.params <- function(user.input){
  with(as.list(user.input),{
    ...
    #Muscle
    PVMu <- 40.43e-2 #Brown et al. 1997
    VMu <- PVMu * BW #muscle volume kg=L
    PVMB <- 0.04 #Brown et al. 1997
    VMuB <- PVMB * PVMu * BW #muscle blood volume kg=L
    PVMF <- 0.054 #Ng, 2013
...
ode.func <- function(time, inits, params){
  with(as.list(c(inits, params)),{
    ...
    # Concentrations in ug/L, masses in ug, volumes in L, flows in L/h
    ...
    MKiF <- MKiFf + MKiFb
    MKiT <- MKiTf + MKiTb
    CKiB <- MKiB/VKiB # blood concentration
    CKiBf <- MKiBf/VKiB
    CKiBb <- MKiBb/VKiB
    ...
    MLiF <- MLiFf + MLiFb
    MLiT <- MLiTf + MLiTb
    CLiB <- MLiB/VLiB # blood concentration
    CLiBf <- MLiBf/VLiB
    CLiBb <- MLiBb/VLiB
    CLiF <- MLiF/VLiF  #interstitial fluid concentration
    CLiFf <- MLiFf/VLiF
    CLiFb <- MLiFb/VLiF
    ...
    MMuF <- MMuFf + MMuFb
    MMuT <- MMuTf
    CMuB <- MMuB/VMuB # blood concentration
    CMuBf <- MMuBf/VMuB
    CMuBb <- MMuBb/VMuB
    CMuF <- MMuF/VMuF  #Interstitial fluid concentration
    CMuFf <- MMuFf/VMuF
    CMuFb <- MMuFb/VMuF
    CMuT <-  MMuT/VMuT # tissue concentration
    
    #Adipose
    MAdB <- MAdBf + MAdBb
    MAdF <- MAdFf + MAdFb
    MAdT <- MAdTf
    CAdB <- MAdB/VAdB # blood concentration
    CAdBf <- MAdBf/VAdB
    CAdBb <- MAdBb/VAdB
    CAdF <- MAdF/VAdF  #Interstitial fluid concentration
    CAdFf <- MAdFf/VAdF
    CAdFb <- MAdFb/VAdF
    CAdT <- MAdT/VAdT # tissue concentration
    
    #Rest-of-the-body
    MReB <- MReBf + MReBb
    MReF <- MReFf + MReFb
    MReT <- MReTf
    CReB <- MReB/VReB # blood concentration
    CReBf <- MReBf/VReB
    CReBb <- MReBb/VReB
    CReF <- MReF/VReF  #Interstitial fluid concentration
    CReFf <- MReFf/VReF
    CReFb <- MReFb/VReF
    CReT <- MReT/VReT # tissue concentration
    ...
    MHtF <- MHtFf + MHtFb
    MHtT <- MHtTf
    CHtB <- MHtB/VHtB # blood concentration
    CHtBf <- MHtBf/VHtB
    CHtBb <- MHtBb/VHtB
    CHtF <- MHtF/VHtF  #Interstitial fluid concentration
    CHtFf <- MHtFf/VHtF
    CHtFb <- MHtFb/VHtF
    CHtT <- MHtT/VHtT # tissue concentration
    ...
    dMLuAFf <- 0 #kLuTLuAF * (CLuAF-CLuT) - Qp * CLuAF/PLungA + IVR*Cair*Cpfoa*dfalveolar
    ...
    Ckidney <- (MKiB + MKiF+ MKiT)/(VKiB+VKiF+VKiT)
    Cliver <- (MLiB + MLiB+ MLiT)/(VLiB+VLiF+VKiT)
    Cstomach <-  (MStB + MStF+ MStT)/(VStB+VStF+VStT)
    Cintestine <-  (MInB + MInF+ MInT+MInL)/(VInB+VInF+VInT+VInL)
    Cmuscle <-  (MMuB + MMuF+ MMuT)/(VMuB+VMuF+VMuT)
    Cadipose <-  (MAdB + MAdF+ MAdT)/(VAdB+VAdF+VAdT)
    ...
    list(c( 'dCalbVenf' = dCalbVenf,  'dMVenb' = dMVenb, 'dCalbArtf' = dCalbArtf, 
            'dMArtb' = dMArtb, 'dCalbLyf' = dCalbLyf, 
            'dMLyb' = dMLyb, 'dCalbKiBf' = dCalbKiBf, 'dMKiBb' = dMKiBb, 
            'dCalbLiBf' = dCalbLiBf, 
            'dMLiBb' = dMLiBb, 'dCalbStBf' = dCalbStBf, 
            'dMStBb' = dMStBb, 'dCalbInBf' = dCalbInBf,
            'dMInBb' = dMInBb, 
            'dCalbMuBf' = dCalbMuBf,  'dMMuBb' = dMMuBb,
            'dCalbAdBf' = dCalbAdBf, 'dMAdBb' = dMAdBb, 
            'dCalbReBf' = dCalbReBf, 
            'dMReBb' = dMReBb, 'dCalbLuBf' = dCalbLuBf, 
            'dMLuBb' = dMLuBb, 'dCalbSpBf' = dCalbSpBf,  'dMSpBb' = dMSpBb, 
            'dCalbHtBf' = dCalbHtBf,
            'dMHtBb' = dMHtBb, 'dCalbBrBf' = dCalbBrBf,
            'dMBrBb' = dMBrBb, 'dCalbGoBf' = dCalbGoBf, 
            'dMGoBb' = dMGoBb, 
            'dCalbSkBf' = dCalbSkBf,  'dMSkBb' = dMSkBb, 'dCalbKiFf' = dCalbKiFf, 
            'dMKiFb' = dMKiFb, 'dCalbLiFf' = dCalbLiFf,'dMLiFb' = dMLiFb, 
            'dCalbStFf' = dCalbStFf, 'dMStFb' = dMStFb, 'dCalbInFf' = dCalbInFf, 
            'dMInFb' = dMInFb, 'dCalbMuFf' = dCalbMuFf, 
            'dMMuFb' = dMMuFb, 
            'dCalbAdFf' = dCalbAdFf, 'dMAdFb' = dMAdFb, 'dCalbReFf' = dCalbReFf, 
            'dMReFb' = dMReFb, 'dCalbLuFf' = dCalbLuFf, 
            'dMLuFb' = dMLuFb, 
            'dCalbSpFf' = dCalbSpFf, 'dMSpFb' = dMSpFb, 'dCalbHtFf' = dCalbHtFf, 
            'dMHtFb' = dMHtFb, 'dCalFbrFf' = dCalFbrFf, 
            'dMBrFb' = dMBrFb, 
            'dCalbGoFf' = dCalbGoFf, 'dMGoFb' = dMGoFb, 'dCalbSkFf' = dCalbSkFf, 
            'dMSkFb' = dMSkFb, 'dCA2uKiTf' = dCA2uKiTf, 'dCFabpKiTf' = dCFabpKiTf, 
            'dMKiTb' = dMKiTb, 'dCFabpLiTf' = dCFabpLiTf, 'dMLiTb' = dMLiTb,
...
create.inits <- function(parameters){
  with(as.list(parameters),{
    CalbVenf<- CalbB_init; MVenf<- 0; MVenb<- 0; CalbArtf<- CalbB_init; 
    MArtf<- 0; MArtb<- 0; CalbLyf<- CalbB_init; MLyf<- 0; MLyb<- 0;
    CalbKiBf<- CalbB_init; MKiBf<- 0; MKiBb<- 0; CalbLiBf<- CalbB_init; MLiBf<- 0; 
    MLiBb<- 0; CalbStBf<- CalbB_init; MStBf<- 0; MStBb<- 0; CalbInBf<- CalbB_init;
    MInBf<- 0; MInBb<- 0; CalbMuBf<- CalbB_init; MMuBf<- 0; MMuBb<- 0; 
    CalbAdBf<- CalbB_init; MAdBf<- 0; MAdBb<- 0; CalbReBf<- CalbB_init; MReBf<- 0; 
    MReBb<- 0; CalbLuBf<- CalbB_init; MLuBf<- 0; MLuBb<- 0; CalbSpBf<- CalbB_init;
    MSpBf<- 0; MSpBb<- 0; CalbHtBf<- CalbB_init; MHtBf<- 0; MHtBb<- 0; 
    CalbBrBf<- CalbB_init; MBrBf<- 0; MBrBb<- 0;CalbGoBf<- CalbB_init; MGoBf<- 0;
    MGoBb<- 0; CalbSkBf<- CalbB_init; MSkBf<- 0; MSkBb<- 0; CalbKiFf<- CalbKiF_init; 
    ...
    return(c('CalbVenf' = CalbVenf,  'MVenb' = MVenb, 'CalbArtf' = CalbArtf, 
             'MArtb' = MArtb, 'CalbLyf' = CalbLyf, 
             'MLyb' = MLyb, 'CalbKiBf' = CalbKiBf, 'MKiBb' = MKiBb, 
             'CalbLiBf' = CalbLiBf, 
             'MLiBb' = MLiBb, 'CalbStBf' = CalbStBf, 
             'MStBb' = MStBb, 'CalbInBf' = CalbInBf,
             'MInBb' = MInBb, 
             'CalbMuBf' = CalbMuBf,  'MMuBb' = MMuBb,
             'CalbAdBf' = CalbAdBf, 'MAdBb' = MAdBb, 
             'CalbReBf' = CalbReBf, 
             'MReBb' = MReBb, 'CalbLuBf' = CalbLuBf, 
             'MLuBb' = MLuBb, 'CalbSpBf' = CalbSpBf,  'MSpBb' = MSpBb, 
             'CalbHtBf' = CalbHtBf,
             'MHtBb' = MHtBb, 'CalbBrBf' = CalbBrBf,
             'MBrBb' = MBrBb, 'CalbGoBf' = CalbGoBf, 
             'MGoBb' = MGoBb, 
             'CalbSkBf' = CalbSkBf,  'MSkBb' = MSkBb, 'CalbKiFf' = CalbKiFf, 
             'MKiFb' = MKiFb, 'CalbLiFf' = CalbLiFf,'MLiFb' = MLiFb, 
             'CalbStFf' = CalbStFf, 'MStFb' = MStFb, 'CalbInFf' = CalbInFf, 
             'MInFb' = MInFb, 'CalbMuFf' = CalbMuFf, 
             'MMuFb' = MMuFb, 
             'CalbAdFf' = CalbAdFf, 'MAdFb' = MAdFb, 'CalbReFf' = CalbReFf, 
             'MReFb' = MReFb, 'CalbLuFf' = CalbLuFf, 
             'MLuFb' = MLuFb, 
             'CalbSpFf' = CalbSpFf, 'MSpFb' = MSpFb, 'CalbHtFf' = CalbHtFf, 
             'MHtFb' = MHtFb, 'CalFbrFf' = CalFbrFf, 
             'MBrFb' = MBrFb, 
             'CalbGoFf' = CalbGoFf, 'MGoFb' = MGoFb, 'CalbSkFf' = CalbSkFf, 
             'MSkFb' = MSkFb, 'CA2uKiTf' = CA2uKiTf, 'CFabpKiTf' = CFabpKiTf, 
             'MKiTb' = MKiTb, 'CFabpLiTf' = CFabpLiTf, 'MLiTb' = MLiTb,
             
             'MArtf'=MArtf, 'MVenf'=MVenf, 'MLyf'=MLyf, 'MKiBf'=MKiBf, 
             'MKiFf'=MKiFf, 'MKiTf'=MKiTf,
             'MFil'=MFil,  'MLiBf'=MLiBf,  'MLiFf'=MLiFf,
             'MLiTf'=MLiTf, 'Mbile'=Mbile,
             'MStL'=MStL,'MInL'=MInL,
...
ub = c(rep(log(1e5), 6),log(1e5),log(1e5),log(1e10),log(1e5), rep(log(1e5),3))
...
save.image("No_saturable_binding.RData")
