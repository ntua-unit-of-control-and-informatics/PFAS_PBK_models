...
create_params <- function(user.input){
  with(as.list(user.input),{  
    ...
    Peff_monolayer <-  Peff_monolayer*3600
    Peff_monolayer_ki1 <-  Peff_monolayer_ki1*3600
    Peff_monolayer_ki2 <-  Peff_monolayer_ki2*3600
    RAFUrat <- RAFOatp_k
    RAFOat1 <- RAFOat3
    ...
    #Papp = Peff_RYU
    k_gut_in = ( (2*Peff_monolayer/100) *AINL)*1000 #L/h
    k_gut_out = ( (2*Peff_monolayer/100) *AINL)*1000 #L/h
    ...
    kLFLT = ((2*Peff_monolayer/100) *AcL)*1000 #m^3/h * 1000 --> L/h
    #kLTLbile = ((Papp/100) * AcLBilec)*1000 #m^3/h * 1000 --> L/h
    kMFMT = ((2*Peff_monolayer/100) *AcM)*1000 #m^3/h * 1000 --> L/h
    kSTFSTT = ((2*Peff_monolayer/100) *AcST)*1000 #m^3/h * 1000 --> L/h 
    kINFINT = ((2*Peff_monolayer/100) *AcIN)*1000 #m^3/h * 1000 --> L/h 
    kAFAT = ((2*Peff_monolayer/100) *AcA)*1000 #m^3/h * 1000 --> L/h 
    kLuTLuF = ((2*Peff_monolayer/100) *AcLu)*1000 #m^3/h * 1000 --> L/h
    kLuTLuAF = ((2*Peff_monolayer/100) *AcALF)*1000 #m^3/h * 1000 --> L/h
    kSPFSPT = ((2*Peff_monolayer/100) *AcSP)*1000 #m^3/h * 1000 --> L/h 
    kHFHT = ((2*Peff_monolayer/100) *AcH)*1000 #m^3/h * 1000 --> L/h 
    kBrFBrT = ((2*Peff_monolayer/100) *AcBr)*1000 #m^3/h * 1000 --> L/h 
    kGoFGoT = ((2*Peff_monolayer/100) *AcGo)*1000 #m^3/h * 1000 --> L/h 
    kSKFSKT = ((2*Peff_monolayer/100) *AcSK)*1000 #m^3/h * 1000 --> L/h
    kBoFBoT = ((2*Peff_monolayer/100) *AcBo)*1000 #m^3/h * 1000 --> L/h
    kRFRT = ((2*Peff_monolayer/100) *AcR)*1000 #m^3/h*1000 --> L/h 
    
    #Diffusion rates in L/h between renal tubule filtrate and tubule cells
    kPtcTu <- ((2*Peff_monolayer_ki1/100) *APT) *1000 #diffusion between proximal tubule cells and tubule filtrate
    kDalcTu <- ((2*Peff_monolayer_ki1/100) *ADAL) *1000 #diffusion between descending/ascending cells and tubule filtrate
    kDtcTu <- ((2*Peff_monolayer_ki1/100) *ADT) *1000 #diffusion between distal tubule cells and tubule filtrate
    kCdcTu <- ((2*Peff_monolayer_ki1/100) *ACD) *1000 #diffusion between collecting duct cells and tubule filtrate
    
    #Diffusion rates in L/h between  tubule cells and interstitial space
    kDtcF <- ((2*Peff_monolayer_ki2/100) *AcK_DTC) *1000
    kPtcF <- ((2*Peff_monolayer_ki2/100) *AcK_PTC) *1000
    kDalcF <- ((2*Peff_monolayer_ki2) *AcK_DALC) *1000 #diffusion between proximal tubule cells and interstitial space
    kCdcF <- ((2*Peff_monolayer_ki2/100) *AcK_CDC) *1000 #diffusion between descending/ascending cells and interstitial space
    kKTrestF  <- ((2*Peff_monolayer_ki2/100) *AcKTrest) *1000 #diffusion between rest of kidney cells and interstitial space
    ...
    return(list(
      ...
      'KmK_baso' = 1e20, 'KmK_api' = 1e20,
      'VmK_baso' = 0,'VmK_api' = 0,
...
PBPK_sensitivity <- function(model, thetas, thetas_names, user_input,
                             ranges, targets,
                             heatmap = FALSE,
                             sample_time){
  ...
  user_input <- c(user_input_0, thetas)
  ...
  if(heatmap){
    ...
    my_colors <- colorRampPalette(c("navy", "blue", "white", "red", "darkred"))(100)
    
    # Force zero to be in the middle by setting symmetric limits
    get_symmetric_limits <- function(data_matrix) {
      max_abs_value <- max(abs(data_matrix))
      return(c(-max_abs_value, max_abs_value))
    }
    
    # Get symmetric limits for AUC
    auc_limits <- get_symmetric_limits(SI_auc)
    
    # Create only 3 legend breaks: min, 0, max
    legend_breaks <- c(auc_limits[1], 0, auc_limits[2])
    ...
    heatmap1 <- pheatmap::pheatmap(as.matrix(SI_auc),
                                   ...
                                   fontsize_col = 20,
                                   col = my_colors,
                                   breaks = seq(auc_limits[1], auc_limits[2], length.out = 101),
                                   main="AUC Sensitivity Indexes")
    
    AUC_heatmap <- recordPlot(heatmap1)
    
    # Get symmetric limits for Cmax
    cmax_limits <- get_symmetric_limits(SI_cmax)
    ...
    heatmap2 <- pheatmap::pheatmap(as.matrix(SI_cmax),
                                   ...
                                   fontsize_col = 20,
                                   col = my_colors,
                                   breaks = seq(cmax_limits[1], cmax_limits[2], length.out = 101),
                                   main="Cmax Sensitivity Indexes")
...
#-------------------------------------------------------------------------------
#                      END OF FUNCTIONS FOR SA
#-------------------------------------------------------------------------------

input_all <- c("RAFOatp_k", "f_fabp_avail", "RAFOat3", "CL_int", "RAFOatp_l",
               "Peff_monolayer",  "Pgap_continuous", "f_alb_avail", "koff_alb", "koff_fabp", "KLfabp" ,
               "Ka", "CLfeces_unscaled", "RAFOatp_lu_ap" )
input_oral_specific <- "RAFOatp2_Int"
input_inhalation_specific <- "kCLEal"
input_nasal_specific <-   c("kabsUA", "kCLEua")

set_default_params <- function(sex){
  if(sex == "M"){
  ...
  Peff_monolayer <-  1.27e-06
  Pgap_continuous <- 1e-04#2.774701e-02 #, cm/s, permeability by diffusion through tight junctions in continuous capillary types
  ...
  kabsUA <-  0.03718735 #L/h/m^2
  kCLEal <- 0.0005692359#L/h/m^2
  kCLEua <- 3.531062#L/h/m^2
  RAFOatp_lu_ap <-  1.039169
  Peff_monolayer_ki2<-  0.004593883
  Peff_monolayer_ki1<-  0.004593883
  
  
  return(c(
    "RAFOatp_k" = RAFOatp_k,
    "f_fabp_avail" = f_fabp_avail,
    "RAFOat3" = RAFOat3,
    "CL_int" = CL_int,
    "RAFOatp_l" = RAFOatp_l,
    "RAFOatp2_Int" = RAFOatp2_Int,
    "Peff_monolayer" = Peff_monolayer,
    "Pgap_continuous" = Pgap_continuous,
    "f_alb_avail" = f_alb_avail,
    "koff_alb" = koff_alb,
    "koff_fabp" = koff_fabp,
    "KLfabp" = KLfabp,
    "Ka" = Ka,
    "CLfeces_unscaled" = CLfeces_unscaled,
    "kabsUA" = kabsUA,
    "kCLEal" = kCLEal,
    "kCLEua" = kCLEua,
    "RAFOatp_lu_ap" = RAFOatp_lu_ap,
      "Peff_monolayer_ki1" = Peff_monolayer_ki1,
      "Peff_monolayer_ki2" = Peff_monolayer_ki2
  ))
}

output_all <- c( 'Cplasma', 'Ckidney', 'Cliver',  'Cstomach', 
                 'Cintestine', 'Cmuscle', 'Cadipose',
                 'Clungs',  'Crest', 'Cspleen', 'Cheart', 'Cbrain', 
                 'Mbrain', 'Cgonads', 'Cskin', 'Cbones',
                 'Mfeces', 'Murine')
output_inhalation_specific <- c('CalveolarLF')
output_nasal_specific <- c('CalveolarLF', 'CUpperair')
...
res=100
#---------------------------------------
#    Case 1: Male, oral, Low dose
#----------------------------------
BW <- 0.3  # body weight (kg)Pgap_continuous
admin.dose_per_g <- 0.05 # administered dose in mg PFOA/kg BW 
...
default_params <- set_default_params(sex)
params <- create_params(c(user_input,default_params))
inits <- create.inits(params)
events <- create.events(params)
sample_time=seq(0,50,1)

thetas = default_params
thetas_names = names(thetas)
test_1_positive = PBPK_sensitivity(model=ode.func,
                                   thetas=thetas,
                                   thetas_names=thetas_names,
                                   user_input=user_input,
                                   ranges= 0.5,
                                   targets=output_all,
                                   heatmap = TRUE, sample_time=sample_time)

test_1_negative = PBPK_sensitivity(model=ode.func,
                                   thetas=thetas,
                                   thetas_names=thetas_names,
                                   user_input=user_input,
                                   ranges=-0.5,
                                   ...
                                   heatmap = TRUE, sample_time=sample_time)
# Positive change
png(filename = "Sensitivity_plots/test_1/positive/AUC_heatmap_plot.png", width = width, height = height, res=res)
replayPlot(test_1_positive$Plot_list$AUC_heatmap)
dev.off()
png(filename = "Sensitivity_plots/test_1/positive/Cmax_heatmap_plot.png", width = width, height = height, res=res)
replayPlot(test_1_positive$Plot_list$Cmax_heatmap)
dev.off()

# Negative change
png(filename = "Sensitivity_plots/test_1/negative/AUC_heatmap_plot.png", width = width, height = height, res=res)
replayPlot(test_1_negative$Plot_list$AUC_heatmap)
dev.off()
png(filename = "Sensitivity_plots/test_1/negative/Cmax_heatmap_plot.png", width = width, height = height, res=res)
replayPlot(test_1_negative$Plot_list$Cmax_heatmap)
dev.off()
