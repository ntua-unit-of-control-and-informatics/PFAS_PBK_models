...
ode.func <- function(time, inits, params){
  with(as.list(c(inits, params)),{
    ...
    dMBart = QBLu*CLuBf - CBfart*(QBK+QBL+QBM+QBA+QBR+QBSP+QBH+QBBr+
                                             QBST+QBIN+QBT+QBSK) - QGFR*CBfart
    
    #Venous Blood
    dMBven = - QBLu*CBfven + QBK*CKBf + QBLtot*CLBf + QBM*CMBf + QBA*CABf + QBR*CRBf+
               QBH*CHBf + QBBr*CBrBf+ QBT*CTBf + QBSK*CSKBf
    
    #Kidney
    #blood subcompartment
    dMBK = QBK*CBfart - QBK*CKBf - PeffK*AK*(CKBf-CKFf) 
    #interstitial fluid subcompartment
    dMKF = PeffK*AK*(CKBf-CKFf) - kKFKT*(CKFf-CKTf) -
    ...
    #Liver
    #blood subcompartment
    dMBL = QBL*CBfart + QBSP*CSPBf + QBIN*CINBf + QBST*CSTBf - QBLtot*CLBf - PeffL*AL*(CLBf-CLFf)
    #interstitial fluid subcompartment 
    dMLF = PeffL*AL*(CLBf-CLFf) - kLFLT*(CLFf-CLTf) - (VmL_Oatp*CLFf/(KmL_Oatp+CLFf)) - 
    ...
    #Stomach
    #blood subcompartment
    dMBST = QBST*CBfart - QBST*CSTBf - PeffST*AST*(CSTBf-CSTFf) 
    #interstitial fluid subcompartment 
    dMSTF =  PeffST*AST*(CSTBf-CSTFf) - kSTFSTT*(CSTFf-CSTT)
    ...
    #Intestine
    #blood subcompartment
    dMBIN = QBIN*CBfart - QBIN*CINBf - PeffIN*AIN*(CINBf-CINFf)
    #interstitial fluid subcompartment 
    dMINF = PeffIN*AIN*(CINBf-CINFf) - kINFINT*(CINFf-CINT) 
    #Intestine tissue subcompartment
    dMINT = kINFINT*(CINFf-CINT) + kabIN*CINL + (VmIn_Oatp2*CINL/(KmIn_Oatp2+CINL))
    #Intestine lumen
    dMINL = QGE*CSTL - (Qfeces*CINL) - kabIN*CINL + P_liver_bile*Qbile*CLTf - 
           (VmIn_Oatp2*CINL/(KmIn_Oatp2+CINL))

    
    #Muscle
    #blood subcompartment
    dMBM = QBM*CBfart - QBM*CMBf - PeffM*AM*(CMBf-CMFf) 
    #interstitial fluid subcompartment 
    dMMF = PeffM*AM*(CMBf-CMFf) - kMFMT*(CMFf- CMT)
    ...
    #Adipose
    #blood subcompartment
    dMBA = QBA*CBfart - QBA*CABf - PeffA*AA*(CABf-CAFf) 
    #interstitial fluid subcompartment 
    dMAF = PeffA*AA*(CABf-CAFf) - kAFAT*(CAFf-CAT) 
    ...
    #Rest of body
    #blood subcompartment
    dMBR = QBR*CBfart - QBR*CRBf - PeffR*AR*(CRBf-CRFf) 
    #interstitial fluid subcompartment 
    dMRF = PeffR*AR*(CRBf-CRFf) - kRFRT*(CRFf -CRT) 
    ...
    #Lung 
    #blood subcompartment
    dMBLu = CBfven*QBLu - QBLu*CLuBf - PeffLu*ALu*(CLuBf-CLuFf) 
    #interstitial fluid subcompartment
    dMLuF = PeffLu*ALu*(CLuBf-CLuFf) + kLuTLuF*(CLuT-CLuFf)
    #Lung tissue
    ...
    #Spleen
    #blood subcompartment
    dMBSP = QBSP*CBfart - QBSP*CSPBf - PeffSP*ASP*(CSPBf-CSPFf)
    #interstitial fluid subcompartment 
    dMSPF = PeffSP*ASP*(CSPBf-CSPFf) - kSPFSPT*(CSPFf -CSPT) 
    ...
    #Heart
    #blood subcompartment
    dMBH = QBH*CBfart - QBH*CHBf - PeffH*AH*(CHBf-CHFf)
    #interstitial fluid subcompartment 
    dMHF = PeffH*AH*(CHBf-CHFf) - kHFHT*(CHFf -CHT) 
    ...
    #Gonads
    #blood subcompartment
    dMBT = QBT*CBfart - QBT*CTBf - PeffT*AT*(CTBf-CTFf)
    #interstitial fluid subcompartment 
    dMTF = PeffT*AT*(CTBf-CTFf) - kTFTT*(CTFf -CTT) 
    ...
    #Skin
    #blood subcompartment
    dMBSK = QBSK*CBfart - QBSK*CSKBf - PeffSK*ASK*(CSKBf-CSKFf) 
    #interstitial fluid subcompartment
    dMSKF = PeffSK*ASK*(CSKBf-CSKFf) - kSKFSKT*(CSKFf -CSKT)
...
lb	= c(rep(log(1e-2), 3),log(1e-3),log(1e-2),rep(log(1e-2),3),log(1e-3),log(1e-2),log(1e-3),log(1e-2),log(1e-2))
