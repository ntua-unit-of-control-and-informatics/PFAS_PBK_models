...
create_variable_params <- function(BW,sex,  estimated_params, fixed_params){
  ...
  CL_int <- estimated_params[3] #uL/min/million hepatocytes
  RAFOatp_l <- estimated_params[4]
  RAFUrat <- RAFOatp_k
  RAFOat1 <- RAFOat3
  RAFOatp2_l <- RAFOatp_l
  RAFOatp_lu_ap <- estimated_params[5]
  RAFOatp_lu_bas <- RAFOatp_lu_ap
  RAFNtcp <- RAFOatp_l
  RAFOatp2_Int <- estimated_params[6]
  
  Papp <- estimated_params[7]
  Papp_gut <- Papp
...
ode.func <- function(time, inits, params){
  ...
  with(as.list(c(inits, params)),{
    ...
    dMPTCf =  QparaKi*(1-SKi)*CKBf + PeffK*A_peritubular_PTC*(CKBf-CPTCf) + 
      kPtcF*(CKFf-CPTCf) - kPtcTu*(CPTCf - frac*CPT)  +
    ...
    dMDALCf =  kDalcF*(CKFf-CDALCf) - kDalcTu*(CDALCf - frac*CDAL)- 
      (kon_a2u*Ca2uDALCf*CDALCf*VDALC + kon_fabp*CFabpDALCf*CDALCf*VDALC -
         koff_fabp*CDALCb*VDALC - koff_a2u*CDALCb*VDALC )
    
    #Distal convoluted tubule cells 
    dMDTCf =   PeffK*A_peritubular_DTC*(CKBf-CDTCf) +kDtcF*(CKFf-CDTCf)- kDtcTu*(CDTCf - frac*CDT)-
      (kon_a2u*Ca2uDTCf*CDTCf*VDTC + kon_fabp*CFabpDTCf*CDTCf*VDTC -
         koff_fabp*CDTCb*VDTC - koff_a2u*CDTCb*VDTC )
    
    #Collecting duct cells 
    dMCDCf =  kCdcF*(CKFf-CCDCf) - kCdcTu*(CCDCf - frac*CCD)- 
    ...
    dMPT =  QGFR*CArtf + kPtcTu*(CPTCf - frac*CPT) - (VmK_Oatp*CPT/(KmK_Oatp+CPT)) - 
      (VmK_Urat*CPT/(KmK_Urat+CPT)) + (VmK_api*CPTCf/(KmK_api+CPTCf))- QTDL*CPT
    
    #Descending limb, Ascending limb (Loop of Henle )
    dMDAL =  QTDL*CPT + kDalcTu*(CDALCf - frac*CDAL)  -  QDT*CDAL
    
    # Distal convoluted tubule 
    dMDT =   QDT*CDAL + kDtcTu*(CDTCf - frac*CDT) -  QCD*CDT
    
    #Collecting duct
    dMCD = QCD*CDT + kCdcTu*(CCDCf - frac*CCD) - Qurine*CCD
...
obj.func <- function(x, dataset, fixed_params){
  ...
  sample_time=c(seq(0,20, 0.1),seq(21,288,1))
  ...
  score[1] <- AAFE(predictions = preds_kim_OR_Mtissues, observations = obs_kim_OR_Mtissues)
  ...
  score[2] <- AAFE(predictions = preds_kim_OR_Ftissues, observations = obs_kim_OR_Ftissues)
  ...
  # sample_time: a vector of time points to solve the ODEs
  sample_time= c(seq(0,20, 0.1),seq(21,864,1)) 
  ...
  score[3] <- AAFE(predictions = preds_dzi_OR_Mtissues, observations = obs_dzi_OR_Mtissues)
  ...
  score[4] <- AAFE(predictions = preds_dzi_OR_Ftissues, observations = obs_dzi_OR_Ftissues)
  ...
  score[5] <- AAFE(predictions = preds_kim_OR_Mblood, observations = obs_kim_OR_Mblood)
  ...
  sex <- "F"
  BW <- 0.2 #kg
  sample_time <- c(seq(0,20, 0.1),seq(21,192,1))  
  ...
  score[6] <- AAFE(predictions = preds_Kemp_OR_Furine_low, observations = obs_Kemp_OR_Furine_low)
  ...
  score[7] <- AAFE(predictions = preds_Kemp_OR_Ffeces_low, observations = obs_Kemp_OR_Ffeces_low)
  ...
  sex <- "M"
  BW <- 0.3 #kg
  sample_time <- c(seq(0,20, 0.1),seq(21,673,1)) 
  ...
  score[8] <- AAFE(predictions = preds_Kemp_OR_Murine_low, observations = obs_Kemp_OR_Murine_low)
  ...
  score[9] <- AAFE(predictions = preds_Kemp_OR_Mfeces_low, observations = obs_Kemp_OR_Mfeces_low)
  ...
  inits <- create.inits(params)
  events <- create.events(params)
  sample_time <- c(c(0, 0.1, 0.25), seq(0.3,20, 0.1), seq(24, 1200, 4))
  ...
  for (i in 1:length(unique(exp_data$Tissue))) {
    ...
    preds_dzi_OR_Mserum_medium [[i]] <- solution[round(solution$time, 3) %in% round(exp_time, 3), column_names[i]]/1000 
  }
  
  obs_dzi_OR_Mserum_medium <- list(exp_data[exp_data$Tissue == "Serum", "concentration"])
  
  score[10] <- AAFE(predictions = preds_dzi_OR_Mserum_medium, observations = obs_dzi_OR_Mserum_medium)
  ...
  inits <- create.inits(params)
  events <- create.events(params)
  sample_time <- c(c(0, 0.083, 0.25), seq(0.3,20, 0.1),seq(21,192,1))  
  ...
  for (i in 1:length(unique(exp_data$Tissue))) {
    ...
    preds_dzi_IV_Fserum [[i]] <- solution[round(solution$time, 3) %in% round(exp_time, 3), column_names[i]]/1000 
  }
  
  obs_dzi_IV_Fserum <- list(exp_data[exp_data$Tissue == "Serum", "concentration"])
  score[11] <- AAFE(predictions = preds_dzi_IV_Fserum, observations = obs_dzi_IV_Fserum)
  ...
  inits <- create.inits(params)
  events <- create.events(params)
  sample_time <- c(c(0,0.1,0.25),seq(0.3,20, 0.1),seq(21,96,1))  
  ...
  for (i in 1:length(unique(exp_data$Tissue))) {
    ...
    preds_dzi_OR_Fserum_low [[i]] <- solution[round(solution$time, 3) %in% round(exp_time, 3), column_names[i]]/1000 
  }
  
  obs_dzi_OR_Fserum_low <- list(exp_data[exp_data$Tissue == "Serum", "concentration"])
  score[12] <- AAFE(predictions = preds_dzi_OR_Fserum_low, observations = obs_dzi_OR_Fserum_low)
  ...
  for (i in 1:length(unique(exp_data$Tissue))) {
    ...
    preds_kim_OR_Fblood [[i]] <- solution[round(solution$time, 3) %in% round(exp_time, 3), column_names[i]]/1000
  }
  
  obs_kim_OR_Fblood <- list(exp_data[exp_data$Tissue == "Plasma", "concentration"])
  score[13] <- AAFE(predictions = preds_kim_OR_Fblood, observations = obs_kim_OR_Fblood)
  ...
  score[14] <- AAFE(predictions = preds_kim_IV_Fblood, observations = obs_kim_IV_Fblood)
  ...
  inits <- create.inits(params)
  events <- create.events(params)
  sample_time= c(seq(0,20, 0.1),seq(21,48,1)) 
  # ode(): The solver of the ODEs
  solution <- data.frame(deSolve::ode(times = sample_time,  func = ode.func,
                                      y = inits, parms = params,
                                      events = events,
  ...
  score[15] <- AAFE(predictions = preds_gus_OR_Mtissues, observations = obs_gus_OR_Mtissues)
...
setwd("/Users/user/Documents/GitHub/PFAS_PBK_models/PFOA inhalation Rat/Scenarios/diffusion/renal_time_simplified")
...
opts <- list( "algorithm" = "NLOPT_LN_SBPLX", #"NLOPT_LN_SBPLX", #"NLOPT_LN_NEWUOA"
              "xtol_rel" = 1e-06,
              "ftol_rel" = 1e-06,
              "ftol_abs" = 0.0,
              "xtol_abs" = 0.0, 
...
fit <- log(c( 0.00000001, 116.93795004,   0.65769979, 552.31584173, 464.19411857,   0.31622777,   0.17300421,
              0.80000000,   0.00273842,   0.05796737))
  #c(rep(log(1),6), rep(log(mean(c(Papp_Kimura,Papp_RYU))),1),log(0.8),log(1e-3), log(1e-1))

lb = c(rep(log(1e-8),2), log(1e-5), log(1e-3),rep(log(1e-8),2),  rep(log(Papp_RYU),1), log(1e-1), log(1e-5), log(1e-4))
ub = c(rep(log(1e8), 2),log(1e5),  log(1e3),rep(log(1e8),  2), rep(log(100*Papp_Kimura),1),log(1), log(1e2), log(1e1))
...
estimated_params <- exp(optimizer$solution)
save.image("renal_time_simplified.RData")
