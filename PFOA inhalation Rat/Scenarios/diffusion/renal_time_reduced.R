...
setwd("/Users/user/Documents/GitHub/PFAS_PBK_models/PFOA inhalation Rat/Scenarios/diffusion/renal_time_reduced")
...
fit <- c(rep(log(1),6), rep(log(mean(c(Papp_Kimura,Papp_RYU))),1),log(0.8),log(1e-3), log(1e-1))

lb = c(rep(log(1e-8),2), log(1e-5), log(1e-3),rep(log(1e-8),2),  rep(log(Papp_RYU),1), log(1e-1), log(1e-5), log(1e-2))
...
estimated_params <- exp(optimizer$solution)
save.image("renal_time_reduced.RData")
...
inits <- create.inits(params)
events <- create.events(params)
sample_time <-c( c(0, 0.083, 0.25), seq(0.5, 192, 0.5))
