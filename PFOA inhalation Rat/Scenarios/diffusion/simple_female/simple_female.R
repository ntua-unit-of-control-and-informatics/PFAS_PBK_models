...
create_variable_params <- function(BW,sex,  estimated_params, fixed_params){
  ...
  Ka <- estimated_params[10] # from Rue et al. (2024)#mol/L
...
obj.func <- function(x, dataset, fixed_params){
  ...
  score[1] <- AAFE(predictions = preds_dzi_OR_Ftissues, observations = obs_dzi_OR_Ftissues)
  ...
  score[2] <- AAFE(predictions = preds_Kemp_OR_Furine_low, observations = obs_Kemp_OR_Furine_low)
  ...
  score[3] <- AAFE(predictions = preds_Kemp_OR_Ffeces_low, observations = obs_Kemp_OR_Ffeces_low)
  ...
  score[4] <- AAFE(predictions = preds_dzi_IV_Fserum, observations = obs_dzi_IV_Fserum)
  ...
  score[5] <- AAFE(predictions = preds_dzi_OR_Fserum_low, observations = obs_dzi_OR_Fserum_low)
  ...
  score[6] <- AAFE(predictions = preds_kim_OR_Fblood, observations = obs_kim_OR_Fblood)
  ...
  score[7] <- AAFE(predictions = preds_kim_IV_Fblood, observations = obs_kim_IV_Fblood)
...
setwd("/Users/user/Documents/GitHub/PFAS_PBK_models/PFOA inhalation Rat/Scenarios/diffusion/simple_female")
...
fit <- c(rep(log(1),6), rep(log(mean(c(Papp_Kimura,Papp_RYU))),1),log(0.8),log(1e-3),log(5e5))

lb = c(rep(log(1e-8),2), log(1e-5), log(1e-3),rep(log(1e-8),2),  rep(log(Papp_RYU),1), log(1e-1), log(1e-5),log(5e4))
ub = c(rep(log(1e8), 2),log(1e5),  log(1e3),rep(log(1e8),  2), rep(log(100*Papp_Kimura),1),log(1), log(1e2),log(7e5))
...
estimated_params <- exp(optimizer$solution)
save.image("simple_female.RData")
