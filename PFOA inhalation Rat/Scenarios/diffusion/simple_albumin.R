...
create_variable_params <- function(BW,sex,  estimated_params, fixed_params){
  ...
  if(sex == "M"){
    RAFOatp_k <- 1
    frac_alb <-estimated_params[9]
  }else{
    RAFOatp_k <- estimated_params[1]
    frac_alb <-estimated_params[10]
  ...
  Ka <-frac_alb * 5.8e05 # from Rue et al. (2024)#mol/L
...
setwd("/Users/user/Documents/GitHub/PFAS_PBK_models/PFOA inhalation Rat/Scenarios/diffusion/simple_albumin")
...
fit <- log(c(4.669038e-08, 1.762773e+00, 1.752380e+00, 1.000000e+03, 2.404709e+02, 1.127547e+00, 8.601846e-02,
              7.314942e-03, 1.796914e+00,2.038734e-01))
             #c(rep(log(1),6), rep(log(mean(c(Papp_Kimura,Papp_RYU))),1),log(1e-3), rep(log(1),2))

lb = c(rep(log(1e-8),2), log(1e-5), log(1e-3),rep(log(1e-8),2),  rep(log(Papp_RYU),1), log(1e-5), rep(log(0.1),2))
ub = c(rep(log(1e8), 2),log(1e5),  log(1e3),rep(log(1e8),  2), rep(log(100*Papp_Kimura),1), log(1e2), rep(log(2),2))
...
estimated_params <- exp(optimizer$solution)
save.image("simple_albumin.RData")
