...
create.params <- function(user.input){
  with(as.list(user.input),{
    ...
    CalbLuAF_init <- VLu*(5.77e-2*4.34*1000)/66500#f_alb_avail*10/100 * CalbB_init #based on Woods et al. 2015 statement https://doi.org/10.1016/j.jconrel.2015.05.269
...
ode.func <- function(time, inits, params){
  with(as.list(c(inits, params)),{
    ...
    dMLuFf = QparaLu*(1-SLu)*CLuBf + PeffLu*ALu*(CLuBf-CLuFf) + kLuTLuF*(CLuT-CLuFf) + 
      koff_alb*CLuFb*VLuF - kon_alb*CalbLuFf*CLuFf*VLuF -((VmK_Oat1/500)*CLuFf/(KmK_Oat1+CLuFf))
    #Lung tissue
    dMLuTf =  - kLuTLuF*(CLuT-CLuFf) -  kLuTLuAF*CLuT/15+kLuTLuAF*CLuAFf+((VmK_Oat1/500)*CLuFf/(KmK_Oat1+CLuFf))
    #Alveolar lining fluid
    dMLuAFf =    kLuTLuAF*CLuT/15-kLuTLuAF*CLuAFf+ koff_alb*CLuAFb*VLuAF - kon_alb*CalbLuAFf*CLuAFf*VLuAF
...
setwd("C:/Users/user/Documents/GitHub/PFAS_PBK_models/PFOA inhalation Rat/Scenarios/NEW/Training/AAFE/koff_04")
...
fit <-  c(rep(log(1),7), log(1),log(1e5))

lb    = c(rep(log(1e-10), 7),log(1e-3), log(1e3))
ub = c(rep(log(1e10), 7),  log(1e3),log(1e6))
