...
create.params <- function(user.input){
  with(as.list(user.input),{
    ...
    Ka <-estimated_params[11] # 5.8e05 #mol/L
...
setwd("C:/Users/user/Documents/GitHub/PFAS_PBK_models/PFOA inhalation Rat/Scenarios/NEW/Training/AAFE/koff_09_lung_different_RAF")
...
fit <-  c(rep(log(1),9), log(1),log(1e5))

lb    = c(rep(log(1e-20), 9),log(1e-3), log(1e3))
ub = c(rep(log(1e6), 9),  log(1e3),log(1e6))
...
estimated_params <- exp(optimizer$solution)
save.image("koff_09_lung_different_RAF.RData")
