...
create_variable_params <- function(BW,sex,  estimated_params, fixed_params){
  ...
  if(sex == "M"){
    ...
    KLfabp <-  estimated_params[10]#(1.2e5+4e4+1.9e4) #[L/mol]*1e-3 , value from Cheng et al. (2017)
    
  }else{
    RAFOatp_k <-estimated_params[2]
    VmK_api <- 0#estimated_params[15]
    Ka <- estimated_params[9]# 7e4 # from Rue et al. (2024)#mol/L
    KLfabp <-  estimated_params[11]#(1.2e5+4e4+1.9e4) #[L/mol]*1e-3 , value from Cheng et al. (2017)
...
setwd("/Users/user/Documents/GitHub/PFAS_PBK_models/PFOA inhalation Rat/Scenarios/permeability/sex_different_ka")
...
fit <- log(c(1.984017e+01, 2.472699e-05, 6.382949e+03, 1.117469e+02, 7.570878e+00, 0.05,4.545368e-05, 1e5,1e5,1e5,1e5  ))

lb = c(rep(log(1e-7),3), log(1e-3), log(1e-1),  rep(log(Papp_RYU),1), log(1e-6), log(1e4),  log(1e4), log(1e4),  log(1e4))
ub = c(rep(log(1e5),  3),log(1e3),  log(2e1), rep(log(Papp_Kimura),1), log(1e-3),log(1e6), log(1e6),log(1e6), log(1e6) )
...
estimated_params <- exp(optimizer$solution)
save.image("sex_different_ka_SBPLX.RData")
