...
create_variable_params <- function(BW,sex,  estimated_params, fixed_params){
  ...
  CLfeces_unscaled <- estimated_params[12] #in L/h/BW^(-0.25), scaling similar to Loccisano et al. (2012)
  ...
  kDtcF <- 0#((2*Peff_monolayer/100) * fixed_params$AcK_DTC) *1000
  kPtcF <- 0#((2*Peff_monolayer/100) * fixed_params$AcK_PTC) *1000
...
create_fixed_params <- function(user.input){
  with(as.list(user.input),{
    ...
    d_peritubular <- 0.024 *((1e-6)^2)/((1e-6)^3) # [um^2/um^3]*((1e-6)^2)/((1e-6)^3) -->m^2/m^3,
    VK_gazzard <- 0.00363#L
    Vcortex <- 2*1295* (VK/VK_gazzard)*(1e-3)^3# [mm^3]*(1e-3)^3 -->m^3 
    A_peritubular <- (d_peritubular*Vcortex) #m^2
...
setwd("/Users/user/Documents/GitHub/PFAS_PBK_models/PFOA inhalation Rat/Scenarios/permeability/IF_no_apical")
...
fit <- fit <- log(c(2.923491e+01, 9.411747e-08, 3.213941e+03, 1.323091e+02, 5.895837e+00,
                    5.610785e-03, 1.312511e-01, 1.512347e+00,1.512,1.512, 2.191817e-01, 4.335417e-04))


lb = c(rep(log(1e-8),3), log(1e-5), log(1e-3),rep(log(1e-8),2),  rep(log(Papp_RYU),3), log(1e-3),log(1e-5))
ub = c(rep(log(1e8),  3),log(1e5),  log(1e3),rep(log(1e8),  2), rep(log(10*Papp_Kimura),3),log(10),  log(1e2) )
