...
create_variable_params <- function(BW,sex,  estimated_params, fixed_params){
  ...
  Pgap_continuous <- estimated_params[8] #, cm/s, permeability by diffusion through tight junctions in continuous capillary types
  ...
  CLfeces_unscaled <- estimated_params[9] #in L/h/BW^(-0.25), scaling similar to Loccisano et al. (2012)
  ...
  fu_if_K = 1/(1+IPR_K*(1-fu)/fu)
  fu_if_L =  1/(1+IPR_L*(1-fu)/fu)
  fu_if_ST =  1/(1+IPR_ST*(1-fu)/fu)
  fu_if_IN =  1/(1+IPR_IN*(1-fu)/fu)
  fu_if_M =  1/(1+IPR_M*(1-fu)/fu)
  fu_if_A =  1/(1+IPR_A*(1-fu)/fu)
  fu_if_Lu =  1/(1+IPR_Lu*(1-fu)/fu)
  fu_if_Sp =  1/(1+IPR_Sp*(1-fu)/fu)
  fu_if_H =  1/(1+IPR_H*(1-fu)/fu)
  fu_if_SK =  1/(1+IPR_SK*(1-fu)/fu)
  fu_if_Br =  1/(1+IPR_Br*(1-fu)/fu)
  fu_if_Go =  1/(1+IPR_Go*(1-fu)/fu)
  fu_if_Bo =  1/(1+IPR_Bo*(1-fu)/fu)
  fu_if_R =  1/(1+IPR_R*(1-fu)/fu)
  ...
  fu_lt <- 1/(1+KLfabp*CFabpLT_init)
  Ka <- (1-fu)/(fu*CalbB_init)# mol/L
  ...
  return(list(
    ...
    "fu_if_K" =fu_if_K,"fu_if_L" = fu_if_L, "fu_if_ST" =fu_if_ST,
    "fu_if_IN" = fu_if_IN,"fu_if_M" =fu_if_M,"fu_if_A" = fu_if_A,
    "fu_if_Lu" = fu_if_Lu,"fu_if_Sp" = fu_if_Sp,"fu_if_H" = fu_if_H,
    "fu_if_SK" = fu_if_SK,"fu_if_Br" = fu_if_Br,"fu_if_Go" = fu_if_Go,
    "fu_if_Bo" = fu_if_Bo, "fu_if_R" = fu_if_R
...
ode.func <- function(time, inits, params){
  ...
  with(as.list(c(inits, params)),{
    ...
    CKTrest <- MKTrest/VKTrest
    CPTC <- MPTC/VPTC
    CDALC <- MDALC/VDALC
    CDTC <- MDTC/VDTC
    CCDC <- MCDC/VCDC
    
    MKT <- MKTrest + MPTC + MDALC + MDTC + MCDC
    ...
    CAF <- MAF/VAF  #Interstitial fluid concentrationF
    ...
    #Arterial Blood
    dMArt = QBLu*CLuB*fu - CArt*fu*(QBK+QBL+QBM+QBA+QBR+QBSP+QBH+QBBr+
                                      QBST+QBIN+QBGo+QBSK+QBBo) - QGFR*CArt*fu 
    
    #Venous Blood
    dMVen = - CVen*fu*QBLu + QBK*CKB*fu + QBLtot*CLB*fu + QBM*CMB*fu + QBA*CAB*fu + QBR*CRB*fu+
      QBH*CHB*fu + QBBr*CBrB*fu+ QBGo*CGoB*fu + QBSK*CSKB*fu + QBBo*CBoB*fu
    
    
    #Kidney
    #blood subcompartment
    dMKB = QBK*CArt*fu - QBK*CKB*fu- (Ptrans_diff_K+PparaKi)*(A_peritubular_PTC+A_peritubular_DTC)*(CKB*fu-CKF*fu_if_K) + 
      (VmK_baso*CPTC/(KmK_baso+CPTC))
    
    #interstitial fluid subcompartment
    dMKF =  (Ptrans_diff_K+PparaKi)*(A_peritubular_PTC+A_peritubular_DTC)*(CKB*fu-CKF*fu_if_K)- 
      kPtcF*(CKF*fu_if_K-CPTC) - kDalcF*(CKF*fu_if_K-CDALC) -
      kDtcF*(CKF*fu_if_K-CDTC) - kCdcF*(CKF*fu_if_K-CCDC)   -  kKTrestF*(CKF*fu_if_K-CKTrest)-
      (VmK_Oat1*CKF*fu_if_K/(KmK_Oat1+CKF*fu_if_K)) - (VmK_Oat3*CKF*fu_if_K/(KmK_Oat3+CKF*fu_if_K))
    
    #proximal tubule  cells subcompartment
    dMPTC = kPtcF*(CKF*fu_if_K-CPTC) - kPtcTu*(CPTC - CPT)  +
      (VmK_Oatp*CPT/(KmK_Oatp+CPT)) + (VmK_Urat*CPT/(KmK_Urat+CPT))+
      (VmK_Oat1*CKF*fu_if_K/(KmK_Oat1+CKF*fu_if_K)) + (VmK_Oat3*CKF*fu_if_K/(KmK_Oat3+CKF*fu_if_K)) - 
      (VmK_baso*CPTC/(KmK_baso+CPTC)) -(VmK_api*CPTC/(KmK_api+CPTC))
    
    #Tubule cells in Loop of Henle 
    dMDALC =  kDalcF*(CKF*fu_if_K-CDALC) - kDalcTu*(CDALC - CDAL)
    
    #Distal convoluted tubule cells 
    dMDTC =   kDtcF*(CKF*fu_if_K-CDTC)- kDtcTu*(CDTC - CDT)
    
    #Collecting duct cells 
    dMCDC =  kCdcF*(CKF*fu_if_K-CCDC) - kCdcTu*(CCDC - CCD)
    
    #Rest of kidney tissue subcompartment
    dMKTrest =  kKTrestF*(CKF*fu_if_K-CKTrest)
    
    #Proximal convoluted tubule
    dMPT =  QGFR*CArt*fu + kPtcTu*(CPTC - CPT) - (VmK_Oatp*CPT/(KmK_Oatp+CPT)) - 
      (VmK_Urat*CPT/(KmK_Urat+CPT)) + (VmK_api*CPTC/(KmK_api+CPTC))- QTDL*CPT
    
    #Descending limb, Ascending limb (Loop of Henle )
    dMDAL =  QTDL*CPT + kDalcTu*(CDALC - CDAL)  -  QDT*CDAL
    
    # Distal convoluted tubule 
    dMDT =   QDT*CDAL + kDtcTu*(CDTC - CDT) -  QCD*CDT
    
    #Collecting duct
    dMCD = QCD*CDT + kCdcTu*(CCDC - CCD) - Qurine*CCD
    ...
    #Liver
    #blood subcompartment
    dMLB = QBL*CArt*fu + QBSP*CSPB*fu + QBIN*CINB*fu + QBST*CSTB*fu - 
      QBLtot*CLB*fu - (Ptrans_diff_L+PparaLi)*AL*(CLB*fu-CLF*fu_if_L)  
    #interstitial fluid subcompartment 
    dMLF = (Ptrans_diff_L+PparaLi)*AL*(CLB*fu-CLF*fu_if_L) - kLFLT*(CLF*fu_if_L-CLT*fu_lt) - 
      (VmL_Oatp*CLF*fu_if_L/(KmL_Oatp+CLF*fu_if_L)) - (VmL_Oatp2*CLF*fu_if_L/(KmL_Oatp2+CLF*fu_if_L)) -
      (VmL_Ntcp*CLF*fu_if_L/(KmL_Ntcp+CLF*fu_if_L)) 
    #Liver tissue subcompartment
    dMLT = kLFLT*(CLF*fu_if_L-CLT*fu_lt) + (VmL_Oatp*CLF*fu_if_L/(KmL_Oatp+CLF*fu_if_L)) + (VmL_Oatp2*CLF*fu_if_L/(KmL_Oatp2+CLF*fu_if_L))+
      (VmL_Ntcp*CLF*fu_if_L/(KmL_Ntcp+CLF*fu_if_L))- CL_hepatobiliary*CLT*fu_lt
    #Bile  canaliculi subcompartment
    dMLbile = CL_hepatobiliary*CLT*fu_lt - CLbile*Qbile
    
    
    
    #Stomach
    #blood subcompartment
    dMSTB = QBST*CArt*fu - QBST*CSTB*fu - (Ptrans_diff_ST+PparaSt)*AST*(CSTB*fu-CSTF*fu_if_ST) 
    #interstitial fluid subcompartment 
    dMSTF = (Ptrans_diff_ST+PparaSt)*AST*(CSTB*fu-CSTF*fu_if_ST) - kSTFSTT*(CSTF*fu_if_ST-CSTT)
    #Stomach tissue subcompartment
    dMSTT = kSTFSTT*(CSTF*fu_if_ST-CSTT) + kabST*CSTL 
    #Stomach lumen
    ...
    #Intestine
    #blood subcompartment
    dMINB = QBIN*CArt*fu - QBIN*CINB*fu - (Ptrans_diff_IN+PparaIn)*AIN*(CINB*fu-CINF*fu_if_IN) 
    #interstitial fluid subcompartment 
    dMINF = (Ptrans_diff_IN+PparaIn)*AIN*(CINB*fu-CINF*fu_if_IN) - kINFINT*(CINF*fu_if_IN-CINT) 
    #Intestine tissue subcompartment
    dMINT = kINFINT*(CINF*fu_if_IN-CINT) + k_gut_in*CINL - k_gut_out*CINT + (VmIn_Oatp2*CINL/(KmIn_Oatp2+CINL)) 
    #Intestine lumen
    dMINL = QGE*CSTL - (CLfeces*CINL) - k_gut_in*CINL + k_gut_out*CINT + CLbile*Qbile - 
      (VmIn_Oatp2*CINL/(KmIn_Oatp2+CINL))
    
    
    #Muscle
    #blood subcompartment
    dMMB = QBM*CArt*fu - QBM*CMB*fu - (Ptrans_diff_M+PparaMu)*AM*(CMB*fu-CMF*fu_if_M)  
    #interstitial fluid subcompartment 
    dMMF =(Ptrans_diff_M+PparaMu)*AM*(CMB*fu-CMF*fu_if_M) - kMFMT*(CMF*fu_if_M- CMT) 
    #Muscle tissue subcompartment 
    dMMT = kMFMT*(CMF*fu_if_M- CMT) 
    
    
    #Adipose
    #blood subcompartment
    dMAB = QBA*CArt*fu - QBA*CAB*fu - (Ptrans_diff_A+PparaAd)*AA*(CAB*fu-CAF*fu_if_A)  
    #interstitial fluid subcompartment 
    dMAF = (Ptrans_diff_A+PparaAd)*AA*(CAB*fu-CAF*fu_if_A)  - kAFAT*(CAF*fu_if_A-CAT) 
    #Adipose tissue subcompartment 
    dMAT =  kAFAT*(CAF*fu_if_A-CAT)
    
    
    #Rest of body
    #blood subcompartment
    dMRB = QBR*CArt*fu - QBR*CRB*fu - (Ptrans_diff_R+PparaRe)*AR*(CRB*fu-CRF*fu_if_R)  
    #interstitial fluid subcompartment 
    dMRF = (Ptrans_diff_R+PparaRe)*AR*(CRB*fu-CRF*fu_if_R) - kRFRT*(CRF*fu_if_R -CRT) 
    #Rest of body tissue subcompartment 
    dMRT = kRFRT*(CRF*fu_if_R -CRT) 
    
    
    #Lung 
    #blood subcompartment
    dMLuB = CVen*fu*QBLu - QBLu*CLuB*fu - (Ptrans_diff_Lu+PparaLu)*ALu*(CLuB*fu-CLuF*fu_if_Lu) 
    #interstitial fluid subcompartment
    dMLuF = (Ptrans_diff_Lu+PparaLu)*ALu*(CLuB*fu-CLuF*fu_if_Lu)+ kLuTLuF*(CLuT-CLuF*fu_if_Lu)  - (VmLu_Oatp_bas*CLuF*fu_if_Lu/(KmLu_Oatp_bas+CLuF*fu_if_Lu)) 
    #Lung tissue
    dMLuT =  - kLuTLuF*(CLuT-CLuF*fu_if_Lu) -  kLuTLuAF*(CLuT-CLuAF*fu_if_Lu) + (VmLu_Oatp_bas*CLuF*fu_if_Lu/(KmLu_Oatp_bas+CLuF*fu_if_Lu)) +
      (VmLu_Oatp_ap*CLuAF*fu_if_Lu/(KmLu_Oatp_ap+CLuAF*fu_if_Lu))
    #Alveolar lining fluid
    dMLuAF =  kLuTLuAF*(CLuT-CLuAF*fu_if_Lu) -(VmLu_Oatp_ap*CLuAF*fu_if_Lu/(KmLu_Oatp_ap+CLuAF*fu_if_Lu)) 
    
    
    #Spleen
    #blood subcompartment
    dMSPB = QBSP*CArt*fu - QBSP*CSPB*fu - (Ptrans_diff_SP+PparaSp)*ASP*(CSPB*fu-CSPF*fu_if_Sp) 
    #interstitial fluid subcompartment 
    dMSPF = (Ptrans_diff_SP+PparaSp)*ASP*(CSPB*fu-CSPF*fu_if_Sp) - kSPFSPT*(CSPF*fu_if_Sp -CSPT) 
    #Spleen tissue subcompartment 
    dMSPT = kSPFSPT*(CSPF*fu_if_Sp -CSPT)
    
    
    #Heart
    #blood subcompartment
    dMHB = QBH*CArt*fu - QBH*CHB*fu - (Ptrans_diff_H+PparaHt)*AH*(CHB*fu-CHF*fu_if_H)
    #interstitial fluid subcompartment 
    dMHF = (Ptrans_diff_H+PparaHt)*AH*(CHB*fu-CHF*fu_if_H) - kHFHT*(CHF*fu_if_H -CHT) 
    #Heart tissue subcompartment 
    dMHT = kHFHT*(CHF*fu_if_H -CHT) 
    
    
    #Brain
    #blood subcompartment
    dMBrB = QBBr*CArt*fu - QBBr*CBrB*fu - (Ptrans_diff_Br+PparaBr)*ABr*(CBrB*fu-CBrF*fu_if_Br)  
    #interstitial fluid subcompartment 
    dMBrF = (Ptrans_diff_Br+PparaBr)*ABr*(CBrB*fu-CBrF*fu_if_Br) - kBrFBrT*(CBrF*fu_if_Br -CBrT) 
    #Brain tissue subcompartment 
    dMBrT = kBrFBrT*(CBrF*fu_if_Br -CBrT) 
    
    
    #Gonads
    #blood subcompartment
    dMGoB = QBGo*CArt*fu - QBGo*CGoB*fu - (Ptrans_diff_Go+PparaGo)*AGo*(CGoB*fu-CGoF*fu_if_Go) 
    #interstitial fluid subcompartment 
    dMGoF = (Ptrans_diff_Go+PparaGo)*AGo*(CGoB*fu-CGoF*fu_if_Go) - kGoFGoT*(CGoF*fu_if_Go -CGoT) 
    #gonads tissue subcompartment 
    dMGoT = kGoFGoT*(CGoF*fu_if_Go -CGoT) 
    
    
    #Skin
    #blood subcompartment
    dMSKB = QBSK*CArt*fu - QBSK*CSKB*fu - (Ptrans_diff_SK+PparaSk)*ASK*(CSKB*fu-CSKF*fu_if_SK) 
    #interstitial fluid subcompartment
    dMSKF =(Ptrans_diff_SK+PparaSk)*ASK*(CSKB*fu-CSKF*fu_if_SK) - kSKFSKT*(CSKF*fu_if_SK -CSKT)
    #Skin tissue subcompartment
    dMSKT = kSKFSKT*(CSKF*fu_if_SK -CSKT) 
    
    
    #Bones
    #blood subcompartment
    dMBoB = QBBo*CArt*fu - QBBo*CBoB*fu - (Ptrans_diff_Bo+PparaBo)*ABo*(CBoB*fu-CBoF*fu_if_Bo) 
    #interstitial fluid subcompartment
    dMBoF = (Ptrans_diff_Bo+PparaBo)*ABo*(CBoB*fu-CBoF*fu_if_Bo) - kBoFBoT*(CBoF*fu_if_Bo -CBoT)
    #Bones tissue subcompartment
    dMBoT =  kBoFBoT*(CBoF*fu_if_Bo -CBoT) 
    ...
    list(c('dMArt'=dMArt, 'dMVen'=dMVen, 'dMKB'=dMKB, 
           'dMKF'=dMKF,  'dMPTC' = dMPTC, 'dMDALC' = dMDALC,  
           'dMDTC' = dMDTC, 'dMCDC' = dMCDC,
           'dMKTrest' = dMKTrest, 'dMPT' = dMPT,  'dMDAL' = dMDAL,
           'dMDT' =  dMDT, 'dMCD' = dMCD,
           
           'dMBladder' = dMBladder, 'dMLB'=dMLB, 
           'dMLF'=dMLF, 'dMLT'=dMLT, 'dMLbile'=dMLbile,
           
           'dMSTB'=dMSTB, 'dMSTF'=dMSTF, 'dMSTT'=dMSTT, 'dMSTL'=dMSTL,
           'dMINB'=dMINB, 'dMINF'=dMINF, 'dMINT'=dMINT,'dMINL'=dMINL,
           
           'dMMB'=dMMB, 'dMMF'=dMMF, 'dMMT'=dMMT,
           'dMAB'=dMAB, 'dMAF'=dMAF, 'dMAT'=dMAT, 
           'dMRB'=dMRB, 'dMRF'=dMRF,'dMRT'=dMRT,
           'dMLuB'=dMLuB, 'dMLuF'=dMLuF,'dMLuT'=dMLuT,'dMLuAF' = dMLuAF,
           
           'dMSPB'=dMSPB, 'dMSPF'=dMSPF, 'dMSPT'=dMSPT,
           'dMHB'=dMHB, 'dMHF'=dMHF, 'dMHT'=dMHT,
           'dMBrB'=dMBrB, 'dMBrF'=dMBrF, 'dMBrT'=dMBrT,
           'dMGoB'=dMGoB, 'dMGoF'=dMGoF, 'dMGoT'=dMGoT,
           'dMSKB'=dMSKB, 'dMSKF'=dMSKF, 'dMSKT'=dMSKT,
           'dMBoB'=dMBoB, 'dMBoF'=dMBoF, 'dMBoT'=dMBoT,
...
create.inits <- function(parameters){
  with(as.list(parameters),{
    ...
    return(c(
      'MArt'=MArt, 'MVen'=MVen, 'MKB'=MKB, 
             'MKF'=MKF,  'MPTC' = MPTC, 'MDALC' = MDALC,  
             'MDTC' = MDTC, 'MCDC' = MCDC,
             'MKTrest' = MKTrest, 'MPT' = MPT,  'MDAL' = MDAL,
             'MDT' =  MDT, 'MCD' = MCD,
             
             'MBladder' = MBladder, 'MLB'=MLB, 
             'MLF'=MLF, 'MLT'=MLT, 'MLbile'=MLbile,
             
             'MSTB'=MSTB, 'MSTF'=MSTF, 'MSTT'=MSTT, 'MSTL'=MSTL,
             'MINB'=MINB, 'MINF'=MINF, 'MINT'=MINT,'MINL'=MINL,
             
             'MMB'=MMB, 'MMF'=MMF, 'MMT'=MMT,
             'MAB'=MAB, 'MAF'=MAF, 'MAT'=MAT, 
             'MRB'=MRB, 'MRF'=MRF,'MRT'=MRT,
             'MLuB'=MLuB, 'MLuF'=MLuF,'MLuT'=MLuT,'MLuAF' = MLuAF,
             
             'MSPB'=MSPB, 'MSPF'=MSPF, 'MSPT'=MSPT,
             'MHB'=MHB, 'MHF'=MHF, 'MHT'=MHT,
             'MBrB'=MBrB, 'MBrF'=MBrF, 'MBrT'=MBrT,
             'MGoB'=MGoB, 'MGoF'=MGoF, 'MGoT'=MGoT,
             'MSKB'=MSKB, 'MSKF'=MSKF, 'MSKT'=MSKT,
             'MBoB'=MBoB, 'MBoF'=MBoF, 'MBoT'=MBoT,
...
create.events <- function(parameters){
  with(as.list(parameters), {
    ...
    if (ltimes != ldose){
      ...
      if (admin.type == "iv"){
        events <- list(data = rbind(data.frame(var = c("MVen"),  time = admin.time, 
...
setwd("/Users/user/Documents/GitHub/PFAS_PBK_models/PFOA inhalation Rat/Scenarios/permeability/fu_estimated")
...
fit <- log(c(4.096924e+02, 9.379320e-05, 3.063877e+03, 6.700283e+02, 5.422986e-01, 2.317223e-02,
             7.245417e-08, 2.230996e-02, 4.182165e-05, 5.017135e-03))

lb = c(rep(log(1e-7),3), log(1e-3), log(1e-1),rep(log(1e-8),2),  rep(log(Papp_RYU),1),log(1e-6), log(1e-4))
ub = c(rep(log(1e5),  3),log(1e3),  log(2e1),rep(log(1e1),  2), rep(log(Papp_Kimura),1), log(1e-3), log(1e-2) )
