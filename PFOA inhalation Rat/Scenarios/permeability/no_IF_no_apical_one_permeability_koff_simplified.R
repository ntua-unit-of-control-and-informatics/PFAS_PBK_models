...
create_params <- function(user_input){
...
setwd("/Users/user/Documents/GitHub/PFAS_PBK_models/PFOA inhalation Rat/Scenarios/permeability/no_IF_no_apical_one_permeability_koff")
...
fit <- log(c(1.984017e+01, 2.472699e-05, 6.382949e+03, 1.117469e+02, 7.570878e+00, 7.413928e-03,
             5.071332e-07, 1.080756e+00, 2.992272e-01, 4.545368e-05,9.984452e-01))

lb = c(rep(log(1e-7),3), log(1e-3), log(1e-1),rep(log(1e-8),2),  rep(log(Papp_RYU),1), log(1e-2), log(1e-6), log(0.9))
ub = c(rep(log(1e5),  3),log(1e3),  log(2e1),rep(log(1e1),  2), rep(log(100*Papp_Kimura),1),log(1), log(1e-3), log(3) )
...
estimated_params <- exp(optimizer$solution)
save.image("no_IF_no_apical_one_permeability_koff.RData")
