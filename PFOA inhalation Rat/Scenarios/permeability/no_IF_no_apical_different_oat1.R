...
create_variable_params <- function(BW,sex,  estimated_params, fixed_params){
  ...
  Pgap_continuous <- estimated_params[9] #, cm/s, permeability by diffusion through tight junctions in continuous capillary types
  Pgap_fenestrated <- estimated_params[10] # cm/s,permeability by diffusion through fenestra in fenestrated capillary types
  Pgap_sinusoidal <- estimated_params[11] # cm/s,permeability by diffusion through discontinuities in sinusoidal capillary types
...
setwd("/Users/user/Documents/GitHub/PFAS_PBK_models/PFOA inhalation Rat/Scenarios/permeability/no_IF_no_apical_different_oat1")
...
fit <- c(rep(log(1),8), rep(log(mean(c(Papp_Kimura,Papp_RYU))),3), log(1), log(1e-3))

lb = c(rep(log(1e-8),4), log(1e-5), log(1e-3),rep(log(1e-8),2),  rep(log(Papp_RYU),3), log(1e-3), log(1e-5))
ub = c(rep(log(1e8),  4),log(1e5),  log(1e3),rep(log(1e8),  2), rep(log(10*Papp_Kimura),3),log(10), log(1e2) )
...
estimated_params <- exp(optimizer$solution)
save.image("no_IF_no_apical_different_oat1.RData")
