...
create_variable_params <- function(BW,sex,  estimated_params, fixed_params){
  ...
  Peff_monolayer <- Papp_RYU
  
  Pgap_continuous <- 0 #, cm/s, permeability by diffusion through tight junctions in continuous capillary types
  ...
  koff_fabp <-   1# estimated_params[15]#
  koff_a2u <- koff_alb
  ...
  Ka <- estimated_params[8]# 7e4 # from Rue et al. (2024)#mol/L
...
setwd("/Users/user/Documents/GitHub/PFAS_PBK_models/PFOA inhalation Rat/Scenarios/permeability/one_permeability")
...
fit <- log(c(3.416684e+01, 1.681949e-06, 8.640175e+03, 9.217010e+01, 2.815961e+00, 2.799992e+00,
             5.261153e+06,
             1.507899e+04, 9.551628e-05, 3.765961e+00, 1.825424e-01, 4.371305e-0))

lb = c(rep(log(1e-7),3), log(1e-3), log(1e-1), log(1e-4),  log(1e4),  log(1e4), log(1e-6),log(1e-2),log(5e-3),log(5e-3))
ub = c(rep(log(1e5),  3),log(1e3),  log(2e1), log(1e2), log(1e7), log(1e6) ,  log(1e-3),log(5),log(5),log(5))
...
estimated_params <- exp(optimizer$solution)
save.image("one_permeability.RData")
