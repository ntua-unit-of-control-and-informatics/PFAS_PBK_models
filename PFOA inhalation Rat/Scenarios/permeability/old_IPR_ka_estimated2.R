...
create_variable_params <- function(BW,sex,  estimated_params, fixed_params){
  ...
  Ka <- 1.267837e+05 # from Rue et al. (2024)#mol/L
...
obj.func <- function(x, dataset, fixed_params){
  ...
  score[36] <- AAFE(predictions = preds_Kemp_OR_Furine_low[length(preds_Kemp_OR_Furine_low)], 
                    observations = obs_Kemp_OR_Furine_low[length(obs_Kemp_OR_Furine_low)])
  #======================================df11b=========================================================
...
setwd("/Users/user/Documents/GitHub/PFAS_PBK_models/PFOA inhalation Rat/Scenarios/permeability/old_IPR_ka_estimated")
...
fit <- log(c(1.984017e+01, 2.472699e-05, 6.382949e+03, 1.117469e+02, 7.570878e+00, 0.05, 2.992272e-01,0.04,  4.545368e-05))

lb = c(rep(log(1e-7),3), log(1e-3), log(1e-1),  rep(log(Papp_RYU),1), log(1e-2),  log(1e-2), log(1e-6))
ub = c(rep(log(1e5),  3),log(1e3),  log(2e1), rep(log(Papp_Kimura),1),log(1), log(1) , log(1e-3))
...
estimated_params <- exp(optimizer$solution)
save.image("old_IPR_ka_estimated2_NEWOA.RData")
