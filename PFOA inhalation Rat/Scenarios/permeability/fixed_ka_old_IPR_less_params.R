...
create_variable_params <- function(BW,sex,  estimated_params, fixed_params){
  ...
  CL_int <- 1.346673e+02 #uL/min/million hepatocytes
  RAFOatp_l <- 1.36
  RAFUrat <- RAFOatp_k
  RAFOat1 <- RAFOat3
  RAFOatp2_l <- RAFOatp_l
  RAFOatp_lu_ap <- 1.299141e-02
  RAFOatp_lu_bas <- RAFOatp_lu_ap
  RAFNtcp <- RAFOatp_l
  RAFOatp2_Int <- 1.525573e-06
  ...
  Pgap_continuous <- 2.228633e-01 #, cm/s, permeability by diffusion through tight junctions in continuous capillary types
  ...
  f_alb_avail <- estimated_params[5] 
  ...
  CLfeces_unscaled <- 6.009681e-05 #in L/h/BW^(-0.25), scaling similar to Loccisano et al. (2012)
...
obj.func <- function(x, dataset, fixed_params){
  ...
  score[30] <- 10*score[30]
  score[31] <- 10*score[31]
  # score[16] <- 2*score[16]
  # score[17] <- 2*score[17]
  # score[18] <- 2*score[18]
  # score[19] <- 2*score[19]
...
setwd("/Users/user/Documents/GitHub/PFAS_PBK_models/PFOA inhalation Rat/Scenarios/permeability/fixed_ka_old_IPR_less_params")
...
fit <- log(c( 1.451704e+04, 1.000000e-05, 1.383550e+03,  0.01, 0.05, 0.1, 0.1))

lb = c(rep(log(1e-7),3),log(1e-3), log(1e-3), log(1e-4), log(1e-4))
ub = c(rep(log(1e5),  3),log(0.5),  log(0.5), log(100), log(100) )
...
estimated_params <- exp(optimizer$solution)
save.image("fixed_ka_old_IPR_less_params.RData")
