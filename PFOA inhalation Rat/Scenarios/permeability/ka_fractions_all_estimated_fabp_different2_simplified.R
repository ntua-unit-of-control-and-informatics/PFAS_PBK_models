...
create_params <- function(user_input){
  ...
  if(sex == "M"){
    ...
    f_fabp_avail  <-estimated_params[13]
  ...
  Pgap_continuous <-  estimated_params[7] #, cm/s, permeability by diffusion through tight junctions in continuous capillary types
  ...
  koff_alb <-  estimated_params[14]
  koff_fabp <-    estimated_params[15]#
  koff_a2u <- koff_alb
  
  VmK_baso <- 0
  KmK_baso <- 1e20
  KmK_api <-   5e4
  KLfabp <-  estimated_params[8]#(1.2e5+4e4+1.9e4) #[L/mol]*1e-3 , value from Cheng et al. (2017)
  Ka <- estimated_params[9]# 7e4 # from Rue et al. (2024)#mol/L
