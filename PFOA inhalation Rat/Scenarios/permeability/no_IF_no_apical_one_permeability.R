...
setwd("/Users/user/Documents/GitHub/PFAS_PBK_models/PFOA inhalation Rat/Scenarios/permeability/no_IF_no_apical_one_permeability")
...
fit <- log(c(2.851400e+01, 2.181054e-07, 3.419019e+03, 1.398225e+02, 4.883055e+00, 5.669825e-03,
             7.663470e-07, 1.276937e+00, 2.268887e-01, 4.300067e-05))

lb = c(rep(log(1e-8),3), log(1e-5), log(1e-3),rep(log(1e-8),2),  rep(log(Papp_RYU),1), log(1e-3), log(1e-5))
ub = c(rep(log(1e8),  3),log(1e5),  log(1e3),rep(log(1e8),  2), rep(log(100*Papp_Kimura),1),log(10), log(1e2) )
...
estimated_params <- exp(optimizer$solution)
save.image("no_IF_no_apical_one_permeability_run2.RData")
