...
create_variable_params <- function(BW,sex,  estimated_params, fixed_params){
  ...
  Ka <-5e4 # from Rue et al. (2024)#mol/L
  ...
  #values from Choi et al. 2024 [doi:10.1016/j.tox.2024.153806]
  IPR_K = 0.0698
  IPR_L = 0.0326
  IPR_ST = 0.0698 #assumed
  IPR_IN = 0.1163
  IPR_M = 0.0698 #assumed
  IPR_A = 0.0698 #assumed
  IPR_Lu = 0.0698 #assumed
  IPR_Sp = 0.0698 #assumed
  IPR_H = 0.0698 #assumed
  IPR_SK = 0.0698 #assumed
  IPR_Br = 0.0698 #assumed
  IPR_Go = 0.0698 #assumed
  IPR_Bo =0.0698 #assumed
  IPR_R = 0.0698 #assumed
...
ode.func <- function(time, inits, params){
  ...
  with(as.list(c(inits, params)),{
    ...
    dMKB = QBK*CArt*fu - QBK*CKB*fu- (Ptrans_diff_K+PparaKi)*(A_peritubular_PTC+A_peritubular_DTC)*(CKB*fu-CKF*fut) + 
      (VmK_baso*CPTC/(KmK_baso+CPTC))
    
    #interstitial fluid subcompartment
    dMKF =  (Ptrans_diff_K+PparaKi)*(A_peritubular_PTC+A_peritubular_DTC)*(CKB*fu-CKF*fut)- 
      kPtcF*(CKF*fut-CPTC) - kDalcF*(CKF*fut-CDALC) -
      kDtcF*(CKF*fut-CDTC) - kCdcF*(CKF*fut-CCDC)   -  kKTrestF*(CKF*fut-CKTrest)-
      (VmK_Oat1*CKF*fut/(KmK_Oat1+CKF*fut)) - (VmK_Oat3*CKF*fut/(KmK_Oat3+CKF*fut))
    
    #proximal tubule  cells subcompartment
    dMPTC = kPtcF*(CKF*fut-CPTC) - kPtcTu*(CPTC - CPT)  +
    ...
    dMDALC =  kDalcF*(CKF*fut-CDALC) - kDalcTu*(CDALC - CDAL)
    
    #Distal convoluted tubule cells 
    dMDTC =   kDtcF*(CKF*fut-CDTC)- kDtcTu*(CDTC - CDT)
    
    #Collecting duct cells 
    dMCDC =  kCdcF*(CKF*fut-CCDC) - kCdcTu*(CCDC - CCD)
    
    #Rest of kidney tissue subcompartment
    dMKTrest =  kKTrestF*(CKF*fut-CKTrest)
    ...
    dMLB = QBL*CArt*fu + QBSP*CSPB*fu + QBIN*CINB*fu + QBST*CSTB*fu - 
      QBLtot*CLB*fu - (Ptrans_diff_L+PparaLi)*AL*(CLB*fu-CLF*fut)  
    #interstitial fluid subcompartment 
    dMLF = (Ptrans_diff_L+PparaLi)*AL*(CLB*fu-CLF*fut) - kLFLT*(CLF*fut-CLT) - 
      (VmL_Oatp*CLF*fut/(KmL_Oatp+CLF*fut)) - (VmL_Oatp2*CLF*fut/(KmL_Oatp2+CLF*fut)) -
      (VmL_Ntcp*CLF*fut/(KmL_Ntcp+CLF*fut)) 
    #Liver tissue subcompartment
    dMLT = kLFLT*(CLF*fut-CLT) + (VmL_Oatp*CLF*fut/(KmL_Oatp+CLF*fut)) + (VmL_Oatp2*CLF*fut/(KmL_Oatp2+CLF*fut))+
      (VmL_Ntcp*CLF*fut/(KmL_Ntcp+CLF*fut))- CL_hepatobiliary*CLT
    #Bile  canaliculi subcompartment
    dMLbile = CL_hepatobiliary*CLT - CLbile*Qbile
    
    
    
    #Stomach
    #blood subcompartment
    dMSTB = QBST*CArt*fu - QBST*CSTB*fu - (Ptrans_diff_ST+PparaSt)*AST*(CSTB*fu-CSTF*fut) 
    #interstitial fluid subcompartment 
    dMSTF = (Ptrans_diff_ST+PparaSt)*AST*(CSTB*fu-CSTF*fut) - kSTFSTT*(CSTF*fut-CSTT)
    #Stomach tissue subcompartment
    dMSTT = kSTFSTT*(CSTF*fut-CSTT) + kabST*CSTL 
    #Stomach lumen
    ...
    #Intestine
    #blood subcompartment
    dMINB = QBIN*CArt*fu - QBIN*CINB*fu - (Ptrans_diff_IN+PparaIn)*AIN*(CINB*fu-CINF*fut) 
    #interstitial fluid subcompartment 
    dMINF = (Ptrans_diff_IN+PparaIn)*AIN*(CINB*fu-CINF*fut) - kINFINT*(CINF*fut-CINT) 
    #Intestine tissue subcompartment
    dMINT = kINFINT*(CINF*fut-CINT) + k_gut_in*CINL - k_gut_out*CINT + (VmIn_Oatp2*CINL/(KmIn_Oatp2+CINL)) 
    #Intestine lumen
    ...
    #Muscle
    #blood subcompartment
    dMMB = QBM*CArt*fu - QBM*CMB*fu - (Ptrans_diff_M+PparaMu)*AM*(CMB*fu-CMF*fut)  
    #interstitial fluid subcompartment 
    dMMF =(Ptrans_diff_M+PparaMu)*AM*(CMB*fu-CMF*fut) - kMFMT*(CMF*fut- CMT) 
    #Muscle tissue subcompartment 
    dMMT = kMFMT*(CMF*fut- CMT) 
    
    
    #Adipose
    #blood subcompartment
    dMAB = QBA*CArt*fu - QBA*CAB*fu - (Ptrans_diff_A+PparaAd)*AA*(CAB*fu-CAF*fut)  
    #interstitial fluid subcompartment 
    dMAF = (Ptrans_diff_A+PparaAd)*AA*(CAB*fu-CAF*fut)  - kAFAT*(CAF*fut-CAT) 
    #Adipose tissue subcompartment 
    dMAT =  kAFAT*(CAF*fut-CAT)
    
    
    #Rest of body
    #blood subcompartment
    dMRB = QBR*CArt*fu - QBR*CRB*fu - (Ptrans_diff_R+PparaRe)*AR*(CRB*fu-CRF*fut)  
    #interstitial fluid subcompartment 
    dMRF = (Ptrans_diff_R+PparaRe)*AR*(CRB*fu-CRF*fut) - kRFRT*(CRF*fut -CRT) 
    #Rest of body tissue subcompartment 
    dMRT = kRFRT*(CRF*fut -CRT) 
    
    
    #Lung 
    #blood subcompartment
    dMLuB = CVen*fu*QBLu - QBLu*CLuB*fu - (Ptrans_diff_Lu+PparaLu)*ALu*(CLuB*fu-CLuF*fut) 
    #interstitial fluid subcompartment
    dMLuF = (Ptrans_diff_Lu+PparaLu)*ALu*(CLuB*fu-CLuF*fut)+ kLuTLuF*(CLuT-CLuF*fut)  - (VmLu_Oatp_bas*CLuF*fut/(KmLu_Oatp_bas+CLuF*fut)) 
    #Lung tissue
    dMLuT =  - kLuTLuF*(CLuT-CLuF*fut) -  kLuTLuAF*(CLuT-CLuAF*fut) + (VmLu_Oatp_bas*CLuF*fut/(KmLu_Oatp_bas+CLuF*fut)) +
      (VmLu_Oatp_ap*CLuAF*fut/(KmLu_Oatp_ap+CLuAF*fut))
    #Alveolar lining fluid
    dMLuAF =  kLuTLuAF*(CLuT-CLuAF*fut) -(VmLu_Oatp_ap*CLuAF*fut/(KmLu_Oatp_ap+CLuAF*fut)) 
    
    
    #Spleen
    #blood subcompartment
    dMSPB = QBSP*CArt*fu - QBSP*CSPB*fu - (Ptrans_diff_SP+PparaSp)*ASP*(CSPB*fu-CSPF*fut) 
    #interstitial fluid subcompartment 
    dMSPF = (Ptrans_diff_SP+PparaSp)*ASP*(CSPB*fu-CSPF*fut) - kSPFSPT*(CSPF*fut -CSPT) 
    #Spleen tissue subcompartment 
    dMSPT = kSPFSPT*(CSPF*fut -CSPT)
    
    
    #Heart
    #blood subcompartment
    dMHB = QBH*CArt*fu - QBH*CHB*fu - (Ptrans_diff_H+PparaHt)*AH*(CHB*fu-CHF*fut)
    #interstitial fluid subcompartment 
    dMHF = (Ptrans_diff_H+PparaHt)*AH*(CHB*fu-CHF*fut) - kHFHT*(CHF*fut -CHT) 
    #Heart tissue subcompartment 
    dMHT = kHFHT*(CHF*fut -CHT) 
    
    
    #Brain
    #blood subcompartment
    dMBrB = QBBr*CArt*fu - QBBr*CBrB*fu - (Ptrans_diff_Br+PparaBr)*ABr*(CBrB*fu-CBrF*fut)  
    #interstitial fluid subcompartment 
    dMBrF = (Ptrans_diff_Br+PparaBr)*ABr*(CBrB*fu-CBrF*fut) - kBrFBrT*(CBrF*fut -CBrT) 
    #Brain tissue subcompartment 
    dMBrT = kBrFBrT*(CBrF*fut -CBrT) 
    
    
    #Gonads
    #blood subcompartment
    dMGoB = QBGo*CArt*fu - QBGo*CGoB*fu - (Ptrans_diff_Go+PparaGo)*AGo*(CGoB*fu-CGoF*fut) 
    #interstitial fluid subcompartment 
    dMGoF = (Ptrans_diff_Go+PparaGo)*AGo*(CGoB*fu-CGoF*fut) - kGoFGoT*(CGoF*fut -CGoT) 
    #gonads tissue subcompartment 
    dMGoT = kGoFGoT*(CGoF*fut -CGoT) 
    
    
    #Skin
    #blood subcompartment
    dMSKB = QBSK*CArt*fu - QBSK*CSKB*fu - (Ptrans_diff_SK+PparaSk)*ASK*(CSKB*fu-CSKF*fut) 
    #interstitial fluid subcompartment
    dMSKF =(Ptrans_diff_SK+PparaSk)*ASK*(CSKB*fu-CSKF*fut) - kSKFSKT*(CSKF*fut -CSKT)
    #Skin tissue subcompartment
    dMSKT = kSKFSKT*(CSKF*fut -CSKT) 
    
    
    #Bones
    #blood subcompartment
    dMBoB = QBBo*CArt*fu - QBBo*CBoB*fu - (Ptrans_diff_Bo+PparaBo)*ABo*(CBoB*fu-CBoF*fut) 
    #interstitial fluid subcompartment
    dMBoF = (Ptrans_diff_Bo+PparaBo)*ABo*(CBoB*fu-CBoF*fut) - kBoFBoT*(CBoF*fut -CBoT)
    #Bones tissue subcompartment
    dMBoT =  kBoFBoT*(CBoF*fut -CBoT) 
...
setwd("/Users/user/Documents/GitHub/PFAS_PBK_models/PFOA inhalation Rat/Scenarios/permeability/fu_fluid_different")
...
fit <- log(c(1.984017e+01, 2.472699e-05, 6.382949e+03, 1.117469e+02, 7.570878e+00, 7.413928e-03,
             5.071332e-07, 1.080756e+00,  4.545368e-05))

lb = c(rep(log(1e-7),3), log(1e-3), log(1e-1),rep(log(1e-8),2),  rep(log(Papp_RYU),1),log(1e-6))
ub = c(rep(log(1e5),  3),log(1e3),  log(2e1),rep(log(1e1),  2), rep(log(100*Papp_Kimura),1), log(1e-3) )
...
estimated_params <- exp(optimizer$solution)
save.image("fu_fluid_different.RData")
