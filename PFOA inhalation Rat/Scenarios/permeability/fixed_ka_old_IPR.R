...
create_variable_params <- function(BW,sex,  estimated_params, fixed_params){
  ...
  Ka <-5e5 # from Rue et al. (2024)#mol/L
...
obj.func <- function(x, dataset, fixed_params){
  ...
  score[36] <- AAFE(predictions = list(preds_kim_IV_Mblood[[1]][length(preds_kim_IV_Mblood[[1]])]), 
                    observations = list(obs_kim_IV_Mblood[[1]][length(obs_kim_IV_Mblood[[1]])]))
  ##########################
  #-------------------------
  # Kemper 2003 (Worley)
  #-------------------------
  ...
  score[20] <- 10*score[20]
  score[22] <- 10*score[22]
  score[35] <- 5*score[35]
  score[36] <- 5*score[36]
  score[37] <- 5*score[37]
  score[38] <- 5*score[38]
...
setwd("/Users/user/Documents/GitHub/PFAS_PBK_models/PFOA inhalation Rat/Scenarios/permeability/fixed_ka_old_IPR")
...
fit <- log(c(1.984017e+01, 2.472699e-02, 6.382949e+01, 1.117469e+02, 7.570878e+00,0.05, 2.992272e-01, 0.04,1e-5))

lb = c(rep(log(1e-5),3), log(1e-3), log(1e-1), rep(log(Papp_RYU),1), log(1e-2), log(1e-2), log(1e-6))
ub = c(rep(log(1e5),  3),log(1e3),  log(1e2),rep(log(Papp_Kimura),1),log(1),log(1), log(1e-3) )
...
estimated_params <- exp(optimizer$solution)
save.image("fixed_ka_old_IPR.RData")
