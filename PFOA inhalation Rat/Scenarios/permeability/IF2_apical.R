...
create_variable_params <- function(BW,sex,  estimated_params, fixed_params){
  ...
  Pgap_fenestrated <- estimated_params[9] # cm/s,permeability by diffusion through fenestra in fenestrated capillary types
  Pgap_sinusoidal <- estimated_params[10] # cm/s,permeability by diffusion through discontinuities in sinusoidal capillary types
  ...
  CLfeces_unscaled <- estimated_params[13] #in L/h/BW^(-0.25), scaling similar to Loccisano et al. (2012)
...
ode.func <- function(time, inits, params){
  ...
  with(as.list(c(inits, params)),{
    ...
    dMKBf = QBK*CArtf - QBK*CKBf- (Ptrans_diff_K+PparaKi)*A_peritubular_PTC*(CKBf-CPTCf)-
      (Ptrans_diff_K+PparaKi)*A_peritubular_DTC*(CKBf-CDTCf) - 
    ...
    dMPTCf = (Ptrans_diff_K+PparaKi)*A_peritubular_PTC*(CKBf-CPTCf)+kPtcF*(CKFf-CPTCf) - kPtcTu*(CPTCf - CPT)  +
    ...
    dMDTCf =   (Ptrans_diff_K+PparaKi)*A_peritubular_DTC*(CKBf-CDTCf)+kDtcF*(CKFf-CDTCf)- kDtcTu*(CDTCf - CDT)-
...
obj.func <- function(x, dataset, fixed_params){
  ...
  score[1] <- AAFE(predictions = preds_kim_IV_Ftissues, observations = obs_kim_IV_Ftissues)
  ...
  score[3] <- AAFE(predictions = preds_dzi_OR_Ftissues, observations = obs_dzi_OR_Ftissues)
  ...
  score[4] <- AAFE(predictions = preds_Kemp_OR_Furine_low, observations = obs_Kemp_OR_Furine_low)
  ...
  score[5] <- AAFE(predictions = preds_Kemp_OR_Ffeces_low, observations = obs_Kemp_OR_Ffeces_low)
  ...
  score[6] <- AAFE(predictions = preds_Kemp_OR_Furine_med, observations = obs_Kemp_OR_Furine_med)
  ...
  score[7] <- AAFE(predictions = preds_Kemp_OR_Ffeces_med, observations = obs_Kemp_OR_Ffeces_med)
  ...
  score[8] <- AAFE(predictions = preds_Kemp_OR_Furine_high, observations = obs_Kemp_OR_Furine_high)
  ...
  score[9] <- AAFE(predictions = preds_Kemp_OR_Ffeces_high, observations = obs_Kemp_OR_Ffeces_high)
  ...
  score[10] <- AAFE(predictions = preds_dzi_IV_Fserum, observations = obs_dzi_IV_Fserum)
  ...
  score[11] <- AAFE(predictions = preds_dzi_OR_Fserum_low, observations = obs_dzi_OR_Fserum_low)
  ...
  score[12] <- AAFE(predictions = preds_dzi_OR_Fserum_medium, observations = obs_dzi_OR_Fserum_medium)
  ...
  score[13] <- AAFE(predictions = preds_dzi_OR_Fserum_high, observations = obs_dzi_OR_Fserum_high)
  ...
  score[14] <- AAFE(predictions = preds_kim_OR_Fblood, observations = obs_kim_OR_Fblood)
  ...
  score[15] <- AAFE(predictions = preds_kim_IV_Fblood, observations = obs_kim_IV_Fblood)
...
setwd("/Users/user/Documents/GitHub/PFAS_PBK_models/PFOA inhalation Rat/Scenarios/permeability/IF2_no_apical")
...
fit <- c(rep(log(1),7), rep(log(mean(c(Papp_Kimura,Papp_RYU))),3), log(1), log(1),log(1e-3))

lb = c(rep(log(1e-8),3), log(1e-5), log(1e-3),rep(log(1e-8),2),  rep(log(Papp_RYU),3), log(1e-3), log(1e-6), log(1e-5))
ub = c(rep(log(1e8),  3),log(1e5),  log(1e3),rep(log(1e8),  2), rep(log(10*Papp_Kimura),3),log(10),log(10),  log(1e2) )
...
estimated_params <- exp(optimizer$solution)
save.image("IF2_no_apical.RData")
