...
ode.func <- function(time, inits, params, growth_model){
  if ((time %% 24) == 0){
    BW_init <- unlist(params["BW"])
    BW_new <- growth_model(time, params["sex"], BW_init)
  ...
  "QparaMu" ,"QparaAd" ,"QparaRe" ,"QparaLu" ,"QparaSp","QparaHt" ,"QparaBr" ,"QparaGo","QparaSk" ,"QparaBo")
    ...
    params[BW_scaled_params] <- lapply(params[BW_scaled_params], function(x) x * BW_new)
    params[Q_scaled_params] <- lapply(params[Q_scaled_params], function(x) x * unlist(params["Qcardiac"]))
...
growth_model <- function(t, sex, initial_weight) {
  # Example using Gompertz growth model
  # Modify based on your preferred growth model
  W0 <- initial_weight
  if (sex == "M"){
    growth_rate = 5.9/1000 #g/d
    Wt <- W0 + growth_rate*t/24
    if(Wt>0.597){
      Wt<-0.597
    }
  }else{
...
obj.func <- function(x, dataset, fixed_params){
  ...
  solution <- data.frame(deSolve::ode(times = sample_time,  func = ode.func,
                                      ...
                                      events = events, growth_model = growth_model,
  ...
  inits <- create.inits (params)
  solution <- data.frame(deSolve::ode(times = sample_time,  func = ode.func,
                                      y = inits, parms = params, events = events, growth_model = growth_model,
                                      method="lsodes",rtol = 1e-04, atol = 1e-04))
...
setwd("/Users/ptsir/Documents/GitHub/PFAS_PBK_models/PFOA inhalation Rat")
...
setwd("/Users/ptsir/Documents/GitHub/PFAS_PBK_models/PFOA inhalation Rat/Scenarios/proximal_tubule/scenario24l_PkSim")
...
estimated_params <- exp(optimizer$solution)
save.image("scenario24l_PkSim.RData")
