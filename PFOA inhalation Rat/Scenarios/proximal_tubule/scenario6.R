...
create_variable_params <- function(BW,sex,  estimated_params, fixed_params){
  ...
  if(sex == "M"){
    ...
    CL_int <- estimated_params[10] #uL/min/million hepatocytes
    
  }else{
    RAFOatp_k <- estimated_params[2]
    CL_int <- estimated_params[11] #uL/min/million hepatocytes
  ...
  Ka <- 7e4 # 5.8e05 from Ryu et al. (2024)#mol/L
...
setwd("C:/Users/user/Documents/GitHub/PFAS_PBK_models/PFOA inhalation Rat/Scenarios/proximal_tubule/scenario6")
...
fit <-  c(rep(log(1),6), log(mean(c(Papp_Kimura,Papp_RYU))), log(1),log(1e-3), rep(log(1),2))

lb = c(rep(log(1e-20),6), log(Papp_RYU),log(1e-2), log(1e-4), rep(log(1e-20),2))
ub = c(rep(log(1e10),  6), log(Papp_Kimura) ,log(10),  log(1e1), rep(log(1e10),2))
