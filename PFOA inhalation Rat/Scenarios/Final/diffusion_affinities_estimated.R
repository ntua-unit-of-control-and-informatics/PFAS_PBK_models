...
create_variable_params <- function(BW,sex,  estimated_params, fixed_params){
  ...
  KLfabp <- estimated_params[9]#(1.2e5+4e4+1.9e4) #[L/mol]*1e-3 , value from Cheng et al. (2017)
  Ka <- estimated_params[10]#5.6e5# from Rue et al. (2024)#mol/L
  Ka2u <- estimated_params[11]
  
  f_alb_avail<-  1#estimated_params[9]
  f_fabp_avail  <- 1#estimated_params[10]
  f_a2u_avail <-  1#estimated_params[11]
...
setwd("/Users/user/Documents/GitHub/PFAS_PBK_models/PFOA inhalation Rat/Scenarios/Final/diffusion_affinities_estimated")
...
fit <- log(c(3.853823e+01, 4.113336e-05, 5.238279e+03, 7.352394e+01, 3.607135e+00, 
             1.470813e+00, 1.109408e-04,  9.651647e-05,1e5, 1e4, 1e3))



lb = c(rep(log(1e-7),3), log(1e-3), log(1e-1), log(1e-4), log(1e-6), log(1e-6),log(1e2),log(1e2),log(1e2))
ub = c(rep(log(1e5),  3),log(1e3),  log(2e1), log(1e2),  log(0.01),  log(1e-3),log(1e8),log(1e8),log(1e8))
...
estimated_params <- exp(optimizer$solution)
save.image("diffusion_affinities_estimated.RData")
