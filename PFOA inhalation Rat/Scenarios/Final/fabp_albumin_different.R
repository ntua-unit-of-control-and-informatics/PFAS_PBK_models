...
create_variable_params <- function(BW,sex,  estimated_params, fixed_params){
  ...
  if(sex == "M"){
    ...
    f_fabp_avail  <-estimated_params[14]
  ...
  koff_alb <-  estimated_params[15]
  koff_fabp <-    estimated_params[16]#
  koff_a2u <- koff_alb
...
setwd("/Users/user/Documents/GitHub/PFAS_PBK_models/PFOA inhalation Rat/Scenarios/Final/fabp_albumin_different")
...
fit <- log(c(4.135966e+01, 4.232215e-07, 6.531865e+03, 1.456908e+02, 2.506555e+00, 1.425835e-03, 
             7.915158e-05, 1.240552e+07,
          1.938180e+04, 7.102263e-05, 1.969160e+00, 2.226372e+00, 9.963063e-02, 3.362341e-02, 7.691145e-01, 
          7.691145e-01))

lb = c(rep(log(1e-7),3), log(1e-3), log(1e-1), log(1e-4), rep(log(Papp_RYU/10),1), log(1e4),  log(5e3), log(1e-6),log(1e-2),log(5e-3),log(5e-3),log(5e-3),log(1e-6),log(1e-6))
ub = c(rep(log(1e5),  3),log(1e3),  log(2e1), log(1e2), rep(log(Papp_Kimura),1),log(1e8), log(1e6) ,  log(1e-3),log(5),log(5),log(5),log(5),log(100),log(100))
...
estimated_params <- exp(optimizer$solution)
save.image("fabp_albumin_different.RData")
