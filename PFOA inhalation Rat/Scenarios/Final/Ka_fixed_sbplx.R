...
create_variable_params <- function(BW,sex,  estimated_params, fixed_params){
  ...
  Ka <- 5.6e5# 7e4 # from Rue et al. (2024)#mol/L
...
setwd("/Users/user/Documents/GitHub/PFAS_PBK_models/PFOA inhalation Rat/Scenarios/Final/Ka_fixed")
...
fit <- log(c(4.303204e+01,1.701470e-06,8.541510e+03,1.091045e+02,3.591703e+00,1.829126e+00,7.936052e-04,1.441703e+06,
             7.363177e-05,0.3,1.119162e-01,5.844764e-02,1e-3))

lb = c(rep(log(1e-7),3), log(1e-3), log(1e-1), log(1e-4), rep(log(Papp_RYU/10),1), log(1e4), log(1e-6),log(1e-2),log(5e-3),log(5e-3),log(1e-6))
ub = c(rep(log(1e5),  3),log(1e3),  log(2e1), log(1e2), rep(log(1e-3),1),log(1e8),   log(1e-3),log(5),log(5),log(5),log(100))
...
estimated_params <- exp(optimizer$solution)
save.image("Ka_fixed_spblx.RData")
