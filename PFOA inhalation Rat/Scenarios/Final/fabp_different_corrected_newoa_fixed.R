...
create_variable_params <- function(BW,sex,  estimated_params, fixed_params){
  ...
  if(sex == "M"){
    RAFOatp_k <- 27
    VmK_api <- 0
    f_fabp_avail  <-estimated_params[4]
    
  }else{
    RAFOatp_k <- 2.989212e-06
    VmK_api <- 0#estimated_params[15]
    f_fabp_avail  <-estimated_params[5]
    
  }
  RAFOat3 <-  7.906265e+03
  CL_int <- 1.678684e+02 #uL/min/million hepatocytes
  RAFOatp_l <- 2.076763e+00
  RAFUrat <- RAFOatp_k
  RAFOat1 <- RAFOat3
  ...
  Pgap_continuous <-   2.970318e-03  #, cm/s, permeability by diffusion through tight junctions in continuous capillary types
  ...
  f_alb_avail<- 2.797643e+00
  
  koff_alb <- estimated_params[2]
  koff_fabp <-     estimated_params[3]#
  koff_a2u <- koff_alb
  
  VmK_baso <- 0
  KmK_baso <- 1e20
  KmK_api <-   5e4
  KLfabp <-   5.392697e+06#(1.2e5+4e4+1.9e4) #[L/mol]*1e-3 , value from Cheng et al. (2017)
  Ka <- 1.954957e+04# 7e4 # from Rue et al. (2024)#mol/L
  CLfeces_unscaled <-8.303451e-05#in L/h/BW^(-0.25), scaling similar to Loccisano et al. (2012)
...
fit <- log(c(2.18, 1, 1.6e-03,  2.71e-01, 1.98e-01))
...
estimated_params <- exp(optimizer$solution)
save.image("fabp_different_corrected_newoa_fixed.RData")
