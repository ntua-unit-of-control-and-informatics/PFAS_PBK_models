...
create_variable_params <- function(BW,sex,  estimated_params, fixed_params){
  ...
  koff_alb <-  1#estimated_params[14]
...
setwd("/Users/user/Documents/GitHub/PFAS_PBK_models/PFOA inhalation Rat/Scenarios/Final/fabp_different")
...
fit <- log(c(3.421976e+01, 2.361523e-06, 1.118783e+04, 1.929577e+02, 5.632397e+00, 2.739665e+00,
             1e-05, 3.224089e+07,
             1.792674e+04, 4.581313e-05, 3.184580e+00, 1.119164e-01, 2.633966e-02, 1))

lb = c(rep(log(1e-7),3), log(1e-3), log(1e-1), log(1e-4), rep(log(Papp_RYU/10),1), log(1e4),  log(5e3), log(1e-6),log(1e-2),log(5e-3),log(5e-3),log(1e-6))
ub = c(rep(log(1e5),  3),log(1e3),  log(2e1), log(1e2), rep(log(1e-3),1),log(1e8), log(1e6) ,  log(1e-3),log(5),log(5),log(5),log(100))
...
estimated_params <- exp(optimizer$solution)
save.image("fabp_different_corrected_NM.RData")
