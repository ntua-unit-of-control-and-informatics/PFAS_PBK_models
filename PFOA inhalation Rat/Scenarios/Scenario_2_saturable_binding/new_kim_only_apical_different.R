...
create.params <- function(user.input){
  with(as.list(user.input),{
    ...
    P_liver_bile <- estimated_params[5] 
...
setwd("C:/Users/user/Documents/GitHub/PFAS_PBK_models/PFOA inhalation Rat/Scenarios/Scenario_2_saturable_binding/Training/AAFE/new_kim_only_apical_different")
...
fit <-  c(rep(log(1),4), log(1), log(0.1), rep(log(1e3),3))

lb	= c(rep(log(1e-3), 4),  log(1e-3), log(1e-4), rep(log(1),3))
ub = c(rep(log(1e3), 4), log(1e3),log(1e2), rep(log(1e6),3))
...
estimated_params <- exp(optimizer$solution)
save.image("new_kim_only_apical_different.RData")
