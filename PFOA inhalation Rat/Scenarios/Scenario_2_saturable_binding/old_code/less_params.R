...
create.params <- function(user.input){
  with(as.list(user.input),{
    ...
    if (sex == "M"){
      ...
      VmK_api <-estimated_params[9]
      P_liver_bile <- estimated_params[14] 
      ...
      P_liver_bile <- estimated_params[15] 
    ...
    KmK_baso <- estimated_params[12] 
    KmK_api <-  estimated_params[13] 
    
    f_prot_avail <- estimated_params[16]
    RAF_papp <- 1
    
    koff_alb <- estimated_params[17]
    koff_fabp <-  koff_alb
    koff_a2u <- koff_alb
...
setwd("C:/Users/user/Documents/GitHub/PFAS_PBK_models/PFOA inhalation Rat/Scenarios/Scenario_2_saturable_binding/Training/AAFE/less_params")
...
fit <-  c(rep(log(1),7), rep(log(3e4),6), rep(log(1),2), log(0.8), log(0.1))

lb	= c(rep(log(1e-2), 7), rep(log(1e3),6), rep(log(1e-3),2), log(0.01), log(1e-3))
ub = c(rep(log(1e2), 7), rep(log(1e6),6), rep(log(1e3),2), log(1), log(1e2))
