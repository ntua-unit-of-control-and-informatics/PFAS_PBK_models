...
create.params <- function(user.input){
  with(as.list(user.input),{
    ...
    CalbB_init <- RAF_protein*486*1e-06 # #[umol/L]*1e-6 -->(mol/L), from Cheng et al. (2017)
    
    
    CalbKF_init <- RAF_protein*243*1e-6 # #[umol/L]*1e-6 -->(mol/L), from Cheng et al. (2017)
    CalbLF_init <- RAF_protein*243*1e-6 # #[umol/L]*1e-6 -->(mol/L), from Cheng et al. (2017)
    CalbSTF_init <- RAF_protein*146*1e-6 # [umol/L]*1e-6 -->(mol/L), same as Gut (assumption)
    CalbINF_init <- RAF_protein*146*1e-6 #[umol/L]*1e-6 -->(mol/L), same as Gut (assumption)
    CalbMF_init <- RAF_protein*146*1e-6 #[umol/L]*1e-6 -->(mol/L), from Cheng et al. (2017)
    CalbAF_init <- RAF_protein*73*1e-6 #[umol/L]*1e-6 -->(mol/L), from Cheng et al. (2017)
    CalbRF_init <- RAF_protein*73*1e-6 #[umol/L]*1e-6 -->(mol/L), from Cheng et al. (2017)
    CalbBoF_init <-RAF_protein* 73*1e-6 #[umol/L]*1e-6 -->(mol/L), from Cheng et al. (2017)
    CalbLuF_init = RAF_protein*CalbINF_init #assumption 
    CalbLuAF_init <- RAF_protein*10/100 * CalbB_init #based on Woods et al. 2015 statement https://doi.org/10.1016/j.jconrel.2015.05.269
    
    MW_albumin <- 66500#g/mol
    CalbSPF_init <- RAF_protein*243e-6 #[umol/L]*1e-6 -->(mol/L), same as liver (assumption)
    CalbGoF_init <- RAF_protein*41/MW_albumin #mg/mL-->  (mol/L) from https://doi.org/10.1210/endo-116-5-1983 --> 41 mg/mL, MW=65 kg/mol
    CalbHF_init <- RAF_protein* 65/MW_albumin##mg/mL--> (mol/L) https://doi.org/10.1007/s12291-010-0042-x --> 6.5 g/100 g tissue, MW=65 kg/mol 
    CalbBrF_init <- RAF_protein*8e-2/MW_albumin  ##mg/mL--> (mol/L) https://doi.org/10.1016/0014-4886(90)90158-O --> 0.08 g/L, MW=65 kg/mol 
    CalbSKF_init <- RAF_protein*21/MW_albumin ##mg/mL-->  (mol/L) https://doi.org/10.1111/j.1748-1716.1973.tb05464.x -->Table 2: 2.1 g/100 mL
    ...
    if (sex == "M"){
      ...
      Ca2uKT_init <- RAF_protein*(a2u_globulin_k*1e-3/15.5e3) #[mol/L]
    ...
    CFabpLT_init = RAF_protein*(L_FABP_L*1e-3/14e3) #[mol/L]
    
    
    #LFABP concentration in kidney and liver tissue (mol/m^3)
    CFabpKT_init <- RAF_protein*2.65*1e-6  #[umol/L]*1e-6 -->(mol/L), from Cheng et al. (2017)
...
#setwd("C:/Users/user/Documents/GitHub/PFAS_PBK_models/PFOA inhalation Rat/Scenarios/Scenario_2/Training/SODI/NoStomachAbs")
setwd("C:/Users/user/Documents/GitHub/PFAS_PBK_models/PFOA inhalation Rat/Scenarios/Scenario_2_saturable_binding/Training/SODI/sum_normal_space")
...
fit <-  exp(c(rep(log(1),7), log(0.1), log(1)))

lb	= exp(c(rep(log(1e-10), 7),log(1e-3), log(1e-3)))
ub = exp(c(rep(log(1e10), 7),log(1e5), log(1e3)))
...
save.image("Scenario_2_saturable_binding_SODI_spblx_sum_normal_space.RData")
