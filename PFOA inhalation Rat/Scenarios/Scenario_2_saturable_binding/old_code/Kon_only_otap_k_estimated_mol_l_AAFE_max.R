...
create.params <- function(user.input){
  with(as.list(user.input),{
    ...
    RAF_protein <- estimated_params[8]
    RAF_papp <- estimated_params[9]
...
fit <-  c(rep(log(1),9), log(0.1), log(1))

lb	= c(rep(log(1e-10), 9),log(1e-3), log(1e-3))
ub = c(rep(log(1e10), 9),log(1e3), log(1e3))
...
estimated_params <- exp(optimizer$solution)
save.image("Scenario_2_saturable_binding_aafe_spblx.RData")
