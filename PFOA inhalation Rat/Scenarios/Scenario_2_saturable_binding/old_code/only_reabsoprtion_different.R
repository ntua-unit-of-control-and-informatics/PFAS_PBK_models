...
create.params <- function(user.input){
  with(as.list(user.input),{
    ...
    RAFNtcp <- RAFOatp_l
    VmK_api <-estimated_params[6]
    KmK_api <-  estimated_params[7] 
...
setwd("C:/Users/user/Documents/GitHub/PFAS_PBK_models/PFOA inhalation Rat/Scenarios/Scenario_2_saturable_binding/Training/AAFE/only_reabsorption_different")
...
fit <-  c(rep(log(1),5), rep(log(3e4),2), rep(log(1),3), log(1), log(0.1))

lb	= c(rep(log(1e-10), 5), rep(log(1),2), rep(log(1e-3),3), log(1e-3), log(1e-2))
ub = c(rep(log(1e10), 5), rep(log(1e6),2), rep(log(1e3),3), log(1e3), log(1))
...
estimated_params <- exp(optimizer$solution)
save.image("only_reabsorption_different.RData")
