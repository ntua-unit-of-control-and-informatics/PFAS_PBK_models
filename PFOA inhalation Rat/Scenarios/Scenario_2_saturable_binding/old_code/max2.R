...
create.params <- function(user.input){
  with(as.list(user.input),{
    ...
    if (sex == "M"){
      ...
      RAFUrat <- estimated_params[5]
      RAFOatp2_l <- estimated_params[6]
      RAFNtcp <- estimated_params[7]
      RAFOatp2_Int <- estimated_params[8]
      
      VmK_baso <- estimated_params[17]
      VmK_api <-estimated_params[18]
      P_liver_bile <- estimated_params[23] 
      
      
    }else if(sex == "F"){
      RAFOatp_k <- estimated_params[9]
      RAFOat1 <- estimated_params[10]
      RAFOatp_l <- estimated_params[11]
      RAFOat3 <- estimated_params[12]
      RAFUrat <- estimated_params[13]
      RAFOatp2_l <- estimated_params[14]
      RAFNtcp <- estimated_params[15]
      RAFOatp2_Int <- estimated_params[16]
      
      VmK_baso <- estimated_params[19]
      VmK_api <- estimated_params[20]
      P_liver_bile <- estimated_params[24] 
      

    }
    KmK_baso <- estimated_params[21] 
    KmK_api <-  estimated_params[22] 
    
    f_prot_avail <- 1
    RAF_papp <- 1
    
    koff_alb <- estimated_params[25]
    koff_fabp <-  estimated_params[26]
    koff_a2u <- estimated_params[27]
...
setwd("C:/Users/user/Documents/GitHub/PFAS_PBK_models/PFOA inhalation Rat/Scenarios/Scenario_2_saturable_binding/Training/AAFE/max2")
...
fit <-  c(rep(log(1),16), rep(log(3e4),6), rep(log(1),2), rep(log(0.1),3))

lb	= c(rep(log(1e-2), 16), rep(log(1e3),6), rep(log(1e-3),2),  rep(log(1e-3),3))
ub = c(rep(log(1e2), 16), rep(log(1e6),6), rep(log(1e3),2),  rep(log(1e2),3))
