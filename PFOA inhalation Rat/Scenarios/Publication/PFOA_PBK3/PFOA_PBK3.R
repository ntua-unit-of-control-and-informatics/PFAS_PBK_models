...
obj.func <- function(x, dataset, fixed_params){
  ...
  obs_kim_OR_Mblood <- list(exp_data[exp_data$Tissue == "Plasma", "concentration"])
    ...
    score[35] <- 10*AAFE(predictions = list(preds_kim_OR_Mblood[[1]][length(preds_kim_OR_Mblood[[1]])]),                    
  ...
  obs_kim_IV_Mblood <- list(exp_data[exp_data$Tissue == "Plasma", "concentration"])
    ...
    N_points <- N_points + N_kim_IV_Mblood 
     ...
     score[36] <- 10*AAFE(predictions = list(preds_kim_IV_Mblood[[1]][length(preds_kim_IV_Mblood[[1]])]),
  ...
  score[37] <- 10 * AAFE(predictions = list(preds_kim_OR_Fblood[[1]][length(preds_kim_OR_Fblood[[1]])]), 
  ...
  score[38] <- 10* AAFE(predictions = list(preds_kim_IV_Fblood[[1]][length(preds_kim_IV_Fblood[[1]])]), 
...
setwd("/Users/user/Documents/GitHub/PFAS_PBK_models/PFOA inhalation Rat/Scenarios/Publication/PFOA_PBK3")
...
fit <- log(c(1.8e+01, 1e-03, 1e3, 0.02, 1, 1, 2e5, 1e-4, 1e-3, 0.02))
