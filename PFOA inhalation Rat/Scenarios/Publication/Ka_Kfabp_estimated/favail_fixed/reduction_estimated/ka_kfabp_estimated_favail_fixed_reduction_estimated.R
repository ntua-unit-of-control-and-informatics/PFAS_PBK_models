...
create_variable_params <- function(BW,sex,  estimated_params, fixed_params){
  ...
  KLfabp <-  estimated_params[9]#[L/mol]
  ...
  koff_alb <-  1e-2*3600#estimated_params[8]*3600#[1/h]
  koff_fabp <-  1e-2*3600#estimated_params[9]*3600#[1/h]
  koff_a2u <- koff_alb
...
setwd("/Users/user/Documents/GitHub/PFAS_PBK_models/PFOA inhalation Rat/Scenarios/Publication/Ka_Kfabp_estimated/favail_fixed/reduction_estimated")
...
fit <- log(c(1.8e+01, 1e-03, 1e3, 0.02, 1, 1, 1e-4,  0.1, 1e5, 1e5))
...
estimated_params <- exp(optimizer$solution)
save.image("ka_kfabp_estimated_favail_fixed_reduction_estimated.RData")
