...
create_variable_params <- function(BW,sex,  estimated_params, fixed_params){
  ...
  Ka <- 5.8e5#[L/mol]. From Rue et al. (2024)
  ...
  f_alb_avail<-  estimated_params[8]#estimated_params[8]
  f_fabp_avail  <- estimated_params[9]#estimated_params[9]
  
  f_a2u_avail <- 1
  
  koff_alb <-  1e-1#estimated_params[8]*3600#[1/h]
  koff_fabp <-  1e-1#estimated_params[9]*3600#[1/h]
  koff_a2u <- koff_alb
  ...
  #LFABP concentration in liver tissue 
  cytosolic_protein <-105*(1000*fixed_params$MLi_drained) #[mg], from Krähenbühl and Krähenbühl (2023) [doi: 10.3390/ijms24054365]
  ...
  if (sex == "M"){
    ...
    L_FABP_L = 30.1*1e-3*cytosolic_protein/fixed_params$VLi_drained #[mg/L]. From  Ockner and  Manning (1976) [doi: 10.1016/S0021-9258(18)34463-6] 
    CFabpLT_init = f_fabp_avail*(L_FABP_L*1e-3/Mr_fabp) #[mol/L]
  }else if(sex == "F"){
    Ca2uKT_init <- 0 #mol/L
    
    L_FABP_L = 51.7*1e-3*cytosolic_protein/fixed_params$VLi_drained #[mg/L]. From  Ockner and  Manning (1976) [doi: 10.1016/S0021-9258(18)34463-6] 
    CFabpLT_init = f_fabp_avail*(L_FABP_L*1e-3/Mr_fabp) #[mol/L]
  }
  
  #LFABP concentration in kidney tissue 
  CFabpKT_init <- f_fabp_avail*2.65*1e-6  #[mol/L], from Cheng et al. (2017)

  kon_alb <- Ka * koff_alb #h/M
  kon_a2u <- Ka2u * koff_a2u#h/M
  kon_fabp <- KLfabp * koff_fabp #h/M
...
setwd("/Users/user/Documents/GitHub/PFAS_PBK_models/PFOA inhalation Rat/Scenarios/Publication/Ka_albumin_fixed/favail_estimated/reduction_fixed/reduction_0.01")
...
fit <- log(c(1.8e+01, 1e-03, 1e3, 0.02, 1, 1, 1e-4, 1, 1))
...
estimated_params <- exp(optimizer$solution)
save.image("ka_fixed_favail_estimated_reduction_fixed_0.01.RData")
