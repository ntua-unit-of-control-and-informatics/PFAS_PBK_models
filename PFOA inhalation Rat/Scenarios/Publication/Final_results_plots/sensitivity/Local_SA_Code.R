...
library(deSolve)
library(splines)
setwd("C:\\Users\\ptsir\\Documents\\GitHub\\PFAS_PBK_models\\PFOA inhalation Rat\\sensitivity_analysis")
...
create_params <- function(user.input){
  with(as.list(user.input),{  
    ...
    kPtcTu <- ((2*Peff_monolayer/100) *APT) *1000 #diffusion between proximal tubule cells and tubule filtrate
    kDalcTu <- ((2*Peff_monolayer/100) *ADAL) *1000 #diffusion between descending/ascending cells and tubule filtrate
    kDtcTu <- ((2*Peff_monolayer/100) *ADT) *1000 #diffusion between distal tubule cells and tubule filtrate
    kCdcTu <- ((2*Peff_monolayer/100) *ACD) *1000 #diffusion between collecting duct cells and tubule filtrate
    ...
    kDalcF <- ((2*Peff_monolayer) *AcK_DALC) *1000 #diffusion between proximal tubule cells and interstitial space
    kCdcF <- ((2*Peff_monolayer/100) *AcK_CDC) *1000 #diffusion between descending/ascending cells and interstitial space
    kKTrestF  <- ((2*Peff_monolayer/100) *AcKTrest) *1000 #diffusion between rest of kidney cells and interstitial space
...
PBPK_sensitivity <- function(model, thetas, thetas_names, user_input,
  ...
  for (i in 1:N_targets) {
    AUC_0[i] <- AUC_trapezoid(solution_0[,"time"],solution_0[,targets[i]])
  ...
  for (i in 1:N_thetas) {
    ...
    for (j in 1:N_targets) {
      ...
      AUC_j <- AUC_trapezoid(solution[,"time"],solution[,targets[j]])
      ...
      SI_auc[i,j] <- 100*((AUC_j-AUC_0[j])/AUC_0[j])/((thetas[i]- thetas_0[i])/thetas_0[i])
      # Relative Sensitivity of Cmax = (dCmax/Cmax)/(dp/p)
      SI_cmax[i,j] <- 100*((Cmax_j-Cmax_0[j])/Cmax_0[j])/((thetas[i]- thetas_0[i])/thetas_0[i])
    }
  }
  ...
  if(heatmap){
    ...
    heatmap1 <- pheatmap::pheatmap(as.matrix(SI_auc),
                                   ...
                                   main="AUC Sensitivity Indexes", cluster_rows = FALSE , cluster_cols =  FALSE)
    ...
    heatmap2 <- pheatmap::pheatmap(as.matrix(SI_cmax),
                                   ...
                                   main="Cmax Sensitivity Indexes", cluster_rows = FALSE , cluster_cols =  FALSE)
...
input_all <- c("RAFOatp_k", "f_fabp_avail", "RAFOat3", "CL_int", "RAFOatp_l",
               ...
               "Ka", "CLfeces_unscaled", "RAFOatp_lu_ap", "RAFOatp2_Int", "kCLEal", "kabsUA", "kCLEua" )
...
output_all <- c( 'Cplasma', 'Ckidney', 'Cliver',  'Cstomach', 
                 'Cintestine', 'Cmuscle', 'Cadipose',"CUpperair",
                 'Clungs',  'Crest', 'Cspleen', 'Cheart', 'Cbrain', 
                 'Cgonads', 'Cskin', 'Cbones',
                 'Mfeces', 'Murine')
...
# #---------------------------------------
# #    Case 1: Male, nasal, medium dose
# #----------------------------------


BW <- 0.3  #kg, not reported in the study - 200-250 g average BW of male CD® IGS (SD) rats at 6 to 8 weekshttps://animalab.eu/cd-sprague-dawley-igs-rat-crl-cd-sd
sex <- "M"
inhalation_params=estimate_BFn_TVn(sex, BW)
BFn = inhalation_params["BFn"]# 1/h
TVn = inhalation_params["TVn"]# L
duration <- 6 #hours
admin.dose_mg_per_m3 <- 3 # administered dose in mg/m^3
depfr_head <- 0.2864
...
inits <- create.inits(params)
events <- create.events(params)
sample_time=seq(0,600,1)

thetas = default_params
thetas_names = names(thetas)
sensitivity = PBPK_sensitivity(model=ode.func,
                                   thetas=thetas,
...
# Positive change
png(filename = "AUC_heatmap_plot.png", width = width, height = height, res=res)
replayPlot(sensitivity$Plot_list$AUC_heatmap)
dev.off()
png(filename = "Cmax_heatmap_plot.png", width = width, height = height, res=res)
replayPlot(sensitivity$Plot_list$Cmax_heatmap)
dev.off()
