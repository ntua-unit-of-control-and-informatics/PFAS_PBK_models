...
create.params <- function(user.input){
  ...
  CL_hepatobiliary <- CL_int*1e-6*HEPGL*(MKi_drained*1000) *60 #L/h
  ...
  intestine_protein_total <- intestine_protein*(1000*MIn_drained) #mg
  ...
  kidney_protein_total <- kidney_protein_per_gram* (1000*MKi_drained) #mg
  ...
  liver_protein_total <- liver_protein_per_gram* (1000*MLi_drained) #mg
  ...
  lung_protein_total <- lung_protein_per_gram*(MLu_drained*1000) #mg
  ...
  cytosolic_protein_L <-105*(1000*MLi_drained) #[mg], from Krähenbühl and Krähenbühl (2023) [doi: 10.3390/ijms24054365]
  cytosolic_protein_K <- 53.3*(1000*Mcortex) #[mg], assuming same value as humans. From Scotcher et al. (2017) [doi: 10.1124/dmd.117.075242]
  if (sex == "M"){
    #Alpha2mu-globulin concentration in male kidney tissue
    a2u_globulin_k = 22.7*(kidney_protein_total/1000)/VKi_drained #mg/L. From Stonard et al. (1986) [doi: 10.1016/0300-483X(86)90197-6]
    Ca2uKT_init <- f_a2u_avail*(a2u_globulin_k*1e-3/Mr_a2u) #[mol/L]
    
    #LFABP concentration in male liver tissue
    L_FABP_L = 30.1*1e-3*cytosolic_protein_L/VLi_drained #[mg/L]. From  Ockner and  Manning (1976) [doi: 10.1016/S0021-9258(18)34463-6]
    CFabpLT_init = f_fabp_avail*(L_FABP_L*1e-3/Mr_fabp) #[mol/L]
    
    #LFABP concentration in male kidney tissue
    L_FABP_K = 0.078*1e-9*cytosolic_protein_K/VKi_drained #[mol/L]. From  Maatman et al. (1992) [doi: 10.1042/bj2880285]
    ...
    L_FABP_L = 51.7*1e-3*cytosolic_protein_L/VLi_drained #[mg/L]. From  Ockner and  Manning (1976) [doi: 10.1016/S0021-9258(18)34463-6]
    CFabpLT_init = f_fabp_avail*(L_FABP_L*1e-3/Mr_fabp) #[mol/L]
    
    #LFABP concentration in female kidney tissue
    L_FABP_K = 0.051*1e-9*cytosolic_protein_K/VKi_drained #[mol/L]. From  Maatman et al. (1992) [doi: 10.1042/bj2880285]
    CFabpKT_init <- f_fabp_avail*L_FABP_K  #[mol/L]
...
save.image("PBK_validation.RData")
