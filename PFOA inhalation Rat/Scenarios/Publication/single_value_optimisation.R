...
create_variable_params <- function(BW,sex, test,  fixed_params){
  
  estimated_params <- c(6.499781e+01, 6.909137e-04, 7.281340e+03, 2.873772e+01,
    3.671650e-01, 3.664009e-01, 2.203370e+05, 2.027476e+04, 1.438040e-04)
  ...
  RAFOat3 <- test[4]*estimated_params[3]
  ...
  KmK_baso <- 1e20
  KmK_api <-   5e4
  KLfabp <- estimated_params[7]#1.2e5 #[L/mol], from Sheng et al. 2018
  Ka <- estimated_params[8]#5.8e5# from Rue et al. (2024)#mol/L
  ...
  koff_alb <-  test[1]#estimated_params[11]
  koff_fabp <-  test[2]# estimated_params[12]#
  koff_a2u <- koff_alb
  
  reduction_factor <- test[3]
  ...
  # For the section below, we assume that all measurement were made relative to rinsed blood weight.
  # Consequently, we multiply by the mass of the drained organ
  ...
  intestine_protein_total <- intestine_protein*(1000*fixed_params$MIn_drained)
  ...
  kidney_cells = 1.47e07 #cells/g [doi: 10.1038/s41598-024-53270-2]
  kidney_cells_total <- kidney_cells* (1000*fixed_params$MKi_drained)
  ...
  CL_hepatobiliary <- CL_int*1e-6*HEPGL*fixed_params$MKi_drained*60 #L/h
  
  #oatp1-liver
  VmL_Oatp_in_vitro= 9.3 #nmol/mg protein/min  (Weaver et al. 2010)
  VmL_Oatp_scaled = 60*VmL_Oatp_in_vitro*MW*liver_protein_per_gram*(fixed_params$MLi_drained*1000)/1000   #physiologically scaled to in vivo, ug/h
  ...
  VmL_Oatp2_scaled = 60*VmL_Oatp2_in_vitro*MW*liver_protein_per_gram*(fixed_params$MLi_drained*1000)/1000  #ug/h
  ...
  #physiologically scaled to in vivo
  VmL_Ntcp_scaled = 60*VmL_Ntcp_in_vitro*MW*liver_protein_per_gram*(fixed_params$MLi_drained*1000)/1000 # ug/h 
  VmL_Ntcp = VmL_Ntcp_scaled*RAFNtcp #in vivo value, in  ug/h
  KmL_Ntcp= 20 * MW #umol/L, Ruggiero et al. 2021 --> ug/L
  
  #Lung
  lung_protein_per_gram <- 134 # 134 mg/mL tissue --> 134 mg/g tissue, Figure 2, [doi: 10.1007/s00580-021-03242-z] 
  
  #oatp-lung-ap (from ALF to tissue)
  VmLu_Oatp_ap_in_vitro= 9.3 #nmol/mg protein/min  (Weaver et al. 2010)
  VmLu_Oatp_ap_scaled = 60*VmLu_Oatp_ap_in_vitro*MW*lung_protein_per_gram*(fixed_params$MLu_drained*1000)/1000   #physiologically scaled to in vivo, ug/h
  ...
  VmLu_Oatp_bas_scaled = 60*VmLu_Oatp_bas_in_vitro*MW*lung_protein_per_gram*(fixed_params$MLu_drained*1000)/1000   #physiologically scaled to in vivo, ug/h
  ...
  Papp_RYU <- 1.46e-6 # cm/s, at pH = 7.4 from Ryu et al. (2024) [doi: 10.1016/j.chemosphere.2024.142390]
  Peff_monolayer <- Papp_RYU*3600 #cm/h
  
  
  #  Lin et al. (2023) used data from kimura et al. (2017) [doi: 10.1016/j.toxlet.2017.05.012]
  ...
  dif <- 5.46e-6 #[cm^2/s], diffusion coefficient of PFOA in water, from  Gauthier et al. (2024) [10.1021/acsestwater.4c00631]
  ...
  f_intestine <- 0.095 #Simionescu et al. 1974 [10.1083/jcb.60.1.128]
...
create_fixed_params <- function(user.input){
  with(as.list(user.input),{
    ...
    # We distinguish residual blood into total residual blood, which is the blood remaining in both large vessels and capillaries 
    # in nonbled animals and capillary residual blood, which is the blood remaining in capillaries in bled animals. We take 
    # the total residual blood fraction from Brown when possible (average of multiple studies in most cases). 
    ...
    # We assume that the fraction of interstitial space from Kawai et al. (1994) is relative to the drained organ volume
    
    #Kidney
    PVKi <- 7.3e-3 #fraction of drained kidney mass relative to body weight. from Brown et al. (1997)
    d_kidney <- 1.05 #density of kidneys
    ...
    VKiB_cap <- fcap_ki * VKi_drained  #volume of kidney blood in capillaries (lv)
    ...
    MK <- (VKi_drained - VKiB_cap) *d_kidney#total kidney mass
    ...
    VKiF <- PVKF *VKi_tot #kidney interstitial fluid volume 
    VKiB_lv <-  VKiB - VKiB_cap #volume of kidney blood in large vessels 
    ...
    VKi_ref <- 2*2.9/1000 # [g]. Gilmer et al. (2018) handled data from Sperber, 1944 (Thesis: “Studies on the Mammalian Kidney").
    ...
    Qcardiac <- 0.235 * (BW^0.75) *60*1000 #L/min->*60-> L/h
...
obj.func <- function(x, dataset, fixed_params){
  ...
  score[7] <- N_dzi_OR_Mtissues*tissue_multiplier*AAFE(predictions = preds_dzi_OR_Mtissues, observations = obs_dzi_OR_Mtissues)  
  ...
  score[8] <- N_dzi_OR_Ftissues*tissue_multiplier*AAFE(predictions = preds_dzi_OR_Ftissues, observations = obs_dzi_OR_Ftissues)  
  N_points <- N_points + N_dzi_OR_Ftissues*tissue_multiplier   
  
  
  N_dzi_OR_Ftissues2 <- length(preds_dzi_OR_Ftissues[[1]])
  score[40] <- N_dzi_OR_Ftissues2*AAFE(predictions = preds_dzi_OR_Ftissues[[1]], observations = obs_dzi_OR_Ftissues[[1]])
  N_points <- N_dzi_OR_Ftissues2 + N_points
  ##########################
  #-------------------------
  # Kim ORAL male blood
  #-------------------------
  ...
  score[35] <- 5*AAFE(predictions = list(preds_kim_OR_Mblood[[1]][length(preds_kim_OR_Mblood[[1]])]),                    
  ...
  score[36] <- 5*AAFE(predictions = list(preds_kim_IV_Mblood[[1]][length(preds_kim_IV_Mblood[[1]])]),                     observations = list(obs_kim_IV_Mblood[[1]][length(preds_kim_IV_Mblood[[1]])]))
  N_points <- N_points + 5
  ##########################
  #-------------------------
  # Kemper 2003 (Worley)
...
setwd("/Users/user/Documents/GitHub/PFAS_PBK_models/PFOA inhalation Rat/Scenarios/Publication/PFOA_PBK2")
...
######################
####Test koff_alb
koff_alb_res <- rep(NA,10)
koff_alb_res <- rep(NA,2)
counter <- 1
for (val in c( 10, 20)){
  print(counter)
  test_pars <- c(val, 1,  0.01,  1)
  koff_alb_res[counter] <- obj.func(test_pars, dataset, fixed_params)
  counter <- counter +1
}
...
koff_fabp_res <- rep(NA,15)
counter <- 1
for (val in c(1e-4, 5e-4, 7.5e-4, 1e-3, 2.5e-3, 5e-3, 7.5e-3, 1e-2, 2.5e-2, 5e-2, 7.5e-2, 1e-1, 2e-1, 5e-1,1, 2)){
  print(counter)
  test_pars <- c(1.5,val, 0.01, 1)
  koff_fabp_res[counter] <- obj.func(test_pars, dataset, fixed_params)
  counter <- counter + 1
...
reduction_factor_res <- rep(NA,3)
counter <- 1
for (val in c( 2e-2,  3e-2, 4e-2)){
  print(counter)
  test_pars <- c(1.5,0.5, val, 1)
...
######################
####Test RAFOat3
RAFOat3_res <- rep(NA,4)
counter <- 1
for (val in c(0.7, 0.8, 0.9, 1.1)){
  print(counter)
  test_pars <- c(1.5, 0.5, 2e-2, val)
