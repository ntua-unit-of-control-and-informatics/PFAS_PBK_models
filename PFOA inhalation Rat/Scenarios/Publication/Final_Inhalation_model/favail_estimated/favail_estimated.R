...
create_variable_params <- function(BW,sex,  estimated_params, fixed_params){
  ...
  RAFOatp_lu_ap <- 0#4.060400e-01
  RAFOatp_lu_bas <- estimated_params[2]
  ...
  if(sex == "M"){
    RAFOatp_k <- 2.284426e+01
  }else{
    RAFOatp_k <-5.234341e-03
  }
  RAFOat3 <- 5.933905e+03
  
  CL_int <- 2.416581e-01 #uL/min/million hepatocytes
  ...
  RAFOatp_l <-5.999219e+00
  RAFUrat <- RAFOatp_k
  RAFOat1 <- RAFOat3
  RAFOatp2_l <- RAFOatp_l

  RAFNtcp <- RAFOatp_l
  RAFOatp2_Int <-  9.057593e-06
  ...
  Ka <-  2.532596e+04#[L/mol]. From Rue et al. (2024)
  Ka2u <- 1e03 #[L/mol], from Han et al. (2004) [doi:10.1081/DCT-200039725]
  
  CLfeces_unscaled <- 6.487071e-05#in L/h/BW^(-0.25), scaling similar to Loccisano et al. (2012)
  CLfeces <- CLfeces_unscaled*BW^(-0.25) 
  
  f_alb_avail<-  6.539149e-01
  f_fabp_avail  <- 1.141497e-01
...
obj.func <- function(x, dataset, fixed_params){
  ...
  inits <- create.inits(params)
  events <- create.events(params)
  events$data$value <- events$data$value*estimated_params[3]
...
setwd("/Users/user/Documents/GitHub/PFAS_PBK_models/PFOA inhalation Rat/Scenarios/Publication/Final_Inhalation_model/favail_estimated")
...
save.image("favail_estimated.RData")
