...
create_variable_params <- function(BW,sex,  estimated_params, fixed_params){
  ...
  k_desorption_fast <-  estimated_params[3] #1/h
  k_desorption_slow <-  estimated_params[4] #1/h
  f_fast <- plogis(estimated_params[5])# fraction of PFOA released fast from dust
...
ode.func <- function(time, inits, params){
  ...
  with(as.list(c(inits, params)),{
    ...
    Clungs <-  (MLuB + MLuF+ MLuT + MLuAF+ MLuAFdust_slow + MLuAFdust_fast+Cblood*VLuB_lv)/VLu_tot
    ...
    CBALF <-  (MLuAF+MLuAFdust_slow + MLuAFdust_fast)/ VBALF_Gustaffson# This is for the Gustaffson et al. (2022) study which used 
...
obj.func <- function(x, dataset, fixed_params){
  ...
  inits <- create.inits(params)
  events <- create.events(params)
  events$data$value <- events$data$value*3
...
setwd("/Users/user/Documents/GitHub/PFAS_PBK_models/PFOA inhalation Rat/Scenarios/Publication/Final_Inhalation_model/favail_fixed_two_pools/dose=3")
...
save.image("favail_fixed_two_pools_dose=3.RData")
