...
create_variable_params <- function(BW,sex,  estimated_params, fixed_params){
  ...
  RAFOatp_lu_ap <- estimated_params[2]#4.992703e-01#4.060400e-01
  RAFOatp_lu_bas <- estimated_params[3]#RAFOatp_lu_ap
  k_desorption_fast <-  estimated_params[4] #1/h
  k_desorption_slow <-  estimated_params[5] #1/h
  f_fast <- plogis(estimated_params[6])# fraction of PFOA released fast from dust
  fdust_retained <-  plogis(estimated_params[7])
...
obj.func <- function(x, dataset, fixed_params){
  ...
  inits <- create.inits(params)
  events <- create.events(params)
  events$data$value <- events$data$value*2
  ...
  score[9] <- 2*AAFE(predictions = preds_gus_INH_Mtissues, observations = obs_gus_INH_Mtissues)
...
setwd("/Users/user/Documents/GitHub/PFAS_PBK_models/PFOA inhalation Rat/Scenarios/Publication/Final_Inhalation_model/favail_fixed_two_pools/dose=2")
...
fit <-  log(c(1,  0.1, 0.1, 1, 0.01, 0.3, 0.3))
...
save.image("favail_fixed_two_pools.RData")
