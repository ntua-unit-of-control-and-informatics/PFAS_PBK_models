...
create_variable_params <- function(BW,sex,  estimated_params, fixed_params){
  ...
  kCLEua <- estimated_params[2]#L/h/m^2
  k_desorption <- 1e3 #L/h
  RAFOatp_lu_ap <- estimated_params[3]#4.992703e-01#4.060400e-01
  RAFOatp_lu_bas <- estimated_params[4]#RAFOatp_lu_ap
...
obj.func <- function(x, dataset, fixed_params){
  ...
  inits <- create.inits(params)
  events <- create.events(params)
  events$data$value <- events$data$value*estimated_params[5]
...
setwd("/Users/user/Documents/GitHub/PFAS_PBK_models/PFOA inhalation Rat/Scenarios/Publication/Final_Inhalation_model/favail_fixed")
...
fit <-  log(c(1,1, 1e-05 , 1.5, 3 ))
lb <- log(c(1e-5, 1e-5, 1e-10, 1e-10, 1 ))
ub <- log(c(1e2, 1e2, 1e3, 1e3, 5 ))
...
save.image("favail_fixed2.RData")
