...
create.params <- function(user.input){
  with(as.list(user.input),{
    ...
    Ka <- estimated_params[15] # 5.8e05 from Rue et al. (2024)#mol/L
...
setwd("C:/Users/user/Documents/GitHub/PFAS_PBK_models/PFOA inhalation Rat/Scenarios/Scenario_Bladder/Training/AAFE/full_params_no_weights")
...
fit <-  c(rep(log(1),14), log(1e5) )

lb = c(rep(log(1e-20), 10), log(0.1),rep(log(0.1),3),  log(1e4))
ub = c(rep(log(1e7), 10),log(10), rep(log(2),3),  log(1e6) )
...
estimated_params <- exp(optimizer$solution)
save.image("full_params_no_weights.RData")
