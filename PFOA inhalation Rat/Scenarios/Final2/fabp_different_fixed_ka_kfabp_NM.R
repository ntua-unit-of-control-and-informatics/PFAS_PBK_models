...
create_variable_params <- function(BW,sex,  estimated_params, fixed_params){
  ...
  dif_reduction <- (1-lambda)^2 * (1 - 2.104*lambda + 2.09*lambda^3 - 0.95*lambda^5)
  ...
  Pgap <- dif*dif_reduction/ (wall_width+basement_membrane) #cm/s
  # In organs with sinusoid there is no basement membrane at gaps,
  # so we use basemenent membrane thickness = 0
  Pgap_sinusoidal <- dif*dif_reduction/ wall_width #cm/s
...
setwd("/Users/user/Documents/GitHub/PFAS_PBK_models/PFOA inhalation Rat/Scenarios/Final2/fabp_different_fixed_ka_kfabp")
...
opts <- list( "algorithm" = "NLOPT_LN_NELDERMEAD", #"NLOPT_LN_NEWUOA"   "NLOPT_LN_SBPLX"
              "xtol_rel" = 1e-06,
              "ftol_rel" = 1e-06,
              "ftol_abs" = 0.0,
              "xtol_abs" = 0.0, 
...
fit <- log(c(1.8e+01, 5e-05, 1e+03, 1e+02, 3.0e+00, 3.5e+00, 7e-05,
             2.316007e-01, 0.01,  0.01, 1, 1e-3))


lb = c(rep(log(1e-7),3), log(1e-3), log(1e-1), log(1e-4), log(1e-6), log(1e-4),log(1e-4),log(1e-4),log(1e-6),log(1e-6))
ub = c(rep(log(1e5),  3),log(1e3),  log(2e1), log(1e2),  log(0.01),  log(10),log(10),log(10),log(100),log(100))
...
estimated_params <- exp(optimizer$solution)
save.image("fabp_different_fixed_ka_kfabp_NM.RData")
