...
ga_fitness <- function(chromosome){
  ...
  res <- obj.func(chromosome, dataset, fixed_params)
  return(res)
...
fit <- log(c(1.8e+01, 5e-05, 1e+03, 1e+02, 3.0e+00, 3.5e+00, 7e-05,
             2.316007e-01, 0.01,  1, 1e-3, 1e-2))


lb = log(c(1e-3, 1e-8, 1e-2, 1e-3, 1e-2, 1e-2, 1e-6, 1e-3,1e-3,1e-4,1e-4, 1e-4))
ub = log(c(1e2,  1e-2,  1e4,  1e3, 1e3,  1e3,   1e2,   10,  10,  10,  10,    1 ))
...
GA_results <- GA::ga(type = "real", fitness = ga_fitness, 
                     ...
                     popSize = 5*(parallel::detectCores()-2), #the population size.
                     ...
                     maxiter = 50, #the maximum number of iterations to run before the GA search is halted.
