...
create_variable_params <- function(BW,sex,  estimated_params, fixed_params){
  ...
  koff_alb <- 2
  koff_fabp <-   estimated_params[10]#
  k_ns <-  estimated_params[11]
  koff_a2u <- koff_alb
...
ode.func <- function(time, inits, params){
  ...
  with(as.list(c(inits, params)),{
    ...
    CVen <- MVen/VVen
    CVenb <- MVenb/VVen
    CVenf <- MVenf/VVen/(1+k_ns)
    
    CArt <- MArt/VArt
    CArtf <-MArtf/VArt/(1+k_ns)
    CArtb <-MArtb/VArt
    ...
    CKBf <- MKBf/VKB/(1+k_ns)
    CKBb <- MKBb/VKB
    CKF <- MKF/VKF  #interstitial fluid concentration
    CKFf <- MKFf/VKF/(1+k_ns)
    CKFb <- MKFb/VKF
    
    CKTrestf <- MKTrestf/VKTrest/(1+k_ns)
    CKTrestb <- MKTrestb/VKTrest
    CPTCf <- MPTCf/VPTC/(1+k_ns)
    CPTCb <- MPTCb/VPTC
    CDALCf <- MDALCf/VDALC/(1+k_ns)
    CDALCb <- MDALCb/VDALC
    CDTCf <- MDTCf/VDTC/(1+k_ns)
    CDTCb <- MDTCb/VDTC
    CCDCf <- MCDCf/VCDC/(1+k_ns)
    CCDCb <- MCDCb/VCDC
    ...
    CSTBf <- MSTBf/VSTB/(1+k_ns)
    CSTBb <- MSTBb/VSTB
    CSTF <- MSTF/VSTF  #interstitial fluid concentration
    CSTFf <- MSTFf/VSTF/(1+k_ns)
    CSTFb <- MSTFb/VSTF
    CSTTf <- MSTTf/VSTT/(1+k_ns) # tissue concentration
    ...
    CINBf <- MINBf/VINB/(1+k_ns)
    CINBb <- MINBb/VINB
    CINF <- MINF/VINF  #Interstitial fluid concentration
    CINFf <- MINFf/VINF/(1+k_ns)
    CINFb <- MINFb/VINF
    CINTf <- MINTf/VINT/(1+k_ns) # tissue concentration
    ...
    CMBf <- MMBf/VMB/(1+k_ns)
    CMBb <- MMBb/VMB
    CMF <- MMF/VMF  #Interstitial fluid concentration
    CMFf <- MMFf/VMF/(1+k_ns)
    CMFb <- MMFb/VMF
    CMTf <-  MMTf/VMT/(1+k_ns) # tissue concentration
    ...
    CABf <- MABf/VAB/(1+k_ns)
    CABb <- MABb/VAB
    CAF <- MAF/VAF  #Interstitial fluid concentration
    CAFf <- MAFf/VAF/(1+k_ns)
    CAFb <- MAFb/VAF
    CATf <- MATf/VAT/(1+k_ns) # tissue concentration
    ...
    CRBf <- MRBf/VRB/(1+k_ns)
    CRBb <- MRBb/VRB
    CRF <- MRF/VRF  #Interstitial fluid concentration
    CRFf <- MRFf/VRF/(1+k_ns)
    CRFb <- MRFb/VRF
    CRTf <- MRTf/VRT/(1+k_ns) # tissue concentration
    ...
    CLuBf <- MLuBf/VLuB/(1+k_ns)
    CLuBb <- MLuBb/VLuB
    CLuF <- MLuF/VLuF  #Interstitial fluid concentration
    CLuFf <- MLuFf/VLuF/(1+k_ns)
    CLuFb <- MLuFb/VLuF
    CLuTf <- MLuTf/VLuT/(1+k_ns) #tissue concentration
    CLuAF <- MLuAF/VLuAF #alveolar lining fluid concentration
    CLuAFf <- MLuAFf/VLuAF/(1+k_ns)
    CLuAFb <- MLuAFb/VLuAF
    ...
    CSPBf <- MSPBf/VSPB/(1+k_ns)
    CSPBb <- MSPBb/VSPB
    CSPF <- MSPF/VSPF  #Interstitial fluid concentration
    CSPFf <- MSPFf/VSPF/(1+k_ns)
    CSPFb <- MSPFb/VSPF
    CSPTf <-  MSPTf/VSPT/(1+k_ns) # tissue concentration
    ...
    CHBf <- MHBf/VHB/(1+k_ns)
    CHBb <- MHBb/VHB
    CHF <- MHF/VHF  #Interstitial fluid concentration
    CHFf <- MHFf/VHF/(1+k_ns)
    CHFb <- MHFb/VHF
    CHTf <- MHTf/VHT /(1+k_ns)# tissue concentration
    ...
    CBrBf <- MBrBf/VBrB/(1+k_ns)
    CBrBb <- MBrBb/VBrB
    CBrF <- MBrF/VBrF  #Interstitial fluid concentration
    CBrFf <- MBrFf/VBrF/(1+k_ns)
    CBrFb <- MBrFb/VBrF
    CBrTf <-  MBrTf/VBrT/(1+k_ns) # tissue concentration
    ...
    CGoBf <- MGoBf/VGoB/(1+k_ns)
    CGoBb <- MGoBb/VGoB
    CGoF <- MGoF/VGoF #Interstitial fluid concentration
    CGoFf <- MGoFf/VGoF/(1+k_ns) 
    CGoFb <- MGoFb/VGoF
    CGoTf <-  MGoTf/VGoT/(1+k_ns) # tissue concentration
    ...
    CSKBf <- MSKBf/VSKB/(1+k_ns)
    CSKBb <- MSKBb/VSKB
    CSKF <- MSKF/VSKF  #Interstitial fluid concentration
    CSKFf <- MSKFf/VSKF/(1+k_ns)
    CSKFb <- MSKFb/VSKF
    CSKTf <- MSKTf/VSKT /(1+k_ns)# tissue concentration
    ...
    CBoBf <- MBoBf/VBoB/(1+k_ns)
    CBoBb <- MBoBb/VBoB
    CBoF <- MBoF/VBoF  #Interstitial fluid concentration
    CBoFf <- MBoFf/VBoF/(1+k_ns)
    CBoFb <- MBoFb/VBoF
    CBoTf <- MBoTf/VBoT/(1+k_ns) # tissue concentration
...
setwd("/Users/user/Documents/GitHub/PFAS_PBK_models/PFOA inhalation Rat/Scenarios/Final2/non_specific")
...
fit <- log(c(1.8e+01, 5e-05, 1e+03, 1e+02, 3.0e+00, 3.5e+00, 7e-05,
             2.316007e-01, 0.01,   1e-3, 1))


# 
# lb = c(rep(log(1e-7),3), log(1e-3), log(1e-1), log(1e-4), log(1e-6), log(1e-4),log(1e-4),log(1e-6),log(1e-6))
#  ub = c(rep(log(1e5),  3),log(1e3),  log(2e1), log(1e2),  log(0.01),  log(10),log(10),log(100),log(100))
