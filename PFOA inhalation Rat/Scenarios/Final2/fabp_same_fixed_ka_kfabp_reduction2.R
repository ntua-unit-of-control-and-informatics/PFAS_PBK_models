...
create_variable_params <- function(BW,sex,  estimated_params, fixed_params){
  ...
  RAFOat3 <- 9e3
  CL_int <- 28 #uL/min/million hepatocytes
  RAFOatp_l <- 5
  RAFUrat <- RAFOatp_k
  RAFOat1 <- RAFOat3
  ...
  CLfeces_unscaled <- 1e-4#in L/h/BW^(-0.25), scaling similar to Loccisano et al. (2012)
  CLfeces <- CLfeces_unscaled*BW^(-0.25) 
  
  f_alb_avail<-  estimated_params[1]
  f_fabp_avail  <- estimated_params[2]
  f_a2u_avail <- 1
  
  koff_alb <- estimated_params[3]
  koff_fabp <-   estimated_params[4]#
  koff_a2u <- koff_alb
  reduction_factor <-   estimated_params[5]#
  ...
  Pgap <- dif*dif_reduction*reduction_factor/ (wall_width+basement_membrane) #cm/s
  # In organs with sinusoid there is no basement membrane at gaps,
  # so we use basemenent membrane thickness = 0
  Pgap_sinusoidal <- dif*dif_reduction*reduction_factor/ wall_width #cm/s
...
fit <- log(c(1, 1,  1, 1e-3, 1e-2))



lb = c( log(1e-4),log(1e-4),log(1e-6),log(1e-6), log(1e-4))
 ub = c(  log(10),log(10),log(100),log(100),log(1))
...
estimated_params <- exp(optimizer$solution)
save.image("fabp_same_fixed_ka_kfabp_reduction2.RData")
