...
obj.func <- function(x, dataset, fixed_params){
  ...
  obs_kim_IV_Mblood <- list(exp_data[exp_data$Tissue == "Plasma", "concentration"])
    ...
    N_points <- N_points + N_kim_IV_Mblood 
     ...
     score[36] <- last_point_multiplier*AAFE(predictions = list(preds_kim_IV_Mblood[[1]][length(preds_kim_IV_Mblood[[1]])]),                     observations = list(obs_kim_IV_Mblood[[1]][length(preds_kim_IV_Mblood[[1]])]))
     N_points <- N_points + last_point_multiplier
  ##########################
  #-------------------------
  # Kemper 2003 (Worley)
...
N_genes <- N_pars
GA_results <- GA::ga(type = "real", fitness = obj.func, 
                     lower = lb,
                     upper = ub, 
                     ...
                     popSize = 4*(parallel::detectCores()), #the population size.
...
estimated_params <- exp(optimizer$solution)
save.image("fabp_same_fixed_ka_kfabp_GA.RData")
