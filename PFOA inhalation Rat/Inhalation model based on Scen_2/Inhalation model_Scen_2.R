...
create.params <- function(user.input){
  with(as.list(user.input),{
    ...
    #RoB
    PVR <- 1 - PVB - PVK - PVL - PVM - PVA - PVSP - PVH - PVBr - PVT - PVST - PVIN - PVSK - PVUA
    ...
    Dps <- c(DpKi, DpLi, DpSt, DpIn, DpMu, DpAd, DpRe, DpLu, DpSp, DpHt, DpBr, DpGo, DpSk, DpUa)
    ...
    PeffUA <- 5e-8*3600*CF_Peff #assumption
...
obj.func <- function(x, dataset){
  ...
  sex <- "M"
  inhalation_params=estimate_BF_TV(sex)
  BF = inhalation_params["BF"]# 1/h
  TV = inhalation_params["TV"]# 1/h
  duration <- 6 #hours
  admin.dose_per_g <- 1.2 # administered dose in mg/m^3
  admin.dose <- rep((admin.dose_per_g*duration*BF*TV)/(duration), duration) #ug PFOA, for 6h inhalation
