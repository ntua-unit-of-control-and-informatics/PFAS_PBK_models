...
create.params <- function(user.input){
  with(as.list(user.input),{
    ...
    # Absorption rate per area intestine
    kabs_in <- estimated_params[8] #m/h
    # Absorption rate per area stomach
    kabs_st <- estimated_params[9] #m/h
    
    # Bile correction factor
    bile_correction_factor <- estimated_params[10]
    
    # Albumin association constant
    Ka <- estimated_params[11]/1e03 #m3/mol
    ...
    Qfeces <- (8.18/0.21)*BW/24 #g/kg BW, based on Cui et al.(2010)
    ...
    VmK_Oatp_scaled <- 60*VmK_Oatp_in_vitro*MW*kidney_protein_per_gram  #physiologically scaled to in vivo, ug/L/h
    ...
    VmK_Oat1_scaled = 60*VmK_Oat1_in_vitro*MW*kidney_protein_per_gram  #physiologically scaled to in vivo, ug/L/h
    ...
    VmK_Oat3_scaled = 60*VmK_Oat3_in_vitro*MW*kidney_protein_per_gram   #physiologically scaled to in vivo, ug/L/h
    ...
    VmL_Oatp_scaled = 60*VmL_Oatp_in_vitro*MW*liver_protein_per_gram   #physiologically scaled to in vivo, ug/L/h
    ...
    VmL_Ntcp_scaled = 60*VmL_Ntcp_in_vitro*MW*liver_protein_per_gram   #physiologically scaled to in vivo, ug/L/h
...
ode.func <- function(time, inits, params){
  with(as.list(c(inits, params)),{
    ...
    # Feces
    dMfeces = Qfeces*CINL + Qbile*Cbile
...
optimizer <- nloptr::nloptr( x0= fit,
                             eval_f = obj.func,
                             lb	= c(rep(log(1e-20), N_pars-1),log(1e2)),
                             ub = c(rep(log(1e15), N_pars-1),log(1e6)),
                             opts = opts,
                             dataset = dataset)
