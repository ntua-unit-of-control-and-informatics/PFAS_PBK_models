...
create.params <- function(BW, T_water, sex, pH_w, C_water, cf_Pbw, delta_logKow){
  ...
  if(is.null(delta_logKow)){
    delta_logKow = 3.1 # Golosovskaia et al.(2024)   
  }else{
    delta_logKow = delta_logKow 
  }
  ...
  # 1st column: Fraction of lipid content (fl)
  # 2nd column: Fraction of water content (fw)
  # 3nd column: Fraction of non-lipid organic matter (NLOM) content (fnlom)
  # Grech et al. (2019)
  tissue_contents <- data.frame(matrix(data = c(
    0.03, 0.97,    # adipose
  ...
  # Add the non-lipid fraction of each tissue estimated as
  # non_lipid <- 1 - lipid_content - water_content
  tissue_contents <- cbind(tissue_contents, c(1 - tissue_contents[,1] - tissue_contents[,2]))
  colnames(tissue_contents) <- c("fl", "fw", "fnlom")
  rownames(tissue_contents) <- c("ad", "br", "git", "go", "ki", "li",
                                 "pp", "rp", "skin")
  ...
  if(is.null(cf_Pbw) ){
    ...
    Pbw <- 10^(0.78 * logKow - 0.82) # Grech et al. (2019)
  }else{
    # Pbw <- (0.008*0.3*Dow + 0.007*2.0*(Dow^0.94) + 0.134*2.9*(Dow^0.63) + 0.851)*cf_Pbw
    Pbw <- (10^(0.78 * logKow - 0.82))*cf_Pbw # Grech et al. (2019)
  }
  ...
  # Function to estimate the Partition coefficient of each tissue - Grech et al. (2019)
  P_ib <- function(fl, fw, fnlom, Pbw){
    P_iw = fw + fl * 10^(logKow) + 0.05*fnlom
    P_ib <- P_iw/Pbw
    return(P_ib)
  }
  ...
  for (i in 1:dim(tissue_contents)[1]) { #loop over all compartments
    current_input <- tissue_contents[i,] * get(paste0("V_", rownames(tissue_contents)[i]))
    PCs_vector[i] <- P_ib(current_input[1],current_input[2], current_input[3], Pbw)
    names(PCs_vector)[i] <- paste0("P_",rownames(tissue_contents)[i]) 
    assign(paste0("P_",rownames(tissue_contents)[i]), PCs_vector[[i]])
  }
  ...
  return(list(#Volume of tissues
    ...
    "CalbB"=CalbB, "Ka"=Ka, "delta_logKow"=delta_logKow
  ))  
}
...
ode.func <- function(time, inits, params){
  with(as.list(c(inits, params)),{
    ...
    dM_gill <-  F_card*(CF_ven - CF_gill / P_rp)
    dM_excr <- k_out*CF_ven + cl_feces*M_lumen
    ...
    dM_ven <- (F_li + F_rp + F_git + F_go)*(CF_li/P_li) + F_br*(CF_br/P_br) +
      ...
      F_card*CF_ven  + kx*C_water - k_out*CF_ven
    
    # Arterial blood
    dM_art <- F_card*(CF_gill / P_rp - CF_art) 
...
obj.func <- function(x, compartments, compartments_last_point, params, rest, 
  ...
  if("cf_Pbw" %in% fitted_params_names){
    cf_Pbw <- x[which(fitted_params_names == "cf_Pbw") ]
    x <- x[- which(fitted_params_names == "cf_Pbw")]
    fitted_params_names <- fitted_params_names[- which(fitted_params_names == "cf_Pbw")]
  }else{cf_Pbw <- NULL}
  
  if("delta_logKow" %in% fitted_params_names){
    delta_logKow <- x[which(fitted_params_names == "delta_logKow") ]
    x <- x[- which(fitted_params_names == "delta_logKow")]
    fitted_params_names <- fitted_params_names[- which(fitted_params_names == "delta_logKow")]
  }else{delta_logKow <- NULL}
  
  params <- create.params(BW, T_water, sex, pH_w, C_water, cf_Pbw, delta_logKow)
  
  # Check if there are any group of parameters and then assign
  # the corresponding correction factor
  if(!is.null(grouped_parameters)){
  ...
  compartments_scores <- PBKtools::AAFE(observations,predictions)
  return(compartments_scores)
}
...
prepare_plot<-function(Test){
  ...
  fitted_values <- Test$optimization$solution
  if("cf_Pbw" %in% Test$fitted_params_names){
    cf_Pbw <- fitted_values[which(Test$fitted_params_names == "cf_Pbw") ]
    fitted_values <- fitted_values[- which(Test$fitted_params_names == "cf_Pbw")]
    Test$fitted_params_names <- Test$fitted_params_names[- which(Test$fitted_params_names == "cf_Pbw")]
  }else{cf_Pbw <- NULL}
  
  if("delta_logKow" %in% Test$fitted_params_names){
    delta_logKow <- fitted_values[which(Test$fitted_params_names == "delta_logKow") ]
    fitted_values <- fitted_values[- which(Test$fitted_params_names == "delta_logKow")]
    Test$fitted_params_names <- Test$fitted_params_names[- which(Test$fitted_params_names == "delta_logKow")]
  }else{delta_logKow <- NULL}
  ...
  if(!is.null(Test$grouped_parameters)){
    ...
    for (i in 1:length(Test$fitted_params_names)) {
      if( !(Test$fitted_params_names[i] %in% names(params)) ){
        cf_vector[i] <- fitted_values[i]
      }else{
        params[[Test$fitted_params_names[i]]] <- fitted_values[i]
      }
    }
  ...
  for (i in 1:length(C_water)) {
    params_male <- create.params(BW, T_water, "Male", pH_w, C_water[i], cf_Pbw, delta_logKow)
    params_female <- create.params(BW, T_water, "Female", pH_w, C_water[i], cf_Pbw, delta_logKow)
  ...
  ###### Second Validation Test - Hagenaars et al. (2013) data
  
  C_water_concentrations <- c(0.1, 0.5, 1) # ug/ml
  sample_time <- seq(0,28,0.5)
  L = 4 # cm
  BW <- 0.0096 * L^3.55 # g Grech et al. (2019)
  T_water = 25 + 273 #K
  pH_w = 8.0
  
  predictions_male_4 <- data.frame(matrix(NA, ncol=4))
  colnames(predictions_male_4) <- c("Sex", "Concentration", "Day", "Whole_body")
  predictions_male_28 <- predictions_male_4
  predictions_female_4 <- predictions_male_4
...
for (i in 1:length(Tests_list)) {
  ...
  ggsave(paste0("Results/Correction_factor_results/Grech_QSAR/Train/", "G_Test_", i, "_Train.png" ), Test$Plots$Training_plot, width = 12, height = 8, units = "in")
  ggsave(paste0("Results/Correction_factor_results/Grech_QSAR/Validation/", "G_Test_", i, "_Validation.png" ), Test$Plots$Validation_plot, width = 12, height = 8, units = "in")
  ggsave(paste0("Results/Correction_factor_results/Grech_QSAR/Validation_Whole_Body/", "G_Test_", i, "_WB_Validation.png" ), Test$Plots$Whole_body, width = 12, height = 8, units = "in")
}


save.image("Results/Correction_factor_results/Grech_QSAR/Grech_Tests.RData")