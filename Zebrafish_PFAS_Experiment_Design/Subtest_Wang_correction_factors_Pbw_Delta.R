...
create.params <- function(BW, T_water, sex, pH_w, C_water, cf_Pbw, delta_logKow){
  ...
  # 3rd column: Fraction of water (fw)
  # 4th column: Fraction of non-lipid organic matter (NLOM) content (fnlom)
  
  tissue_contents <- data.frame(matrix(data = c(
    88.4, 	0.9, 	5.9, 	  4.8,  # adipose
    #0.8, 	  0.7, 	85.1, 	13.4, # blood contents, not needed
    4.7, 	  4.3, 	68.2, 	22.8,	# brain
    5.2, 	  2.3, 	67.0, 	25.5,	# git
    4.2, 	  2.4, 	67.2, 	26.2,	# gonads
    5.9, 	  2.3, 	70.3, 	21.5,	# kidney
    3.7, 	  2.5, 	70.4, 	23.4,	# liver
    2.4, 	  0.9, 	76.9, 	19.9,	# poorly perfused
    6.6, 	  0.8, 	58.7, 	33.8,	# richly perfused
    3.7, 	  1.2, 	69.2, 	25.9  # skin
  )/100, byrow = T, ncol = 4))
  ...
  }else{Pbw <- (0.008*0.3*Dow + 0.007*2.0*(Dow^0.94) + 0.134*2.9*(Dow^0.63) + 0.851)*cf_Pbw}
...
for (i in 1:length(Tests_list)) {
  ...
  ggsave(paste0("Results/Correction_factor_results/Wang_QSAR/subset/Train/", "WS_Test_", i, "_Train.png" ), Test$Plots$Training_plot, width = 12, height = 8, units = "in")
  ggsave(paste0("Results/Correction_factor_results/Wang_QSAR/subset/Validation/", "WS_Test_", i, "_Validation.png" ), Test$Plots$Validation_plot, width = 12, height = 8, units = "in")
  ggsave(paste0("Results/Correction_factor_results/Wang_QSAR/subset/Validation_Whole_Body/", "WS_Test_", i, "_WB_Validation.png" ), Test$Plots$Whole_body, width = 12, height = 8, units = "in")
}


save.image("Results/Correction_factor_results/Wang_QSAR/subset/Wang_Tests.RData")
