...
exp_data_plasma <- read.csv("exp_data_plasma.csv")
...
BW <- 82# average body weight in kg [ATSDR, 2016]
...
admin_dose_bolus <- c(3.96) # administered dose through bolus
...
sample_time <- seq(0,60,0.01)  #days

exp_data_short<-subset(exp_data_plasma, exp_data_plasma$time<60 )
...
datapoints <- data.frame(Cserum = exp_data_short$PFOA , time = exp_data_short$time)
simplot<-ggplot() +
  geom_line(data=solution, aes(x = time, y = CA)) +
...
# Wrapper function that takes parameter values and returns model outputs
pfoa_model <- function(user_input) {
  
  # Extract parameters from the vector
  # You'll need to decide which parameters to include in sensitivity analysis
  
  
  # Update specific parameters you want to test in sensitivity analysis
  # For example:
  
  # Add other parameters you want to test...
  params<-create.params(user_input)
  inits <- create.inits(params)
  events <- create.events(params)
  ...
  # Return outputs of interest for sensitivity analysis
  # For example: maximum concentration, AUC, concentration at specific times
  result <- c(
  ...
  if( any(!is.finite(result))){stop("eeee")}
  return(result[1])
}
...
# Choose parameters that are uncertain or known to be important
...
morris_result <- morris(
  ...
  bsup = sapply(param_ranges, function(x) x[2])
)
