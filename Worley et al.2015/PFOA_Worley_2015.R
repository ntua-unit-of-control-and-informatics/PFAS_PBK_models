...
create.params  <- function(user_input){
  with( as.list(user_input),{
    ...
    return(list( "Free" = Free, "QC" = QC, "QK" = QK, "QL" = QL, "QR" = QR, 
                 ...
                 "PL" = PL, PK = "PK", "PR" = PR, "admin.type" = admin.type,
...
ggplot(solution, aes(x = time, y = CA)) +
  geom_line() +
  scale_y_continuous(limits = c(0, 12)) +  # Set the y-axis to logarithmic scale
  theme_minimal()   # Set a minimal theme

##############################################################
# Parameters for reproducing Figure 3 of Worley et al. 2015
sex <- "M" # rat sex, values: M/F
BW <- 0.3# rat body weight in kg
...
inits <- create.inits(params)
events <- create.events(params)
sample_time <- seq(0,800,0.1)  #hours
...
ggplot(solution, aes(x = time, y = CA)) +
  geom_line() +
  scale_y_continuous(limits = c(0, 8))   # Set the y-axis to logarithmic scale
  theme_minimal() +  # Set a minimal theme
  
# Parameters for reproducing iv dose of kudo et al.2007
sex <- "F"
user_input$sex <- sex
...
inits <- create.inits(params)
events <- create.events(params)
sample_time <- seq(0,40,0.1)  #hours

solution2 <-  as.data.frame(ode(times = sample_time,  func = ode.func, y = inits, parms = params,
                                events = events, method="bdf",rtol = 1e-05, atol = 1e-05))

ggplot(solution2, aes(x = time, y = CA)) +
  geom_line() +
  scale_y_continuous(limits = c(0, 5))+   # Set the y-axis to logarithmic scale
  scale_x_continuous(limits = c(0, 100))+   # Set the y-axis to logarithmic scale
  theme_minimal()   # Set a minimal theme
  

##############################################################
# Parameters for reproducing Figure 4 of Worley et al. 2015
sex <- "M" # rat sex, values: M/F
BW <- 0.3# rat body weight in kg
admin.type <-  "iv" # administration type values: iv/oral
...
user_input$admin.dose <- admin.dose 
...
combined_solution <- rbind(transform(solution, solution = "Solution 1"), transform(solution2, solution = "Solution 2"))

# Create a plot using ggplot2
p <- ggplot(combined_solution, aes(x = time, y = CL, color = solution)) +
  geom_line() +
  scale_y_log10(limits = c(0.1, 1000)) +  # Set the y-axis to logarithmic scale
  labs(x = "Time", y = "CL") +  # Label the axes
  theme_minimal() +  # Set a minimal theme
...
# Create a plot using ggplot2
p <- ggplot(combined_solution, aes(x = time, y = CA, color = solution)) +
  geom_line() +
  scale_y_log10(limits = c(0.1, 1000)) +  # Set the y-axis to logarithmic scale
  labs(x = "Time", y = "CL") +  # Label the axes
  theme_minimal() +  # Set a minimal theme
