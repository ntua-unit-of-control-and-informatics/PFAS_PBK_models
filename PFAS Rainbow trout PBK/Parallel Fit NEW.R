...
create.params <- function(user.input){
  with(as.list(user.input),{
    ...
    return(list('F_card_ref'=F_card_ref, 'BW_ref'=BW_ref, 'KT'=KT, 
                ...
                'K_urine'=K_urine, 'Cl_urine'=Cl_urine/10,
...
parallel_func <- function(substance){
  ...
  # Consider a of the data points (CV = sd/mean)
  CV <- 100/100
  ...
  sink(file= paste0(substance, 'optimization.txt'), append = F)
...
stopCluster(cl)
Simulation_time <- Sys.time() - start_time
