...
obj.func <- function(x){
  ...
  parameters_names <- c('Ku', 'CLU_coef', 'P_liver', 'P_muscle', 'P_kidney', 
                        'P_skin', 'P_gills', 'P_carcass', 'P_viscera',
                        'Cl_feces')

  common_params <- x[1:2]
  names(common_params) <- parameters_names[1:2]
  
  substance_spcific_params <- matrix(x[-c(1,2)], nrow=length(substances), byrow = T)
  colnames(substance_spcific_params) <- parameters_names[-c(1,2)]  
  rownames(substance_spcific_params) <- substances
  ...
  for (i in 1:length(substances)) {
    ...
    current_params <- substance_spcific_params[substance,]
...
substances <- c("PFOS",  "PFOA",  "PFBS",  "PFHxS", "PFNA" )
params_names <- c('Ku', 'CLU_coef', 'P_liver', 'P_muscle', 'P_kidney', 
                  'P_skin', 'P_gills', 'P_carcass', 'P_viscera',
                  'Cl_feces')
set.seed(0)
#x0 <- runif(42)
# x0 <- c(0.8,0.06, runif(40))
x0 <- c(0.8187498, 0.06, 
        2.90263477,  0.23452522,  0.07662705,  0.58039072,  0.31280620,  0.23915067,  3.95692548,  1.29276519,  6.33000868,  0.59077799,
        0.19390634,  1.98169957,  1.28247758,  0.38611409, 19.12336480,  0.59111115,  2.80832800,  0.88032917,  0.17299274,  0.80342143,
        0.76170740,  0.02342558,  0.59956583,  0.74537611, 89.46243264,  2.40941427,  0.59407547, 17.27490113,  2.88040740,  0.99538729,
        0.00001000,  0.18889385, 85.27967857, 10.45391581,  0.35484658, 44.49696533, 49.58719995, 20.44604992,  0.00001000,  9.34961754)
start_time <- Sys.time()
optimization <- nloptr::nloptr( x0 = x0,
                                eval_f = obj.func,
                                lb	= rep(1e-05, length(x0)),
                                ub = rep(1e04, length(x0)),
                                opts = opts)

Simulatiom_time <- start_time - Sys.time()
...
common_params <- optimized_params[1:2]
names(common_params) <- parameters_names[1:2]

substance_spcific_params <- matrix(optimized_params[-c(1,2)], nrow=length(substances), byrow = T)
colnames(substance_spcific_params) <- parameters_names[-c(1,2)]
rownames(substance_spcific_params) <- substances
...
print(common_params)
print(substance_spcific_params)
