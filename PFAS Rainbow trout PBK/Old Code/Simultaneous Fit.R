...
obj.func <- function(x, user.input, N_params){
  x_all <- x
  # A vector to save the subscore for each substance
  sub_scores <-c()
  PFAS_set <- names(data_list)
  # Loop for the substances
  for (j in 1:length(PFAS_set)) {
    substance <- PFAS_set[j]
    start_point <- 1+(j-1)*N_params
    end_point <- j*N_params
    x <- x_all[start_point : end_point] # select x values based on j
    ...
    sub_scores[j] <- fitness.metric(observations,predictions)
    names(sub_scores)[j] <- paste0(substance, '_score')
...
################################################################
#x0 <- runif(9*5)
x0 <- rep(c(1.17613431, 0.08398678, 0.07102088, 0.33793084, 0.39039634, 0.23137421, 0.78637215, 0.63154606, 0.70663080), 5)
names(x0) <- rep(c('P_liver', 'P_muscle', 'P_kidney', 
                   'P_skin', 'P_gills', 'P_carcass',
                   'Cl_bile', 'Cl_feces', 'Cl_urine'), 5)
N_params <- 9 # number of parameters fitted for each substance
...
for (i in 1:length(data_list)) {
  substance <- names(data_list)[i]
  start_point <- 1+(i-1)*N_params
  end_point <- i*N_params 
  optimized_parameters[[i]] <- optimization$solution[start_point : end_point]
  names(optimized_parameters)[i] <- names(data_list)[i]
}


library(ggplot2)

draw_plots <- function(optimized_parameters){
  plots_list <- list()
  for (i in 1:length(optimized_parameters)) {
    substance <- names(optimized_parameters)[i]
    
    x_opt <- optimized_parameters[[i]]
    ...
    plots_list[[i]] <- plot
    names(plots_list)[i] <- substance
  }
  return(plots_list)
}

final_plots <- draw_plots(optimized_parameters)
