...
main_func <- function(substance){
  ...
  PBKOF <- function(observed, predicted, comp.names =NULL, data_weights){
    ...
    for (i in 1:Ncomp){
      ...
      weights_i <- data_weights[[i]] # weight factors of compartment i
      for (j in 1:N){
        # sum of relative squared errors (error = observed - predicted)
        Et <- Et + weights_i[j]*( abs(observed[[i]][j] - predicted[[i]][j])  / observed[[i]][j] )  ^2
        St <- St + weights_i[j]*( abs(observed[[i]][j] - predicted[[i]][j])  / predicted[[i]][j] )  ^2
      }
  ...
  obj.func <- function(x, user.input, substance, data_weights){
    ...
    score <- PBKOF(observations,predictions, data_weights=data_weights)
  ...
  # Calculate the weight factors of the data points
  data_weights <- Weighting.func(data_list[[substance]])
