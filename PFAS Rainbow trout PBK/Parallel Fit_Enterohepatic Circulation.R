...
main_func <- function(substance){
  create.params <- function(user.input){
    ...
    return(list('F_card_ref'=F_card_ref, 'BW_ref'=BW_ref, 'KT'=KT, 
                ...
                'Free'=Free, 'a_skin'=a_skin, 'a_muscle'=a_muscle,
                'P_viscera'=P_viscera, 'P_brain'=P_brain, 'Q_bile_coef'=Q_bile_coef,
                'f_reab'=f_reab, 'plasma'=plasma))
  }
  ...
  ode.func <- function(time, inits, params){
    with(as.list(c(inits, params)),{
      ...
      dM_lumen = f_reab*Free*Q_bile*C_liver - Ku*M_lumen - Cl_feces*M_lumen 
      
      # Viscera lumen_2- Unavailable PFAS for absorption. Can be only eliminated.
      dM_lumen_2 = (1-f_reab)*Free*Q_bile*C_liver - Cl_feces*M_lumen_2 
      ...
      dM_Liver <- Q_liver*Free*C_art + Q_viscera*Free*C_viscera/P_viscera - 
        (Q_liver + Q_viscera)*Free*C_liver/P_liver - Free*Q_bile*C_liver
