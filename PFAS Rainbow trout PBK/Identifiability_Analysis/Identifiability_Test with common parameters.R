...
obj_f <- function(x, constant_theta, constant_theta_names, params_names, constant_params=NULL,
  ...
  for (i in 1:length(substances)) {
    ...
    exp_time <- data_list[[substance]][,1]
    
    data_df <- data_list[[substance]]
    errors_df <- errors_list[[substance]]
    ...
    if(substance == 'PFOS'){
      ...
      pbk_params <- c('Ku'=Ku[[1]], 'CLU_coef'=CLU_coef[[1]], constant_params[substance,])
      names(pbk_params)[-c(1,2)] <- colnames(constant_params)
    }
...
for (k in 1:5) {
  ...
  for (i in 1:nrow(data_df)) {
    for (j in 2:ncol(data_df)) {
      set.seed(100)
      errors_list[[k]][i,j] <- abs(rnorm(1, data_df[i,j]*CV, 1))
    }
  }
  errors_list[[k]][,1] <- data_df[,1]
  colnames(errors_list[[k]]) <- c("PFOS",  "PFOA",  "PFBS",  "PFHxS", "PFNA" )
}
...
x_opt <- c(0.88273563, 0.06471162, 1.503282, 0.10870230, 0.3203126, 0.2610116,
           0.2211235, 0.10070826, 5.9215910, 0.0000100)
names(x_opt) <- c('Ku', 'CLU_coef', 'P_liver', 'P_muscle', 'P_kidney', 
                  'P_skin', 'P_gills', 'P_carcass', 'P_viscera',
                  'Cl_feces')

constant_params <- matrix(c(2.493694, 0.04653785, 1.3529777, 0.3910003, 0.4270691, 0.21971103, 6.1969100, 0.2800101,
                            1.552501, 0.14772187, 0.4135563, 0.2230857, 0.2714598, 0.11809071, 0.0000100, 0.7455554,
                            2.422851, 0.06502302, 0.3517993, 0.4348632, 0.2221513, 0.06079711, 0.7605164, 0.4192570,
                            1.142793, 0.09066439, 0.3522885, 0.3338768, 0.3129940, 0.15607105, 6.4708276, 0.0000100),
                          nrow = 4, ncol = 8, byrow = T)
colnames(constant_params) <- names(x_opt)[-c(1,2)]
rownames(constant_params) <- names(data_list)[-1]
...
thetas <- x_opt
thetas_names <- c('Ku', 'CLU_coef' , 'P_liver', 'P_muscle', 'P_kidney', 
                  'P_skin', 'P_gills', 'P_carcass', 'P_viscera',
                  'Cl_feces')
names(thetas) <- thetas_names

# lower bounds of parameters
lb <- rep(1e-08,length(thetas))
# upper bounds of parameters
ub <- rep(1e02, length(thetas))
...
global_optimum <- obj_f(x=x_opt, constant_theta = NULL, constant_theta_names = NULL, params_names = thetas_names, constant_params = constant_params, data_df = data_list, errors_df = errors_list) #global_optimization$objective
