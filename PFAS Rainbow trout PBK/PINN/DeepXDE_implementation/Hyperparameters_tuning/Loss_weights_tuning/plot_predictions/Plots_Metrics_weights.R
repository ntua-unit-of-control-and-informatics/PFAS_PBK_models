...
plot_with_legend <- create_plot(1, 'bottom', plots_titles[1])
...
final_plot <- grid.arrange(arrangeGrob(plot_1, plot_2, plot_3, plot_4, plot_5, plot_6, plot_7, plot_8, ncol = 3),
...
estimate_scores <- function(i){
  ...
  # Keep the predictions only for the time points at which there are available data
  predictions_df <- solution[solution$time %in% PFOS_data$Time,c('time', 'C_Liver', 'C_Blood', 
                                                                 'C_Skin', 'C_Muscle', 'C_Gills',
                                                                 'C_Kidney', 'C_Carcass')]
  names(predictions_df)[1] <- 'Time'
  predictions_df[,-1] <- predictions_df[,-1]*1000
  ...
  for (z in 2:dim(predictions_df)[2]) {
    observations[[z-1]] <- PFOS_data[,z]
    names(observations)[z-1] <- colnames(PFOS_data)[z] 
    predictions[[z-1]] <- predictions_df[,z]
    names(predictions)[z-1] <- colnames(predictions_df)[z]
  }
...
model_names <- c(0.01, 0.1,1,2,5,10,100)

for (i in 1:8) {
  scores_df[i,1] <- paste0('model_',model_names[i])
  scores_df[i,2:5] <-  estimate_scores(i)
}
...
#write.csv(csv_df, 'scores_from_kinetic_model.csv', row.names = F)