# This code estimates the scores for the parametric set estimated from the 
# nloptr optimization.
...
#                       1.5685318, 0.11316576, 0.4398762, 0.2715996, 0.2291869, 0.1074253, 3.699111)
...
names(optimized_params) <- parameters_names
...
solution <- data.frame(deSolve::ode(times = sample_time,  func = ode.func, y = inits, 
                                    parms = c(optimized_params, params),
                                    events = events,
                                    method="lsodes",rtol = 1e-03, atol = 1e-03))

# Keep the predictions only for the time points at which there are available data
results <- solution[solution$time %in% PFOS_data$Time,c('time', 'C_Liver', 'C_Blood', 
                                                      'C_Skin', 'C_Muscle', 'C_Gills',
                                                      'C_Kidney', 'C_Carcass')]
# Transform predictions and data_df in list data type
observations <- list()
predictions <- list()
for (z in 2:dim(PFOS_data)[2]) {
  observations[[z-1]] <- PFOS_data[,z]
  names(observations)[z-1] <- colnames(PFOS_data)[z] 
...
PBKtools::AAFE(predictions, observations)

scores <- c()
for (i in 1:length(observations)) {
  obs <- observations[[i]]
  preds <- predictions[[i]]
  scores[i] <- PBKtools::mse_custom(obs, preds)
}
print(mean(scores))

PBKtools::mse_custom(observations, predictions)