  ...
  for (i in 1:length(substances)) {
    ...
    for (k in 1:nrow(data_df)) {
      for (j in 2:ncol(data_df)) {
        set.seed(0)
        errors_df[k,j] <- data_df[k,j]*CV #abs(rnorm(1, data_df[k,j]*CV, 0.1))
      }
    }
...
set.seed(10)
#x0 <- c(runif(5*7+3))
x0 <- c(1.46792293, 0.06880127, 0.61853442,
        2.046785, 0.1436849, 0.04693691, 0.3876488, 0.3107156, 0.15220979,  7.286974519,
        4.143644, 0.1039690, 0.13053486, 1.1266312, 1.0532043, 0.48664445,  1.756689310,
        1.472783, 0.1599029, 0.02339682, 0.2281978, 0.2385370, 0.09879311,  0.001473912,
        10.000000, 0.7439051, 0.10625280, 4.0428805, 1.8525773, 0.48380285, 10.000000000,
        1.941927, 0.1566788, 0.05300477, 0.5616889, 0.5137844, 0.26365135,  7.274143975)
