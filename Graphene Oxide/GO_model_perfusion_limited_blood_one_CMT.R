...
# based on the work of Li et al., 2013, https://doi.org/10.3109/17435390.2013.863406
  ...
  #========================================================================================================
    ...
    dMB = QLn*(CLn/coef_lung) -QKi*CB -QLi*CB -QSt*CB -QSIn*CB -QLIn*CB -
          QSpl*CB -QH*CB -QBr*CB -QRe*CB -QLn*CB +QKi*(CKi/coef_kidney) +QLitot*(CLi/coef_liver) +
          QH*(CH/coef_heart) +QBr*(CBr/coef_brain) +QRe*(CRe/coef_rob)
      
    #Kidney
    #tissue subcompartment
    dMKi = QKi*(CB - (CKi/coef_kidney)) - CLurine*MKi #Qurine*MKi
    
    #Liver
    #tissue subcompartment
    dMLi =  QLi*CB - QLitot*(CLi/coef_liver) + QSt*(CSt/coef_stomach) + QSpl*(CSpl/coef_spleen) + 
    ...
    #Stomach
    #tissue subcompartment 
    dMSt = QSt*(CB - (CSt/coef_stomach))
    
    #Small Intestine
    #tissue subcompartment 
    dMSIn = QSIn*(CB - (CSIn/coef_smallIn)) 
    
    #Large Intestine
    #tissue subcompartment 
    dMLIn = QLIn*(CB - (CLIn/coef_largeIn)) - CLfeces*CLIn #Qfeces*CLIn 
    
    #Lung 
    #tissue subcompartment
    dMLn = QLn*(CB - (CLn/coef_lung))
    
    #Spleen
    #tissue subcompartment 
    dMSpl = QSpl*(CB - (CSpl/coef_spleen))
    
    #Heart
    #tissue subcompartment 
    dMH = QH*(CB - (CH/coef_heart))
    
    #Brain
    #Tissue subcompartment 
    dMBr = QBr*(CB - (CBr/coef_brain))
    
    #Rest of body
    #Tissues ubcompartment 
    dMRe = QRe*(CB - (CRe/coef_rob))
...
create.events <- function(parameters){
  with(as.list(parameters), {
    ...
    if (ltimes != ldose){
      ...
      if (admin.type == "iv"){
        events <- list(data = rbind(data.frame(var = c("MB"),  time = admin.time, 
