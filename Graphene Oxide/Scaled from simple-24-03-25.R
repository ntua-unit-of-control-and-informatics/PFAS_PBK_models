...
pbkCode <- nimbleCode({
  ...
  for(i in 1:nObs) {
    ...
    pred[i] <- max(xOde[timeIndex[i], obsComp[i]], 1e-10)
    
    y[i] ~ dlnorm(log(pred[i]+ 1e-10), sd = sigma)
...
runPBPKModel <- function() {
  ...
  nimble_constants <- list(
    ...
    nParams = 12, # Adjust based on your params vector
    nObs = nrow(exp_data),
  ...
  # Set up initial values for MCMC
  mcmc_inits <- list(
    coef_liver = 0.5,
    coef_spleen = 0.5,
    coef_kidney = 0.5,
  ...
  mcmcConf <- configureMCMC(
    model = pbkModel,
    monitors = c("coef_liver", "coef_spleen","coef_kidney",
  ...
  # Compile the MCMC
  cMcmc <- compileNimble(mcmc, project = pbkModel)
  ...
  samples <- runMCMC(mcmc = cMcmc, niter = niter, nburnin = nburnin, 
                     nchains = nchains, inits = chain_inits, 
  ...
  gelmanDiag = gelman.diag(samples)
  ))
}
  ...
  # Example of using updated parameters to run the model
  plot_updated_model <- function(results) {
    # Get original parameters
    BW <- 0.04
    admin.dose_per_kg <- 1
    admin.dose <- admin.dose_per_kg * BW * 1e03
    user_input <- list(
      'BW' = BW,
      "admin.dose" = admin.dose,
      "np_size" = 3/2,
    ...
    # Get original and updated parameters
    original_params <- create_fixed_params(user_input)
    updated_params <- update_params_with_mcmc(original_params, results$summary)
    
    # Run both models
    time_points <- seq(0, 10, 0.05)
    original_solution <- run_ode_model(create.inits(original_params), time_points, original_params)
    updated_solution <- run_ode_model(create.inits(updated_params), time_points, updated_params)
    
    # Plot comparison (example for liver)
    plot(
      time_points, original_solution[, "Mliver"], 
      type = "l", col = "blue", lty = 2,
      xlab = "Time (min)", ylab = "Mass in Liver (ug)",
      main = "Comparison of Original vs Calibrated Model"
    )
    lines(time_points, updated_solution[, "Mliver"], col = "red")
    legend(
      "topright", 
