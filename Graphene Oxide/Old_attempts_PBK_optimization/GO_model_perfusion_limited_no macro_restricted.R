...
# based on the work of Li et al., 2013, https://doi.org/10.3109/17435390.2013.863406
    ...
    if(np_size == "np_size_small"){
      ...
      coef_stomach <- estimated_params[1]
      coef_smallIn <- estimated_params[2]
      coef_largeIn <- estimated_params[3]
      coef_spleen <- estimated_params[4]
      coef_brain <- estimated_params[5]
      coef_heart <- estimated_params[6]
      CLurine <- estimated_params[7]
      CLfeces <- estimated_params[8]
      
    }else{
      
      coef_stomach <- estimated_params[9]
      coef_smallIn <- estimated_params[10]
      coef_largeIn <- estimated_params[11]
      coef_spleen <- estimated_params[12]
      coef_brain <- estimated_params[13]
      coef_heart <- estimated_params[14]
      CLurine <- estimated_params[15]
    ...
    coef_liver <- estimated_params[18]
    coef_kidney <- estimated_params[19]
...
N_pars <- 20 # Number of parameters to be fitted
fit <-  c(rep(log(1), 20))

lb = c(rep(log(1e-10),8), rep(log(1),6), rep(log(1e-10),6))
ub = c(rep(log(1e10),8), rep(log(1e5),6), rep(log(1e-10),6))
