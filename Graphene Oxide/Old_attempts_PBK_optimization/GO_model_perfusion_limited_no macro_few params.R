...
# based on the work of Li et al., 2013, https://doi.org/10.3109/17435390.2013.863406
    ...
    if(np_size == "np_size_small"){
      ...
      coef_spleen <- estimated_params[1]
      coef_lung <- estimated_params[2]
      coef_brain <- estimated_params[3]
      CLurine <- estimated_params[4]
      CLfeces <- estimated_params[5]
      
    }else{
      
      coef_spleen <- estimated_params[6]
      coef_lung <- estimated_params[7]
      coef_brain <- estimated_params[8]
      CLurine <- estimated_params[9]
      CLfeces <- estimated_params[10]
    }
    
    coef_liver <- estimated_params[11]
    coef_kidney <- estimated_params[12]
    coef_heart <- estimated_params[13]
    coef_rob <- estimated_params[14]
    coef_stomach <- estimated_params[15]
    coef_smallIn <- estimated_params[16]
    coef_largeIn <- estimated_params[17]
...
lb = c(rep(log(1e-10),17))
ub = c(rep(log(1e10),17))
