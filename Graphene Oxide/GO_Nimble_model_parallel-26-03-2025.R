...
R_ode <- function(y, times, params) {
  ...
  complete_params <- c(fixed_params_global, variable_params)
  
  events <- create.events(complete_params)
  inits <- create.inits(complete_params)
...
runPBPKModel <- function(x, dataset = NULL){
  ...
  tissue_map <- list( "heart" = which(colnames(solution) == "Mheart"), 
                      "liver" = which(colnames(solution) == "Mliver"),
                      "spleen" = which(colnames(solution) == "Mspleen"),
                      "stomach" = which(colnames(solution) == "Mstomach"),
                      "kidneys" = which(colnames(solution) == "Mkidneys"),
                      "lungs" = which(colnames(solution) == "Mlungs"),
                      "brain" = which(colnames(solution) == "Mbrain"),
                      "small_intestine" = which(colnames(solution) == "Msmall_intestine"),
                      "large_intestine" = which(colnames(solution) == "Mlarge_intestine"))
  ...
  test_solution <- run_ode_model(inits, sample_time, fixed_params)
  print("ODE solution column names:")
  print(colnames(test_solution))
  print("Tissue indices corresponding to experimental data:")
  for (i in 1:nrow(exp_data)) {
    tissue <- as.character(exp_data$Tissue[i])
    time_point <- exp_data$time[i]
    time_idx <- match(time_point, sample_time)
    comp_idx <- tissue_map[[tissue]]
    
    cat(sprintf("Tissue: %s, Time: %.2f, ODE Time Index: %d, Compartment Index: %d\n",
                tissue, time_point, time_idx, comp_idx))
    
    cat(sprintf("Data value: %.4f, ODE prediction: %.4f\n",
                exp_data$mass[i], test_solution[time_idx, comp_idx]))
  }
 ...
 # Create initial values for each chain
  ...
  clusterExport(cl, c("pbkCode", "nimble_constants", "nimble_data", "chain_inits", 
                      "niter", "nburnin", "configureMCMC", "buildMCMC", 
                      "compileNimble", "runMCMC", "nimbleModel", "create.events",
                      "create.inits", "ode.func", "create_variable_params", 
                      "fixed_params_global", "user_input_global"), 
                envir = environment())
  ...
  samples_list <- parLapply(cl, 1:nchains, function(i) {
  library(nimble)
  library(coda)
  library(deSolve)
...
results <- runPBPKModel(x = c(rep(log(0.5), 10), log(0.05), log(0.05)))
