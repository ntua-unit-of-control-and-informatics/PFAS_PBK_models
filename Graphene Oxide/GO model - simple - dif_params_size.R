...
create.params <- function(user.input){
  with(as.list(user.input),{
    ...
    #as a percentage of cardiac output (Qcardiac L/min), which itself is a function
    #of body weight (BW)
    
    Qcardiac <- 0.275*(BW^0.75)/60 # L/min Brown et al. 1997
    PQBKi <- 9.1/100 #Brown et al. 1997, p 438, Table 23
    QBKi <- PQBKi * Qcardiac #L/min
    PQBLi <- 16.1/100
    QBLi <- PQBLi * Qcardiac #L/min Brown et al. 1997, p 438, Table 23
    PQBSt <- 1e-3 #assumption, same as small intestine, check
    QBSt <- PQBSt * Qcardiac #L/min
    PQBSIn <- 1e-3
    QBSIn <- PQBSIn * Qcardiac #L/min
    PQBLIn <- 1e-3 #assumption, same as small intestine, check
    QBLIn <- PQBLIn * Qcardiac #L/min
    PQBLn <- 1 
    QBLn <- PQBLn * Qcardiac #L/min
    PQBSpl <- 1e-3 #assumption, same as small intestine, check 
    QBSpl <- PQBSpl * Qcardiac #L/min
    PQBH <- 6.6/100 #Brown et al. 1997, p 438, Table 23
    QBH <- PQBH * Qcardiac #L/min
    PQBBr <- 3.3/100 #Brown et al. 1997, p 438, Table 23
    QBBr <- PQBBr * Qcardiac #L/min
    ...
    PQBRe = 1 - PQBKi - PQBLi - PQBSt - PQBSIn - PQBLIn - PQBSpl - PQBH - PQBBr
...
# based on the work of Li et al., 2013, https://doi.org/10.3109/17435390.2013.863406
    ...
    }else if(np_size == np_size_large){
      
      coef_heart <- estimated_params[14]
      coef_lung <- estimated_params[15]
      coef_liver <- estimated_params[16]
      coef_spleen <- estimated_params[17]
      coef_kidney <- estimated_params[18]
      coef_stomach <- estimated_params[19]
      coef_smallIn <- estimated_params[20]
      coef_largeIn <- estimated_params[21]
      coef_brain <- estimated_params[22]
      coef_rob <- estimated_params[23]
      CLE_feces <- estimated_params[24]
      CLEh <- estimated_params[25]
      CLE_urine <- estimated_params[26]
    ...
    #Stomach
    #blood subcompartment
    dMBSt = QBSt*CBart - QBSt*CStB - QparaSt*(1-SSt)*CStB - coef_stomach*QBSt*(CStB-CSt)
    #tissue subcompartment 
    dMSt = QparaSt*(1-SSt)*CStB + coef_stomach*QBSt*(CStB-CSt)
    #lumen
    dMStlumen = - QGE*MStlumen
    
    #Small Intestine
    #blood subcompartment
    dMBSIn = QBSIn*CBart - QBSIn*CSInB - QparaSIn*(1-SSIn)*CSInB - coef_smallIn*QBSIn*(CSInB-CSIn)
    #tissue subcompartment 
    dMSIn = QparaSIn*(1-SSIn)*CSInB + QGE*MStlumen + coef_smallIn*QBSIn*(CSInB-CSIn)
    
    
    #Large Intestine
    #blood subcompartment
    dMBLIn = QBLIn*CBart - QBLIn*CLInB - QparaLIn*(1-SLIn)*CLInB - coef_largeIn*QBLIn*(CLInB-CLIn)
    #tissue subcompartment 
    ...
    #Lung 
    #blood subcompartment
    dMBLn = QBLn*CBven- QBLn*CLnB - QparaLn*(1-SLn)*CLnB - coef_lung*QBLn*(CLnB-CLn)
    #tissue subcompartment
    dMLn = QparaLn*(1-SLn)*CLnB + coef_lung*QBLn*(CLnB-CLn)
    
    
    #Spleen
    #blood subcompartment
    dMBSpl = QBSpl*CBart - QBSpl*CSplB - QparaSpl*(1-SSpl)*CSplB - coef_spleen*QBSpl*(CSplB-CSpl)
    #tissue subcompartment 
    dMSpl = QparaSpl*(1-SSpl)*CSplB + coef_spleen*QBSpl*(CSplB-CSpl)
    
    
    #Heart
    #blood subcompartment
    dMBH = QBH*CBart - QBH*CHB - QparaH*(1-SHt)*CHB - coef_heart*QBH*(CHB-CH)
    #tissue subcompartment 
    dMH = QparaH*(1-SHt)*CHB + coef_heart*QBH*(CHB-CH)
    
    
    #Brain
    #blood subcompartment
    dMBBr = QBBr*CBart - QBBr*CBrB - QparaBr*(1-SBr)*CBrB - coef_brain*QBBr*(CBrB-CBr)
    #Tissue subcompartment 
    dMBr = QparaBr*(1-SBr)*CBrB + coef_brain*QBBr*(CBrB-CBr)
    
    
    #Rest of body
    #blood subcompartment
    dMBRe = QBRe*CBart - QBRe*CReB - QparaRe*(1-SRe)*CReB - coef_rob*QBRe*(CReB-CRe)
    #interstitial fluid subcompartment 
    dMRe = QparaRe*(1-SRe)*CReB + coef_rob*QBRe*(CReB-CRe)
    ...
    list(c( "dMBart"=dMBart, "dMBven"=dMBven, "dMBKi"=dMBKi, "dMKi"=dMKi,
            "dMBLi"=dMBLi, "dMLi"=dMLi, "dMBSt"=dMBSt, "dMSt"=dMSt, "dMStlumen"=dMStlumen,
           ...
           'CBven'=CBven, 'CBart'=CBart, 'CKiB'=CKiB, 'CKi'=CKi, 'CLiB'=CLiB, 'CLi'=CLi,
           'CStB'=CStB, 'CSt'=CSt, 'CSInB'=CSInB, 'CSIn'=CSIn, 'CLInB'=CLInB,
...
create.inits <- function(parameters){
  with(as.list(parameters),{
    ...
    return(c("MBart"=MBart, "MBven"=MBven, "MBKi"=MBKi, "MKi"=MKi,
             "MBLi"=MBLi, "MLi"=MLi, "MBSt"=MBSt, "MSt"=MSt, "MStlumen"=MStlumen,
             "MBSIn"=MBSIn, "MSIn"=MSIn, "MBLIn"=MBLIn, "MLIn"=MLIn, 
             "MLInlumen"=MLInlumen, "MBLn"=MBLn, "MLn"=MLn, "MBSpl"=MBSpl,
             "MSpl"=MSpl, "MBH"=MBH, "MH"=MH, "MBBr"=MBBr, "MBr"=MBr,
             "MBRe"=MBRe, "MRe"=MRe, "Murine"=Murine, "Mfeces"=Mfeces,
...
obj.func <- function(x, dataset){
  ...
  #2nd case, Liu (2012) 1 mg/kg small_tissues_diff_time_points
  ...
  #5th case, Liu (2012) 10 mg/kg small_tissues
  ...
  # Set up simulations for the 3rd case, i.e. Liu (2012) 1 mg/kg large_blood_diff_time_points
...
N_pars <- 26 # Number of parameters to be fitted
fit <-  c(rep(log(1),26))

lb = c(rep(log(1e-10),26))
ub = c(rep(log(1e10),26))
