...
runPBPKModel <- function() {
  ...
  # Modified MCMC configuration to print progress
  mcmcConf <- configureMCMC(
    model = pbkModel,
    monitors = c("coef_liver", "coef_spleen", "coef_kidney",
                 "coef_heart", "coef_lung", "coef_brain", "coef_rob", 
                 "coef_stomach", "coef_smallIn", "coef_largeIn",
                 "CLurine", "CLfeces","sigma"),
    # Add a custom sampler that prints progress
    control = list(
      adaptInterval = 100,  # This can also serve as a progress indicator
      adaptationIterations = nburnin,
      printProgress = function(iteration, niter) {
        if (iteration %% 100 == 0) {
          elapsed_time <- difftime(Sys.time(), start_time, units = "mins")
          progress_pct <- (iteration / niter) * 100
          
          cat(sprintf(
            "Progress: %d/%d iterations (%.2f%%) - Elapsed: %.2f mins\n", 
            iteration, niter, progress_pct, as.numeric(elapsed_time)
          ))
        }
      }
    )
  ...
  mcmc <- buildMCMC(mcmcConf)
  cMcmc <- compileNimble(mcmc, project = pbkModel, resetFunctions = TRUE)
