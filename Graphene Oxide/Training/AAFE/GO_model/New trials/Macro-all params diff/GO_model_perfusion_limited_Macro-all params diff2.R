...
# based on the work of Li et al., 2013, https://doi.org/10.3109/17435390.2013.863406
    ...
    if(np_size == "np_size_small"){
      ...
      coef_rob <- estimated_params[6]
      coef_stomach <- estimated_params[7]
      coef_smallIn <- estimated_params[8]
      coef_largeIn <- estimated_params[9]
      coef_brain <- estimated_params[10]
      ...
      coef_heart <- estimated_params[16]
      coef_lung <- estimated_params[17]
      coef_rob <- estimated_params[18]
    ...
    Pup_Li <- estimated_params[24]
    Km_Li <- estimated_params[25]
    CLup <- estimated_params[26]
    ...
    Cmacro_Ln = Mmacro_Ln/V_macro_Ln #macrophages concentration
  ...
  #========================================================================================================
    ...
    dMBart =  QLn*(CLn/coef_lung) -QKi*CBart -QLi*CBart -QSt*CBart -QSIn*CBart -QLIn*CBart -
              QSpl*CBart -QH*CBart -QBr*CBart -QRe*CBart - CLurine*CBart
    ...
    dMKi = QKi*(CBart - (CKi/coef_kidney)) 
    ...
    dMLi =  QLi*CBart - QLitot*(CLi/coef_liver) + QSt*(CSt/coef_stomach) + QSpl*(CSpl/coef_spleen) + 
            ...
            Pup_Li*V_macro_Li*CLi*(1-(Cmacro_Li/(Km_Li + Cmacro_Li))) - CLfeces*CLi
    ...
    #macrophages Lung
    dMmacro_Ln = Pup_Ln*V_macro_Ln*CLn*(1-(Cmacro_Ln/(Km_Ln + Cmacro_Ln))) - CLup*Cmacro_Ln*V_macro_Ln
    
    #Spleen
    #tissue subcompartment 
    dMSpl = QSpl*(CBart - (CSpl/coef_spleen)) - Pup_Spl*V_macro_Spl*CSpl*(1-(Cmacro_Spl/(Km_Spl + Cmacro_Spl)))
    #macrophages Spleen
    dMmacro_Spl = Pup_Spl*V_macro_Spl*CSpl*(1-(Cmacro_Spl/(Km_Spl + Cmacro_Spl))) - CLup*Cmacro_Spl*V_macro_Spl
    ...
    # Urine
    dMurine = CLurine*CBart #Qurine*MKi
    # Feces
    dMfeces = CLfeces*CLi #Qfeces*CLIn
    
    dMmacro = CLup*Cmacro_Spl*V_macro_Spl + CLup*Cmacro_Li*V_macro_Li + CLup*Cmacro_Ln*V_macro_Ln
    ...
    Clungs <-  (MLn + Mmacro_Ln) /VLn
    Crest <-  MRe/VRe
    Cspleen <- (MSpl + Mmacro_Spl)/VSpl
    Cheart <- MH/VH
    Cbrain <-  MBr/VBr
    Cmacro <- Mmacro/(V_macro_Li + V_macro_Spl)
    ...
    list(c( 
            ...
            "dMLn"=dMLn, "dMmacro_Ln"=dMmacro_Ln,
            "dMSpl"=dMSpl, "dMmacro_Spl"=dMmacro_Spl, "dMH"=dMH, "dMBr"=dMBr,
...
create.inits <- function(parameters){
  with(as.list(parameters),{
    ...
    MSIn<-0; MLIn<-0; MLn<-0; Mmacro_Ln <-0;MSpl<-0; Mmacro_Spl <-0; MH<-0; MBr<-0;
    MRe<-0; Murine<-0; Mfeces<-0; Mmacro <-0; Vurine <-0; Vfeces <-0
    
    
    return(c(
             "MBart"=MBart, "MBven"=MBven,"MKi"=MKi,
             "MLi"=MLi, "Mmacro_Li"=Mmacro_Li,"MSt"=MSt, "MSIn"=MSIn,
             "MLIn"=MLIn,"MLn"=MLn, "Mmacro_Ln"=Mmacro_Ln,
             "MSpl"=MSpl, "Mmacro_Spl"=Mmacro_Spl,
             "MH"=MH, "MBr"=MBr,"MRe"=MRe,
             "Murine"=Murine, "Mfeces"=Mfeces, "Mmacro"=Mmacro,
