...
# based on the work of Li et al., 2013, https://doi.org/10.3109/17435390.2013.863406
    ...
    if(np_size == "np_size_small"){
      ...
      coef_liver <- estimated_params[4]
      coef_spleen <- estimated_params[5]
      coef_kidney <- estimated_params[6]
  
    }
    
    coef_heart <- estimated_params[7]
    coef_lung <- estimated_params[8]
    coef_rob <- estimated_params[9]
    coef_stomach <- estimated_params[10]
    coef_smallIn <- estimated_params[11]
    coef_largeIn <- estimated_params[12]
    coef_brain <- estimated_params[13]
    
    Pup_Li <- estimated_params[14]
    Km_Li <- estimated_params[15]
    CLup <- estimated_params[16]
    
    CLurine <- estimated_params[17]
    CLfeces <- estimated_params[18]
...
N_pars <- 18 # Number of parameters to be fitted
fit <-  c(rep(log(1), 18))
# 
# lb = c(rep(log(1e-9),15))
# ub = c(rep(log(1e9),15))
