...
# based on the work of Li et al., 2013, https://doi.org/10.3109/17435390.2013.863406
    ...
    coef_lung <- estimated_params[4]
    coef_heart <- estimated_params[5]
    ...
    CLurine <- estimated_params[14]
    CLfeces <- estimated_params[15]
    ...
    #tissue subcompartment
    dMLi =  QparaLi*(1-SLi)*CLiB + coef_liver*(QBLi+QBSpl+QBSIn+QBLIn+QBSt)*(CLiB - CLi)  
    #macrophages
    ...
    #tissue subcompartment 
    dMSpl = QparaSpl*(1-SSpl)*CSplB + coef_spleen*QBSpl*(CSplB-CSpl) - Pup*V_macro_Spl*CSplB*(1-(Cmacro_Spl/(Km + Cmacro_Spl)))
    #macrophages
...
lb = c(rep(log(1e-10),15))
ub = c(rep(log(1e10),15))
# 
