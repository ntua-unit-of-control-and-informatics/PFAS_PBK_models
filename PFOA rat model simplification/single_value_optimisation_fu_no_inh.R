...
create_fixed_params <- function(user.input){
  with(as.list(user.input),{
    ...
    return(list(
      ...
      'VLuF'=VLuF,'VLuAF'=VLuAF, 'VLuT'=VLuT, 
...
ode.func <- function(time, inits, params){
  ...
  with(as.list(c(inits, params)),{
    ...
    dMLuAF_tot = kLuTLuAF*(CLuTf-CLuAFf) - (VmLu_Oatp_ap*CLuAFf/(KmLu_Oatp_ap+CLuAFf)) 
    ...
    CBALF <-  MLuAF / VBALF_Gustaffson# This is for the Gustaffson et al. (2022) study which used 
    ...
    Clungs_bled <-  (MLuB + MLuF+ MLuT + MLuAF)/(VLu_tot-VLuB_lv)
    Clungtissue_bled <- (MLuB + MLuF+ MLuT)/(VLuB+VLuF+VLuT-VLuB_lv)
    ...
    "CBile" = CBile, 'CalveolarLF' = CalveolarLF,
  ...
  with(as.list(parameters), {
    ...
    if (ltimes != ldose){
      ...
      if (admin.type == "iv"){
        ...
        stop("Unsupported administration type. Use 'iv' or 'oral'.")
      }
    }
    return(events)
  })
...
lb = c(rep((1e-4),2), rep((1e-7),3), (1e-3), (1e-1), (1e-8), (5e3), (1e-6),(1e-3))
ub = c(rep((1e2),2), rep((1e5),3), (1e3),  (2e1), (1e2),(1e6), (1e-3), (1e2))
