...
create.events <- function(parameters){
  with(as.list(parameters),{
    ...
    if (ltimes_oral != ldose_oral){
      ...
      events_oral <- data.frame(var = c(rep(c('C_water'), ltimes_oral)), 
...
ode.func <- function(time, inits, params, custom.func){
  with(as.list(c(inits, params)),{
    # Transform Texp from Celsium to Kelvin
    Texp <- Texp + 273
...
user.input <- list(Texp = 19,      # C
                   ...
                   admin.time_oral=c(0,500),
                   C_water =	c(0.4, 0)	# PFOS concentration in water (ng/L)
)
