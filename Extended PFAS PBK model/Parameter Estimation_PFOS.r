  ...
  create.params <- function(user_input,variables,PC) {
    ...
    PAdi <-(1-Htc) * PC["Adipose","PFOS"]/4 # adipose tissue:plasma;
    PBra <- (1-Htc) * PC["Brain","PFOS"]/4 # brain:plasma;
    PGon <- (1-Htc) * PC["Gonads","PFOS"]/2 # gonads:plasma;
    PGI <-  (1-Htc) * PC["Gut","PFOS"] # GI tract:plasma;
    PHea <- (1-Htc) * PC["Heart","PFOS"] # heart:plasma;
    PL <-   (1-Htc) * PC["Liver","PFOS"] # liver:plasma;
    PLun <- (1-Htc) * PC["Lung","PFOS"]/4  # lung:plasma;
    PMus <- (1-Htc) * PC["Muscle","PFOS"]/10 # muscle:plasma;
    PSki <- (1-Htc) * PC["Skin","PFOS"]   # skin:plasma;
    PSpl <- (1-Htc) * PC["Spleen","PFOS"]  # spleen:plasma;
  ...
  kbilec <-0#variables["kbilec", "PFOS"]  # biliary elimination rate (1/(day*BW^-0.25))
  ...
  obj.func<-function(x, weight, threshold, metric){
    ...
    ,weight = weight,threshold = threshold ,times = time_plasma)
      ...
      score_plasma <- rmsd(preds_plasma, plasma_exp$PFOS
                           ,weight = weight,threshold = threshold ,times = time_plasma)
    }
    ...
    # return((score_plasma*length(plasma_exp$PFOS) + score_urine*length(exp_data_urine) +
    #  score_feces*length(exp_data_feces))
    # /(length(plasma_exp$PFOS) + length(exp_data_urine) + length(exp_data_feces)))
  }
  #---------------------------------#
  ...
  #   filter(time > 6, PFOS>0) %>% 
  #   select(time, PFOS) %>% 
  #   mutate(PFOS = PFOS / 1000) %>%  # Convert from ng/g to ug/g
  #   arrange(time)
  ...
  # # Interpolate PFOS onto dense grid & compute cumulative mass
  # feces_cum_dense <- approx(feces_ext$time, feces_ext$PFOS, xout = t_dense_feces)$y %>% 
  ...
  # urine_cum_dense <- approx(urine_ext$time, urine_ext$PFOS, xout = t_dense_urine)$y %>% 
  ...
  plasma_exp <- read.csv("exp_data_plasma.csv")%>% select("time","PFOS") #%>%filter(time<=28)#%>%
  ...
  # plasma_art<- data.frame(time = c(total_time), PFOS = C_el)
  ...
  plot2 <- ggplot()+
    ...
    geom_point(data = plasma_exp, aes(x = time/365, y = PFOS, color='CA'), size=5)+
    #geom_point(data = plasma_art, aes(x = time/365, y = PFOS, color='CA'), size=5, shape=17)+
    labs(title = "Predicted vs Observed Values",
...
conc_exp <- data.frame(Subject = "Experimental", Time = plasma_exp$time, Concentration = plasma_exp$PFOS)
...
# # Optional: Save to CSV
  # write.csv(nca_results_df, "NCA_results_PFOS.csv", row.names = FALSE)
  # write.csv(c(params,x_opt), "Optimized_Parameters_PFOS.csv", row.names = TRUE)
