...
variables_df <- data.frame(
  PFHpA = c(364.06, 4.50E-03, 3300, 263, 4500, 60, 3.24E-02, 2.15E-04, 1.69E-03, 3.8, 3.076585024),
  PFOA = c(414.07, 2.45E-03, 2500, 137.5, 4500, 47, 0.007757613, 0.005284911, 2.40E-03, 3.96, 2.4),
  PFNA = c(464.08, 1.87E-03, 1000, 42, 8500, 58, 1.72, 1.26E-03, 2.04E-03, 4.02, 2.52077),
  PFDA = c(514.09, 6.18E-04, 2500, 137.5, 6000, 39, 78.37437317, 7.29E-05, 0.002076146, 4.01, 0.156992915),
  PFBS = c(300.1, 3.65E-02, 2500, 137.5, 4500, 47, 2.47E-04, 5.55E-04, 0.00E+00, 18.42, 7.378217792),
  PFHxS = c(403.11, 6.95E-04, 5800, 778, 7300, 92, 44.76000058, 0.000165342, 2.40E-03, 3.76, 0.959152914),
  PFOS = c(500.13, 2.18E-03, 2500, 137.5, 2200, 48, 0.000424071, 0.700484202, 0, 3.78, 0.069918193),
  DONA = c(378.07, 0.0268, 2500, 137.5, 4500, 39, 5.68E-05, 8.11E-05, 0.015495299, 3.71, 98.46893334),
  HFPO_DA = c(330.0489, 0.00385, 2500, 137.5, 4500, 47, 0.089824574, 0, 0.055013665, 19.58, 0.936736109),
  PFBA = c(214.04, 2.29E-01, 2500, 137.5, 4500, 47, 0.00E+00, 2.26E-04, 0.00E+00, 4.01, 3.62E+01),
  PFHxA = c(314.05, 3.80E-02, 2500, 137.5, 4500, 47, 4.98E-03, 0.00E+00, 0.00E+00, 3.99, 0)
)select(all_of(chemical)) 

# Set the row names to match the 'Parameters' column
rownames(variables_df) <- c('MW', 'Free', 'Vmax_baso_invitro', 'Km_baso', 'Vmax_apical_invitro', 
 ...
 PC<- data.frame(
  ...
  HFPO_DA = c(0.265822654, 0.167241461, 0.335472967, 0.300497404, 0.363279275, 1.489884277, 0.642857005, 0.350626131, 0.297788612, 0.405961151),
...
"Lung", "Muscle", "Skin", "Spleen")
    ...
    PAdi <- PC["Adipose",] # adipose tissue:blood;
    PBra <- PC["Brain",]  # brain:blood;
    PGon <- PC["Gonads",]  # gonads:blood;
    PGI <- PC["Gut",]# GI tract:blood;
    PHea <- PC["Heart",]  # heart:blood;
    PL<- PC["Liver",]  # liver:blood;
    PLun <- PC["Lung",]  # lung:blood;
    PMus <- PC["Muscle",] # muscle:blood;
    PSki <- PC["Skin",]  # skin:blood;
    PSpl <- PC["Spleen",]  # spleen:blood;
