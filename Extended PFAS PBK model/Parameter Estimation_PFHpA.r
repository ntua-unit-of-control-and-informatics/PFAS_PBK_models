  ...
  create.params <- function(user_input,variables,PC) {
    ...
    MW <- variables["MW","PFHpA"]  # PFHpA molecular mass (g/mol)
    Free <- variables["Free","PFHpA"]  # free fraction in plasma (Smeltz 2023); fitted to model

    # --- Kidney Transport Parameters ---
    Vmax_baso_invitro <- variables["Vmax_baso_invitro","PFHpA"]  # Vmax of basolateral transporter (pmol/mg protein/min)
    Km_baso <- variables["Km_baso","PFHpA"] * variables["MW","PFHpA"]  # Km of basolateral transporter (ug/L)
    Vmax_apical_invitro <- variables["Vmax_apical_invitro", "PFHpA"]  # Vmax of apical transporter (pmol/mg protein/min)
    Km_apical <- variables["Km_apical", "PFHpA"] * variables["MW", "PFHpA"]  # Km of apical transporter (ug/L)
    ...
    PAdi <- (1-Htc)*PC["Adipose","PFHpA"] # adipose tissue:plasma;
    PBra <- (1-Htc)*PC["Brain","PFHpA"] # brain:plasma;
    PGon <- (1-Htc)*PC["Gonads","PFHpA"] # gonads:plasma;
    PGI <-  (1-Htc)*PC["Gut","PFHpA"] # GI tract:plasma;
    PHea <- (1-Htc)*PC["Heart","PFHpA"] # heart:plasma;
    PL <-   (1-Htc)*PC["Liver","PFHpA"]*2 # liver:plasma;
    PLun <- (1-Htc)*PC["Lung","PFHpA"]  # lung:plasma;
    PMus <- (1-Htc)*PC["Muscle","PFHpA"] # muscle:plasma;
    PSki <- (1-Htc)*PC["Skin","PFHpA"]   # skin:plasma;
    PSpl <- (1-Htc)*PC["Spleen","PFHpA"]  # spleen:plasma;
    ...
    Vmax_basoC <- (Vmax_baso_invitro * RAFbaso * PTC * protein * 60 * (variables["MW", "PFHpA"] / 1e12) * 1e6) * 24
    Vmax_apicalC <- (Vmax_apical_invitro * RAFapi * PTC * protein * 60 * (variables["MW", "PFHpA"] / 1e12) * 1e6) * 24
  ...
  ode.func <- function(time, inits, params) {
    with(as.list(c(inits, params)), {
      ...
      CA <- CA_free / Free  # concentration of total PFHpA in plasma (ug/L)
  ...
  admin_dose_bolus <- c(variables["admin_dose_bolus","PFHpA"])  # administered dose through bolus (ug)
  ...
  #============================
  RAFbaso <-variables["RAFbaso", "PFHpA"]  # relative activity factor, basolateral transporters
  RAFapi <- variables["RAFapi", "PFHpA"] # relative activity factor, apical transporter
  keffluxc <-variables["keffluxc", "PFHpA"] # rate of efflux from PTC to blood (1/(day*BW^-0.25))
  kbilec <-variables["kbilec", "PFHpA"]  # biliary elimination rate (1/(day*BW^-0.25))
  ...
  obj.func<-function(x, weight, threshold, metric){
    ...
    score_plasma <- AAFE(preds_plasma, plasma_exp$PFHpA
    ,weight = weight,threshold = threshold ,times = time_plasma)
    }else if(metric == "rmsd"){
      score_plasma <- rmsd(preds_plasma, plasma_exp$PFHpA
                           ,weight = weight,threshold = threshold ,times = time_plasma)
    }
    ...
    # return((score_plasma*length(plasma_exp$PFHpA) + score_urine*length(exp_data_urine) +
    #  score_feces*length(exp_data_feces))
    # /(length(plasma_exp$PFHpA) + length(exp_data_urine) + length(exp_data_feces)))
  }
  #---------------------------------#
  ...
  feces_exp <- cumulative_exp_data(exp_data_feces, "time", "PFHpA", "feces.weight") %>% 
  ...
  #   filter(time > 6, PFHpA>0) %>% 
  #   select(time, PFHpA) %>% 
  #   mutate(PFHpA = PFHpA / 1000) %>%  # Convert from ng/g to ug/g
  #   arrange(time)
  ...
  # # Interpolate PFHpA onto dense grid & compute cumulative mass
  # feces_cum_dense <- approx(feces_ext$time, feces_ext$PFHpA, xout = t_dense_feces)$y %>% 
  ...
  urine_exp <- cumulative_exp_data(exp_data_urine, "time", "PFHpA", "urine.volume") %>% 
  ...
  # urine_cum_dense <- approx(urine_ext$time, urine_ext$PFHpA, xout = t_dense_urine)$y %>% 
  ...
  plasma_exp <- read.csv("exp_data_plasma.csv")%>% select("time","PFHpA") #%>%filter(time<=28)#%>%
  ...
  # plasma_art<- data.frame(time = c(total_time), PFHpA = C_el)
  ...
  plot2 <- ggplot()+
    ...
    geom_point(data = plasma_exp, aes(x = time/365, y = PFHpA, color='CA'), size=5)+
    labs(title = "Predicted vs Observed Values",
...
conc_exp <- data.frame(Subject = "Experimental", Time = plasma_exp$time, Concentration = plasma_exp$PFHpA)
...
# # Optional: Save to CSV
 write.csv(nca_results_df, "NCA_results_PFHpA.csv", row.names = FALSE)
 write.csv(c(params,x_opt), "Optimized_Parameters_PFHpA.csv", row.names = TRUE)
