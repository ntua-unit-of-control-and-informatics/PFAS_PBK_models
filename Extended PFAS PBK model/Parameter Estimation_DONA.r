  ...
  create.params <- function(user_input,variables,PC) {
    ...
    MW <- variables["MW","DONA"]  # DONA molecular mass (g/mol)
    Free <- variables["Free","DONA"]  # free fraction in plasma (Smeltz 2023); fitted to model

    # --- Kidney Transport Parameters ---
    Vmax_baso_invitro <- variables["Vmax_baso_invitro","DONA"]  # Vmax of basolateral transporter (pmol/mg protein/min)
    Km_baso <- variables["Km_baso","DONA"] * variables["MW","DONA"]  # Km of basolateral transporter (ug/L)
    Vmax_apical_invitro <- variables["Vmax_apical_invitro", "DONA"]  # Vmax of apical transporter (pmol/mg protein/min)
    Km_apical <- variables["Km_apical", "DONA"] * variables["MW", "DONA"]  # Km of apical transporter (ug/L)
    ...
    PAdi <- (1-Htc)*PC["Adipose","DONA"] # adipose tissue:plasma;
    PBra <- (1-Htc)*PC["Brain","DONA"] # brain:plasma;
    PGon <- (1-Htc)*PC["Gonads","DONA"] # gonads:plasma;
    PGI <-  (1-Htc)*PC["Gut","DONA"] # GI tract:plasma;
    PHea <- (1-Htc)*PC["Heart","DONA"] # heart:plasma;
    PL <-   (1-Htc)*PC["Liver","DONA"] # liver:plasma;
    PLun <- (1-Htc)*PC["Lung","DONA"]  # lung:plasma;
    PMus <- (1-Htc)*PC["Muscle","DONA"] # muscle:plasma;
    PSki <- (1-Htc)*PC["Skin","DONA"]   # skin:plasma;
    PSpl <- (1-Htc)*PC["Spleen","DONA"]  # spleen:plasma;
    ...
    Vmax_basoC <- (Vmax_baso_invitro * RAFbaso * PTC * protein * 60 * (variables["MW", "DONA"] / 1e12) * 1e6) * 24
    Vmax_apicalC <- (Vmax_apical_invitro * RAFapi * PTC * protein * 60 * (variables["MW", "DONA"] / 1e12) * 1e6) * 24
  ...
  ode.func <- function(time, inits, params) {
    with(as.list(c(inits, params)), {
      ...
      CA <- CA_free / Free  # concentration of total DONA in plasma (ug/L)
  ...
  admin_dose_bolus <- c(variables["admin_dose_bolus","DONA"])  # administered dose through bolus (ug)
  ...
  #============================
  RAFbaso <-variables["RAFbaso", "DONA"]  # relative activity factor, basolateral transporters
  RAFapi <- variables["RAFapi", "DONA"] # relative activity factor, apical transporter
  keffluxc <-variables["keffluxc", "DONA"] # rate of efflux from PTC to blood (1/(day*BW^-0.25))
  kbilec <-variables["kbilec", "DONA"]  # biliary elimination rate (1/(day*BW^-0.25))
  ...
  obj.func<-function(x, weight, threshold, metric){
    ...
    score_plasma <- AAFE(preds_plasma, plasma_exp$DONA
    ,weight = weight,threshold = threshold ,times = time_plasma)
    }else if(metric == "rmsd"){
      score_plasma <- rmsd(preds_plasma, plasma_exp$DONA
                           ,weight = weight,threshold = threshold ,times = time_plasma)
    }
    ...
    # return((score_plasma*length(plasma_exp$DONA) + score_urine*length(exp_data_urine) +
    #  score_feces*length(exp_data_feces))
    # /(length(plasma_exp$DONA) + length(exp_data_urine) + length(exp_data_feces)))
  }
  #---------------------------------#
  ...
  feces_exp <- cumulative_exp_data(exp_data_feces, "time", "DONA", "feces.weight") %>% 
  ...
  #   filter(time > 6, DONA>0) %>% 
  #   select(time, DONA) %>% 
  #   mutate(DONA = DONA / 1000) %>%  # Convert from ng/g to ug/g
  #   arrange(time)
  ...
  # # Interpolate DONA onto dense grid & compute cumulative mass
  # feces_cum_dense <- approx(feces_ext$time, feces_ext$DONA, xout = t_dense_feces)$y %>% 
  ...
  urine_exp <- cumulative_exp_data(exp_data_urine, "time", "DONA", "urine.volume") %>% 
  ...
  # urine_cum_dense <- approx(urine_ext$time, urine_ext$DONA, xout = t_dense_urine)$y %>% 
  ...
  plasma_exp <- read.csv("exp_data_plasma.csv")%>% select("time","DONA") #%>%filter(time<=28)#%>%
  ...
  # plasma_art<- data.frame(time = c(total_time), DONA = C_el)
  ...
  plot2 <- ggplot()+
    ...
    geom_point(data = plasma_exp, aes(x = time/365, y = DONA, color='CA'), size=5)+
    #geom_point(data = plasma_art, aes(x = time/365, y = DONA, color='CA'), size=5, shape=17)+
    labs(title = "Predicted vs Observed Values",
...
conc_exp <- data.frame(Subject = "Experimental", Time = plasma_exp$time, Concentration = plasma_exp$DONA)
...
#  write.csv(nca_results_df, "NCA_results_DONA.csv", row.names = FALSE)
#  write.csv(c(params,x_opt), "Optimized_Parameters_DONA.csv", row.names = TRUE)
