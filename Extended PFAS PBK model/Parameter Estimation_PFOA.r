  ...
  create.params <- function(user_input,variables,PC) {
    ...
    MW <- variables["MW","PFOA"]  # PFOA molecular mass (g/mol)
    Free <- variables["Free","PFOA"]  # free fraction in plasma (Smeltz 2023); fitted to model

    # --- Kidney Transport Parameters ---
    Vmax_baso_invitro <- variables["Vmax_baso_invitro","PFOA"]  # Vmax of basolateral transporter (pmol/mg protein/min)
    Km_baso <- variables["Km_baso","PFOA"] * variables["MW","PFOA"]  # Km of basolateral transporter (ug/L)
    Vmax_apical_invitro <- variables["Vmax_apical_invitro", "PFOA"]  # Vmax of apical transporter (pmol/mg protein/min)
    Km_apical <- variables["Km_apical", "PFOA"] * variables["MW", "PFOA"]  # Km of apical transporter (ug/L)
    ...
    PAdi <- (1-Htc)* PC["Adipose","PFOA"]/4 # adipose tissue:plasma;
    PBra <- (1-Htc)* PC["Brain","PFOA"] # brain:plasma;
    PGon <- (1-Htc)* PC["Gonads","PFOA"] # gonads:plasma;
    PGI <-  (1-Htc)* PC["Gut","PFOA"]/3 # GI tract:plasma;
    PHea <- (1-Htc)* PC["Heart","PFOA"] # heart:plasma;
    PL <-   (1-Htc)* PC["Liver","PFOA"]*1.8 # liver:plasma;
    PLun <- (1-Htc)* PC["Lung","PFOA"]/4  # lung:plasma;
    PMus <- (1-Htc)* PC["Muscle","PFOA"]/4 # muscle:plasma;
    PSki <- (1-Htc)* PC["Skin","PFOA"]/1.8   # skin:plasma;
    PSpl <- (1-Htc)* PC["Spleen","PFOA"]*2  # spleen:plasma;
    ...
    Vmax_basoC <- (Vmax_baso_invitro * RAFbaso * PTC * protein * 60 * (variables["MW", "PFOA"] / 1e12) * 1e6) * 24
    Vmax_apicalC <- (Vmax_apical_invitro * RAFapi * PTC * protein * 60 * (variables["MW", "PFOA"] / 1e12) * 1e6) * 24
  ...
  admin_dose_bolus <- c(variables["admin_dose_bolus","PFOA"])  # administered dose through bolus (ug)
  ...
  #============================
  RAFbaso <-100#variables["RAFbaso", "PFOA"]  # relative activity factor, basolateral transporters
  RAFapi <-0.6#variables["RAFapi", "PFOA"] # relative activity factor, apical transporter
  keffluxc <-2.6#variables["keffluxc", "PFOA"] # rate of efflux from PTC to blood (1/(day*BW^-0.25))
  kbilec <-0.01#variables["kbilec", "PFOA"]  # biliary elimination rate (1/(day*BW^-0.25))
  ...
  obj.func<-function(x, weight, threshold, metric){
    ...
    ,weight = weight,threshold = threshold ,times = time_plasma)
      ...
      score_plasma <- rmsd(preds_plasma, plasma_exp$PFOA
                           ,weight = weight,threshold = threshold ,times = time_plasma)
    }
  ...
  #   filter(time > 6, PFOA>0) %>% 
  #   select(time, PFOA) %>% 
  #   mutate(PFOA = PFOA / 1000) %>%  # Convert from ng/g to ug/g
  #   arrange(time)
  ...
  # # Interpolate PFOA onto dense grid & compute cumulative mass
  # feces_cum_dense <- approx(feces_ext$time, feces_ext$PFOA, xout = t_dense_feces)$y %>% 
  ...
  # urine_cum_dense <- approx(urine_ext$time, urine_ext$PFOA, xout = t_dense_urine)$y %>% 
  ...
  lower_bounds <- c("RAFbaso" = 1e-03,
                    "RAFapi"= 1e-05#,
                    #"kbilec"= 5e-02,
                    #keffluxc"= 1e-05
                                        )

  upper_bounds <- c("RAFbaso"=1e05,
                    "RAFapi"= 1e02#,
                    #"kbilec"= 1e01,
                    #"keffluxc"= 1e02
                    )    
...
conc_exp <- data.frame(Subject = "Experimental", Time = plasma_exp$time, Concentration = plasma_exp$PFOA)
...
# # Optional: Save to CSV
 write.csv(nca_results_df, "NCA_results_PFOA.csv", row.names = FALSE)
 write.csv(c(params,x_opt), "Optimized_Parameters_PFOA.csv", row.names = TRUE)
